...
async function mapTextToStatEntry(text: string, statEffects: StatValue[][], id: number, spellStatEffects: Map<number, StatValue[][]>, isEnchant: boolean): Promise<[string, StatEntry]> {
	...
	const pattern = text.replace(/[+-]?(?:\$(?:(\{.*?\})|(?:\/\d+;)?(\d*)([a-z])(\d?))|([\d\.]+(?<=\d)))/g, function(match, expression: string, alternateSpellID: string, identifier: string, identifierIndex: string, plainNumber: string, _offset: number, input: string) {
