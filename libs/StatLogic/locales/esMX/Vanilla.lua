-- THIS FILE IS AUTOGENERATED. Add new entries by editing & running GenerateStatLocales.mts or StatLocaleData.json instead.
local addonName, addon = ...
if GetLocale() ~= "esMX" then return end
local StatLogic = LibStub(addonName)
local Stats = StatLogic.Stats

local W = addon.WholeTextLookup
W["reduce el daño infligido pero proporciona la posibilidad de silenciar al golpear"] = { [Stats.AverageWeaponDamage] = -300, } -- s4242
W["calienta la sangre y calma los nervios"] = { [Stats.Agility] = 5, [Stats.Stamina] = 5, } -- s8553
W["portas el toque de zanzil. el veneno recorre tus venas. ¡busca ayuda!"] = { [Stats.Agility] = -15, } -- s9991
W["controla la mente de brasaliza cuando le falla la voluntad"] = { [Stats.MeleeHaste] = -25, } -- s16053
W["aumenta tu velocidad de regeneración de vida y la velocidad de movimiento"] = { [Stats.HealthRegen] = 20, } -- s17625
W["sufrimiento de taelan"] = { [Stats.MeleeHaste] = 100, [Stats.AverageWeaponDamage] = 30, } -- s18810
W["¡qué rápido!"] = { [Stats.RangedHaste] = 150, [Stats.MeleeHit] = -75, [Stats.RangedHit] = -75, } -- s23674
W["se garantiza que el próximo ataque de tu mascota asestará un golpe crítico si tiene la capacidad de asestar golpes críticos"] = { [Stats.MeleeCrit] = 100, [Stats.RangedCrit] = 100, } -- s24353
W["te sientes con más ánimo"] = { [Stats.Spirit] = 50, } -- s25947
W["¡felicita las fiestas a un amigo!"] = { [Stats.Spirit] = 20, } -- s26004
W["¡brinda por tu amigo!"] = { [Stats.Stamina] = 20, } -- s26008
W["combina un fardo de tarjetas, un saco de pan casero y una colección de juramentos de ventormenta para formar una colección de regalos de ventormenta"] = { [Stats.Intellect] = 30, } -- s27664
W["combina una caja de cerveza casera, una colección de juramentos de forjaz y una caja de tarjetas para formar una colección de regalos de forjaz"] = { [Stats.Stamina] = 30, } -- s27665
W["combina una caja de talla de madera, una colección de juramentos de darnassus y un montón de tarjetas para formar una colección de regalos de darnassus"] = { [Stats.Agility] = 30, } -- s27666
W["combina una caja de tartas recientes, una colección de juramentos de orgrimmar y un paquete de tarjetas para formar una colección de regalos de orgrimmar"] = { [Stats.Agility] = 30, } -- s27669
W["combina una cesta de flores, una cartera de tarjetas y una colección de juramentos de cima del trueno para formar una colección de regalos de cima del trueno"] = { [Stats.Stamina] = 30, } -- s27670
W["combina un libro de poemas románticos, un fajo de tarjetas y una colección de juramentos de entrañas para formar una colección de regalos de entrañas"] = { [Stats.Intellect] = 30, } -- s27671
W["raise a toast to your friend!"] = { [Stats.Stamina] = 10, [Stats.Stamina] = 10, } -- s29006
W["los ataques cuerpo a cuerpo del maligno de las sombras succionan energía del objetivo y la convierten en maná para su amo"] = { [Stats.MeleeHit] = 9, [Stats.RangedHit] = 9, } -- s401984
W["tu próximo ataque será un golpe crítico"] = { [Stats.MeleeCrit] = 100, [Stats.RangedCrit] = 100, } -- s418508
W["otorga un pequeño beneficio al daño y al poder de sanación de tu grupo"] = { [Stats.AverageWeaponDamage] = 5, [Stats.SpellDamage] = 10, [Stats.HealingPower] = 16, } -- s418510
W["agotado"] = { [Stats.MeleeHaste] = -100, } -- s424574
W["permite a quien lo ingiere hablar con el cadáver de un individuo con asuntos pendientes en el mundo mortal. también puede curar la indigestión"] = { [Stats.AllStats] = 1, } -- s426606
W["aumenta notablemente el daño cuando tienes inspiración"] = { [Stats.MeleeHaste] = 20, [Stats.AverageWeaponDamage] = 25, } -- s436430
W["el próximo ataque de tu mascota será un golpe crítico, si es que el ataque tiene la capacidad de asestar golpes críticos"] = { [Stats.MeleeCrit] = 100, [Stats.RangedCrit] = 100, } -- s462214
W["juzgado por el poder de la crematoria. aturdido. reduce la armadura y la probabilidad de esquivar"] = { [Stats.Dodge] = -50, } -- s1229508
W["hendido"] = { [Stats.AverageWeaponDamage] = -10, } -- e27
W["aceite de zahorí menor"] = { [Stats.SpellPower] = 8, } -- e2623
W["aceite de maná menor"] = { [Stats.GenericManaRegen] = 4, } -- e2624
W["aceite de maná inferior"] = { [Stats.GenericManaRegen] = 8, } -- e2625
W["aceite de zahorí inferior"] = { [Stats.SpellPower] = 16, } -- e2626
W["aceite de zahorí"] = { [Stats.SpellPower] = 24, } -- e2627
W["aceite de zahorí luminoso"] = { [Stats.SpellPower] = 36, [Stats.SpellCrit] = 1, } -- e2628
W["aceite de maná luminoso"] = { [Stats.GenericManaRegen] = 12, [Stats.HealingPower] = 25, } -- e2629
W["incendio"] = { [Stats.SpellCrit] = 15, } -- e6729
W["maestría con escudos"] = { [Stats.BlockChance] = 10, } -- e6876
W["maestro tirador"] = { [Stats.MeleeCrit] = 5, [Stats.RangedCrit] = 5, } -- e6889
W["fanatismo"] = { [Stats.SpellCrit] = 18, } -- e6967
W["reflejos felinos"] = { [Stats.Dodge] = 20, } -- e6990
W["aceite de maná brazanegra"] = { [Stats.SpellHit] = 2, [Stats.GenericManaRegen] = 12, } -- e7099
W["moledora automática de muchedumbre"] = { [Stats.MeleeHaste] = 50, } -- e7123
W["recubrimiento de escudo conductor"] = { [Stats.SpellPower] = 24, } -- e7602
W["ley de la naturaleza"] = { [Stats.SpellDamage] = 30, [Stats.HealingPower] = 55, } -- e7603
W["especialización de sanación"] = { [Stats.HealingPower] = 26, } -- e7638
W["especialización de meditación"] = { [Stats.GenericManaRegen] = 5, } -- e7639
W["lustre trol magnífico"] = { [Stats.SpellPower] = 36, [Stats.SpellCrit] = 1, } -- e7644
W["estadísticas de vida"] = { [Stats.AllStats] = 4, [Stats.NatureResistance] = 15, } -- e7645
W["repelente encantado"] = { [Stats.SpellPower] = 45, [Stats.SpellCrit] = 1, } -- e7650
W["bloqueador prístino"] = { [Stats.BlockValue] = 30, } -- e7676
W["experto en puyazos"] = { [Stats.MeleeCrit] = 5, [Stats.RangedCrit] = 5, } -- e7696
W["hechizos refinados"] = { [Stats.MeleeCrit] = 2, [Stats.RangedCrit] = 2, [Stats.SpellCrit] = 2, } -- e7708
W["jefe"] = { [Stats.SpellCrit] = 2, } -- e7825
W["guardián ancestral"] = { [Stats.BlockChance] = 10, } -- e7839
W["lunático"] = { [Stats.MeleeHit] = 3, [Stats.RangedHit] = 3, [Stats.SpellHit] = 3, } -- e7866
W["pericia animal"] = { [Stats.DodgeReduction] = 5, [Stats.ParryReduction] = 5, } -- e7875

local L = addon.StatIDLookup
L["aumenta tu velocidad de ataque un %s% durante %s s"] = { { Stats.MeleeHaste, Stats.RangedHaste, }, false, } -- s28701
L["inutiliza al objetivo, lo que reduce un %s% su velocidad de movimiento, aumenta un %s% el tiempo entre ataques cuerpo a cuerpo y aumenta un %s% el tiempo entre ataques a distancia. dura %s s"] = { false, { Stats.MeleeHaste, }, { Stats.RangedHaste, }, false, reduction = true } -- s89
L["el druida ruge y disminuye %s p. el poder de ataque cuerpo a cuerpo de los enemigos cercanos. dura %s s"] = { { Stats.AttackPower, }, false, reduction = true } -- s99
L["aumenta %s p. la armadura.  si un enemigo golpea al taumaturgo, su movimiento puede verse ralentizado un %s% y el tiempo entre sus ataques puede aumentar un %s% durante %s s. un mago solo puede tener un tipo de hechizo de armadura activo a la vez.  dura %s min"] = { { Stats.Armor, }, false, { Stats.MeleeHaste, }, false, false, } -- s168
L["ralentiza el movimiento del enemigo un %s% y su velocidad de ataque un %s% durante %s s"] = { false, { Stats.MeleeHaste, }, false, reduction = true } -- s246
L["restaura %s p. de maná durante %s s. debes permanecer sentado mientras bebes"] = { { Stats.GenericManaRegen, }, false, } -- s430
L["otorga %s p. de armadura extra a los miembros del grupo en un radio de %s m. los jugadores solo pueden tener un aura a la vez por cada paladín"] = { { Stats.Armor, }, false, } -- s465
L["disminuye la agilidad del objetivo en %s p"] = { { Stats.Agility, }, reduction = true } -- s474
L["una ráfaga de energía sagrada que aumenta %s p. la armadura del taumaturgo. cada golpe de daño cuerpo a cuerpo o a distancia contra el sacerdote resta una carga. dura %s min o hasta que se hayan utilizado %s cargas"] = { { Stats.Armor, }, false, false, } -- s588
L["una ráfaga de energía sagrada, que aumenta la armadura del taumaturgo en %s p. cada golpe de daño cuerpo a cuerpo o a distancia contra el sacerdote restará una carga. dura %s min o hasta que se hayan usado %s cargas"] = { { Stats.Armor, }, false, false, } -- s602
L["controla una mente humanoide de un nivel máximo de %s, pero aumenta el tiempo entre ataques un %s%. dura un máximo de %s min"] = { false, { Stats.MeleeHaste, }, false, reduction = true } -- s605
L["protege al paladín de todo el daño y los hechizos durante %s s, pero aumenta un %s% el daño entre tus ataques. una vez protegido, el objetivo no puede volverse invulnerable nuevamente con escudo divino, protección divina ni bendición de protección durante %s s"] = { false, { Stats.MeleeHaste, }, false, reduction = true } -- s642
L["da %s p. de armadura extra a los miembros del grupo en un radio de %s m. los jugadores solo pueden tener un aura con ellos por paladín a la vez"] = { { Stats.Armor, }, false, } -- s643
L["aumenta %s p. la armadura durante %s h"] = { { Stats.Armor, }, false, } -- s673
L["protege al taumaturgo, aumenta %s p. su armadura y restaura %s p. de salud cada %s s durante %s min"] = { { Stats.Armor, }, { Stats.HealthRegen, }, false, false, } -- s687
L["protege al taumaturgo aumentando su armadura en %s p. y también restaura %s p. de salud cada %s s. durante %s min"] = { { Stats.Armor, }, { Stats.HealthRegen, }, false, false, } -- s696
L["reduce %s p. el daño que inflige el objetivo durante %s min. un brujo solo puede tener una maldición activa sobre cada objetivo"] = { { Stats.AverageWeaponDamage, }, false, reduction = true } -- s702
L["lanza sobre el objetivo una maldición de temeridad, aumentando su poder de ataque cuerpo a cuerpo en %s p. pero disminuyendo su armadura en %s p. durante %s min. los enemigos malditos no huirán e ignorarán los efectos del miedo y el horror. un brujo solo puede tener una maldición activa a la vez sobre un mismo objetivo"] = { { Stats.AttackPower, }, { Stats.Armor, }, false, } -- s704
L["aumenta la fuerza %s p. y te inflige %s p. de daño cada %s s durante %s min"] = { { Stats.Strength, }, false, false, false, } -- s806
L["protege al taumaturgo, lo que aumenta %s p. la armadura y %s p. la resistencia a las sombras, y restaura %s p. de salud cada %s s durante %s min"] = { { Stats.Armor, }, { Stats.ShadowResistance, }, { Stats.HealthRegen, }, false, false, } -- s706
L["reduce la armadura del objetivo %s p. durante %s s. mientras dura su efecto, el objetivo no puede utilizar el sigilo ni hacerse invisible"] = { { Stats.Armor, }, false, reduction = true } -- s770
L["disminuye la armadura del objetivo en %s p. durante %s s. mientras dura su efecto, el objetivo no puede utilizar el sigilo ni hacerse invisible"] = { { Stats.Armor, }, false, reduction = true } -- s778
L["sana instantáneamente %s p. de daño. también restaura %s p. de maná durante %s s. debes permanecer sentado mientras bebes"] = { false, { Stats.GenericManaRegen, }, false, } -- s833
L["lanza sobre el enemigo una maldición de imbecilidad, disminuyendo su intelecto y su espíritu en %s p. cada %s s hasta un total de %s p. cada una. un brujo solo puede tener una maldición activa a la vez sobre un mismo objetivo"] = { { Stats.Intellect, Stats.Spirit, }, false, { Stats.Intellect, }, reduction = true } -- s1010
L["protege al taumaturgo, lo que aumenta %s p. la armadura y %s p. la resistencia a las sombras y restaura %s p. de salud cada %s s durante %s min"] = { { Stats.Armor, }, { Stats.ShadowResistance, }, { Stats.HealthRegen, }, false, false, } -- s1086
L["subyuga al demonio objetivo, de nivel %s como máximo, y lo obliga a seguir tus órdenes. mientras está subyugado, el tiempo entre los ataques del demonio aumenta un %s% y su velocidad de lanzamiento de hechizos se reduce un %s%. dura un máximo de %s min. si subyugas reiteradamente al mismo demonio, cada vez será más difícil de controlar"] = { false, { Stats.MeleeHaste, }, { Stats.SpellHaste, }, false, reduction = true } -- s1098
L["aumenta la probabilidad de esquivar en un %s%"] = { { Stats.Dodge, }, } -- s1114
L["aumenta %s p. la armadura de un objetivo aliado durante %s min"] = { { Stats.Armor, }, false, } -- s1126
L["aumenta la resistencia a la escarcha y a las sombras %s p. durante %s min"] = { { Stats.FrostResistance, Stats.ShadowResistance, }, false, } -- s1138
L["aumenta el tiempo entre los ataques del objetivo un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, reduction = true } -- s1139
L["reduce %s p. el poder de ataque cuerpo a cuerpo de todos los enemigos en un radio de %s m durante %s s"] = { { Stats.AttackPower, }, false, false, reduction = true } -- s1160
L["infunde poder al objetivo y aumenta su aguante en %s p. durante %s min"] = { { Stats.Stamina, }, false, } -- s1243
L["aumenta %s p. el intelecto del objetivo durante %s min"] = { { Stats.Intellect, }, false, } -- s1459
L["aumenta el intelecto del objetivo en %s p. durante %s min"] = { { Stats.Intellect, }, false, } -- s1460
L["maldice al objetivo durante %s min, disminuye %s p. su resistencia al fuego y a la escarcha y aumenta el daño de fuego y de escarcha que recibe un %s%. un brujo solo puede tener una maldición activa a la vez sobre un mismo objetivo"] = { false, { Stats.FireResistance, Stats.FrostResistance, }, false, reduction = true } -- s1490
L["obliga al objetivo a hablar en demoníaco, lo que ralentiza el lanzamiento de todos los hechizos un %s%. solo puede haber una maldición por brujo activa en un objetivo. dura %s s"] = { { Stats.SpellHaste, }, false, reduction = true } -- s1714
L["el guerrero obtendrá un aumento del %s% de la probabilidad de golpe crítico y será inmune a los efectos de miedo durante los próximos %s s"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, false, } -- s1719
L["el guerrero obtendrá un aumento del %s% de la probabilidad de golpe crítico y será inmune a los efectos de miedo durante los próximos %s s, pero todo el daño recibido aumenta un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, false, false, } -- s1719
L["el druida ruge y disminuye en %s p. el poder de ataque cuerpo a cuerpo de los enemigos cercanos. dura %s s"] = { { Stats.AttackPower, }, false, reduction = true } -- s1735
L["aumenta tu habilidad con magia sagrada %s p. y reduce tu resistencia a las sombras %s p"] = { false, { Stats.ShadowResistance, }, reduction = true } -- s2123
L["aumenta %s p. la fuerza durante %s h"] = { { Stats.Strength, }, false, } -- s2367
L["aumenta %s p. la agilidad durante %s h"] = { { Stats.Agility, }, false, } -- s2374
L["aumenta %s p. el intelecto durante %s h"] = { { Stats.Intellect, }, false, } -- s2376
L["aumenta %s p. la salud máxima del jugador durante %s h"] = { { Stats.Health, }, false, } -- s2378
L["aumenta la probabilidad de bloquear un %s% durante %s s, pero solo bloquea %s ataque"] = { { Stats.BlockChance, }, false, false, } -- s2565
L["aumenta la probabilidad de bloquear un %s% durante %s s, pero solo bloquea %s ataques"] = { { Stats.BlockChance, }, false, false, } -- s2565
L["restaura %s p. de salud y %s p. de maná durante %s s. debes permanecer sentado mientras comes"] = { false, { Stats.GenericManaRegen, }, false, } -- s2639
L["reduce %s p. el daño a distancia recibido y aumenta la probabilidad de esquivar un %s% durante %s s"] = { false, { Stats.Dodge, }, false, } -- s2651
L["fuerza aumentada en %s p"] = { { Stats.Strength, }, } -- s2692
L["aumenta %s p. el daño con armas afiladas durante %s minutos"] = { { Stats.AverageWeaponDamage, }, false, } -- s2828
L["aumenta de forma permanente en %s p. el valor de armadura de un objeto de piernas, torso, manos o pies"] = { { Stats.Armor, }, } -- s2831
L["aumenta de forma permanente en %s p. el valor de armadura de un objeto de piernas, torso, manos o pies. solo se puede aplicar en objetos de nivel %s o superior"] = { { Stats.Armor, }, false, } -- s2833
L["aumenta la probabilidad de conseguir un golpe crítico con hechizos en un %s%"] = { { Stats.SpellCrit, }, } -- s2916
L["aumenta %s p. el daño físico que inflige el taumaturgo durante %s s"] = { { Stats.AverageWeaponDamage, }, false, } -- s3019
L["pica al enemigo y reduce la fuerza y la agilidad en %s p. durante %s s. solo puede haber una picadura activa por cazador sobre un mismo objetivo"] = { { Stats.Strength, Stats.Agility, }, false, reduction = true } -- s3043
L["aumenta la velocidad de ataque a distancia un %s% y la velocidad de ataque cuerpo a cuerpo un %s% durante %s s"] = { { Stats.RangedHaste, }, { Stats.MeleeHaste, }, false, } -- s3045
L["aumenta la velocidad de ataque a distancia un %s%  durante %s s"] = { { Stats.RangedHaste, }, false, } -- s3045
L["reduce %s p. todos los atributos de un enemigo durante %s min"] = { { Stats.AllStats, }, false, reduction = true } -- s3105
L["aumenta %s p. el daño de un arma roma durante %s minutos"] = { { Stats.AverageWeaponDamage, }, false, } -- s3112
L["inflige daño normal más %s p. a un enemigo, y aumenta un %s% el tiempo entre sus ataques durante %s s"] = { false, { Stats.MeleeHaste, }, false, reduction = true } -- s3130
L["aumenta la velocidad de ataque de los aliados cercanos un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, } -- s3136
L["reduce la fuerza de los enemigos cercanos %s p. y su agilidad %s p. durante %s s"] = { { Stats.Strength, }, { Stats.Agility, }, false, reduction = true } -- s3146
L["reduce %s p. el aguante de un enemigo durante %s s"] = { { Stats.Stamina, }, false, reduction = true } -- s3148
L["aumenta %s p. la fuerza de los miembros del grupo cercanos durante %s s"] = { { Stats.Strength, }, false, } -- s3149
L["reduce un %s% el índice de regeneración de salud de un enemigo, y reduce %s p. su fuerza y %s p. su agilidad durante %s min"] = { false, { Stats.Strength, }, { Stats.Agility, }, false, reduction = true } -- s3150
L["aumenta la velocidad de ataque del taumaturgo un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, } -- s3151
L["regenera %s p. de salud cada %s s durante %s h"] = { { Stats.HealthRegen, }, false, false, } -- s3219
L["aumenta la velocidad de ataque de un aliado un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, } -- s3229
L["encanta de forma permanente una capa que aumenta %s p. de agilidad"] = { { Stats.Agility, }, } -- s3231
L["reduce %s p. el espíritu de un enemigo durante %s min"] = { { Stats.Spirit, }, false, reduction = true } -- s3237
L["aumenta la probabilidad de parar del taumaturgo un %s% durante %s s"] = { { Stats.Parry, }, false, } -- s3238
L["aumenta la probabilidad de bloqueo del taumaturgo un %s% durante %s s"] = { { Stats.BlockChance, }, false, } -- s3248
L["reduce un %s% la armadura de un enemigo durante %s s"] = { { Stats.Armor, }, false, reduction = true } -- s3252
L["reduce %s p. la fuerza, %s p. la agilidad y %s p. el intelecto de todos los enemigos en el área seleccionada durante %s min"] = { { Stats.Strength, }, { Stats.Agility, }, { Stats.Intellect, }, false, reduction = true } -- s3256
L["aturde al enemigo y reduce su armadura %s p. durante %s s"] = { { Stats.Armor, }, false, reduction = true } -- s3263
L["aumenta la velocidad de ataque del taumaturgo un %s% durante %s s. en cambio, el taumaturgo estará cansado por el esfuerzo extra"] = { { Stats.MeleeHaste, }, false, } -- s3269
L["aumenta el tiempo entre los ataques de los enemigos cercanos un %s% durante %s min"] = { { Stats.MeleeHaste, }, false, reduction = true } -- s3335
L["quema a un enemigo, inflige %s p. de daño y reduce su resistencia al fuego %s p. durante %s s"] = { false, { Stats.FireResistance, }, false, reduction = true } -- s3356
L["aumenta la fuerza del taumaturgo %s p. durante %s min"] = { { Stats.Strength, }, false, } -- s3369
L["aumenta %s p. el daño físico que inflige el taumaturgo y acelera su velocidad de movimiento un %s% durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, } -- s3385
L["reduce %s p. la armadura de un enemigo e inflige %s p. de daño de naturaleza cada %s s durante %s s"] = { { Stats.Armor, }, false, false, false, reduction = true } -- s3396
L["llena de ira al taumaturgo durante %s s, aumenta el daño físico que inflige %s p. pero reduciendo el tiempo entre sus ataques un %s%"] = { false, { Stats.AverageWeaponDamage, }, { Stats.MeleeHaste, }, } -- s3416
L["aumenta la probabilidad de bloquear del taumaturgo un %s% durante %s s"] = { { Stats.BlockChance, }, false, } -- s3419
L["enferma a un enemigo durante %s min, aumenta %s p. el daño físico que recibe y reduce %s p. su aguante. el objetivo enfermo tiene un %s% de probabilidad de propagar su enfermedad a uno de sus aliados cercanos cuando lo golpean en combate"] = { false, false, { Stats.Stamina, }, false, reduction = true } -- s3436
L["controla a un enemigo humanoide hasta el nivel %s durante %s s pero aumenta el tiempo entre sus ataques un %s%"] = { false, false, { Stats.MeleeHaste, }, reduction = true } -- s3442
L["aumenta la velocidad de ataque del taumaturgo un %s% durante %s s. pero reduce el daño físico causado %s p"] = { { Stats.MeleeHaste, }, false, { Stats.AverageWeaponDamage, }, } -- s3490
L["sume al taumaturgo en un estado de frenesí enajenado durante %s s. aumenta la velocidad de ataque del taumaturgo un %s% pero ralentiza su movimiento un %s% y reduce el daño físico que inflige %s p"] = { false, { Stats.MeleeHaste, }, false, { Stats.AverageWeaponDamage, }, } -- s3547
L["reduce la velocidad de lanzamiento de un enemigo un %s% durante %s s"] = { { Stats.SpellHaste, }, false, reduction = true } -- s3603
L["lanza barro a los ojos del enemigo reduciendo su probabilidad de golpear un %s% durante %s s"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, reduction = true } -- s3650
L["añade una mira permanente a un arco o arma de fuego que aumenta %s p. el daño que inflige"] = { { Stats.AverageWeaponDamage, }, } -- s3974
L["transforma al objetivo en un gnomo paria, reduce %s p. su daño cuerpo a cuerpo y con hechizos, y reduce un %s% su índice de movimiento durante %s s"] = { { Stats.SpellDamage, }, false, false, reduction = true } -- s4060
L["tiene un %s% de probabilidad de aumentar la armadura %s durante %s s al ser golpeado en combate"] = { false, { Stats.Armor, }, false, ignoreSum = true } -- s4070
L["aumenta tu resistencia a los hechizos de escarcha %s p"] = { { Stats.FrostResistance, }, } -- s4080
L["aumenta tu resistencia a los hechizos de naturaleza %s p"] = { { Stats.NatureResistance, }, } -- s4081
L["aumenta %s p. tu resistencia a los hechizos de las sombras"] = { { Stats.ShadowResistance, }, } -- s4084
L["aumenta la probabilidad de esquivar del taumaturgo un %s%. dura hasta que se cancela"] = { { Stats.Dodge, }, } -- s4086
L["enfada al objetivo, atrayendo su atención y disminuyendo el valor de su armadura %s p"] = { { Stats.Armor, }, reduction = true } -- s4101
L["aumenta %s p. la fuerza de los miembros del grupo cercanos y mejora %s p. su agilidad durante %s s"] = { { Stats.Strength, }, { Stats.Agility, }, false, } -- s4146
L["aumenta %s p. la fuerza y en %s p. la agilidad de los miembros del grupo situados en un radio de %s m durante %s s"] = { { Stats.Strength, }, { Stats.Agility, }, false, false, } -- s4147
L["da al usuario %s p. de salud extra durante %s min"] = { { Stats.Health, }, false, } -- s4148
L["aumenta el valor de la armadura en %s p. durante %s s"] = { { Stats.Armor, }, false, } -- s4149
L["el gato entra en un estado de ira violenta que aumenta su velocidad un %s%, a la vez que aumenta en %s el daño que recibe de los ataques durante %s s"] = { { Stats.MeleeHaste, }, false, false, } -- s4154
L["aumenta tu agilidad en %s p"] = { { Stats.Agility, }, } -- s4171
L["aguante aumentado en %s p"] = { { Stats.Stamina, }, } -- s4187
L["aumenta tu intelecto en %s p"] = { { Stats.Intellect, }, } -- s4204
L["aumenta tu espíritu en %s p"] = { { Stats.Spirit, }, } -- s4222
L["aumenta el máximo de p. de salud de los aliados de tu grupo en %s p. durante %s min"] = { { Stats.Health, }, false, } -- s4238
L["aumenta tu salud en %s p"] = { { Stats.Health, }, } -- s4247
L["aumenta tu maná en %s p"] = { { Stats.Mana, }, } -- s4263
L["aumenta tu maná %s p"] = { { Stats.Mana, }, } -- s4268
L["aumenta la probabilidad de parar con espadas de una mano en un %s%"] = { { Stats.Parry, }, } -- s4301
L["aumenta la probabilidad de parar con hachas de una mano en un %s%"] = { { Stats.Parry, }, } -- s4307
L["reduce la fuerza en %s p. y la agilidad en %s p. durante %s s"] = { { Stats.Strength, }, { Stats.Agility, }, false, reduction = true } -- s4316
L["aumenta tu agilidad %s p. durante %s min"] = { { Stats.Agility, }, false, } -- s4318
L["aumenta la probabilidad de parar con mazas de una mano en un %s%"] = { { Stats.Parry, }, } -- s4328
L["aumenta el daño con hachas de una mano en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4334
L["aumenta %s p. el daño con hachas de una mano"] = { { Stats.AverageWeaponDamage, }, } -- s4336
L["aumenta %s p el daño con hachas de una mano"] = { { Stats.AverageWeaponDamage, }, } -- s4342
L["aumenta el daño con espadas de una mano en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4350
L["aumenta el daño con espadas de una mano %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4352
L["aumenta el daño con mazas de una mano en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4366
L["aumenta tu velocidad de ataque con dagas en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4382
L["aumenta el daño con dagas en %s p. con una probabilidad del %s% de hacer sangrar al objetivo con un daño de %s p. durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, false, } -- s4387
L["aumenta la probabilidad de parar con hachas de dos manos en un %s%"] = { { Stats.Parry, }, } -- s4414
L["aumenta la probabilidad de conseguir un golpe crítico con espadas de dos manos en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s4418
L["aumenta la probabilidad de conseguir un golpe crítico con una espada de dos manos en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s4420
L["aumenta la probabilidad de conseguir un golpe crítico con espadas de dos manos un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s4421
L["aumenta la probabilidad de parar con mazas de dos manos en un %s%"] = { { Stats.Parry, }, } -- s4426
L["aumenta la probabilidad de conseguir un golpe crítico con todas las armas cuerpo a cuerpo en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s4433
L["aumenta el daño con hachas de dos manos en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4440
L["aumenta el daño con hachas de dos manos %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4443
L["aumenta el daño con espadas de dos manos en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4456
L["aumenta el daño con espadas de dos manos %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4464
L["aumenta el daño con mazas de dos manos en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4472
L["aumenta el daño con bastones en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s4488
L["aumenta el daño con bastones %s p. con una probabilidad del %s% de aturdir al objetivo durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, } -- s4493
L["aumenta el daño con bastones en %s p. con una probabilidad del %s% de aturdir al objetivo durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, } -- s4494
L["acelera los ataques un %s% y el movimiento un %s% durante %s s., pero inflige %s p. de daño de las sombras al taumaturgo cada %s s"] = { { Stats.MeleeHaste, }, false, false, false, false, } -- s4514
L["aumenta %s p. el daño infligido por el infernal pero reduce un %s% su velocidad de ataque durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, } -- s4539
L["aumenta la resistencia al fuego en %s p"] = { { Stats.FireResistance, }, } -- s4547
L["aumenta la resistencia a la naturaleza %s p"] = { { Stats.NatureResistance, }, } -- s4548
L["aumenta la resistencia a la escarcha en %s p"] = { { Stats.FrostResistance, }, } -- s4549
L["aumenta la resistencia a las sombras en %s p"] = { { Stats.ShadowResistance, }, } -- s4550
L["aumenta la resistencia al fuego en %s p. y da un %s% de probabilidad de rechazar hechizos de fuego hostiles devolviéndoselos al taumaturgo"] = { { Stats.FireResistance, }, false, } -- s4553
L["aumenta la resistencia a la naturaleza en %s p"] = { { Stats.NatureResistance, }, } -- s4558
L["aumenta la resistencia a la naturaleza en %s p. y da un %s% de probabilidad de rechazar hechizos de naturaleza hostiles devolviéndoselos al taumaturgo"] = { { Stats.NatureResistance, }, false, } -- s4560
L["aumenta la resistencia a la escarcha en %s p. y da un %s% de probabilidad de rechazar hechizos de escarcha hostiles devolviéndoselos al taumaturgo"] = { { Stats.FrostResistance, }, false, } -- s4567
L["aumenta la resistencia a las sombras en %s p. y da un %s% de probabilidad de rechazar hechizos de las sombras hostiles devolviéndoselos al taumaturgo"] = { { Stats.ShadowResistance, }, false, } -- s4574
L["aumenta la resistencia a todo tipo de hechizos en %s p"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- s4579
L["aumenta la probabilidad de bloquear con un escudo (no una rodela) en un %s%"] = { { Stats.BlockChance, }, } -- s4760
L["aumenta la probabilidad de bloquear con un escudo (no una rodela) en un %s% y devuelve un %s% de los hechizos hostiles a quien los lanzó"] = { { Stats.BlockChance, }, false, } -- s4762
L["aumenta el daño de los hechizos de fuego en %s p"] = { { Stats.FireDamage, }, } -- s4839
L["aumenta tu habilidad con magia sagrada en %s p. y reduce tu resistencia a las sombras en %s p"] = { false, { Stats.ShadowResistance, }, reduction = true } -- s4786
L["aumenta tu habilidad en magia de fuego en %s p. y reduce tu resistencia a la escarcha en %s p"] = { false, { Stats.FrostResistance, }, reduction = true } -- s4788
L["aumenta tu habilidad en magia de naturaleza en %s p. y reduce tu resistencia al fuego y a las sombras en %s p"] = { false, { Stats.ShadowResistance, Stats.FireResistance, }, reduction = true } -- s4792
L["aumenta tu magia de la escarcha en %s p. y reduce tu resistencia al fuego en %s p"] = { false, { Stats.FireResistance, }, reduction = true } -- s4796
L["aumenta el daño de los hechizos de naturaleza en %s p"] = { { Stats.NatureDamage, }, } -- s4848
L["aumenta el daño de los hechizos de escarcha en %s p"] = { { Stats.FrostDamage, }, } -- s4864
L["aumenta el daño con hechizos de escarcha %s p"] = { { Stats.FrostDamage, }, } -- s4870
L["aumenta el daño de los hechizos de las sombras en %s p"] = { { Stats.ShadowDamage, }, } -- s4880
L["aumenta la probabilidad de conseguir un golpe crítico con arcos en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s4915
L["aumenta el daño con armas de fuego en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s4921
L["aumenta la probabilidad de conseguir un golpe crítico con armas de fuego en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s4923
L["aumenta la armadura %s p. durante %s min"] = { { Stats.Armor, }, false, } -- s4941
L["da al taumaturgo %s p. extra de daño por golpe durante %s s pero reduce su velocidad de movimiento un %s% y aumenta el tiempo entre los ataques un %s%"] = { { Stats.AverageWeaponDamage, }, false, false, { Stats.MeleeHaste, }, } -- s4955
L["inmoviliza un enemigo y aumenta el tiempo entre sus ataques un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, reduction = true } -- s4962
L["reduce %s p. el aguante de un enemigo y %s p. su espíritu durante %s min"] = { { Stats.Stamina, }, { Stats.Spirit, }, false, reduction = true } -- s4974
L["restaura %s p. de salud durante %s s. debes permanecer sentado mientras comes. si comes durante al menos %s s, estarás bien alimentado y ganarás %s p. de aguante y espíritu durante %s min"] = { false, false, false, { Stats.Stamina, }, false, ignoreSum = true } -- s5004
L["una potente cerveza que aumenta tu fuerza %s p. y reduce tu intelecto %s p. durante %s min"] = { { Stats.Strength, }, { Stats.Intellect, }, false, } -- s5020
L["aumenta %s p. tu salud pero reduce %s p. tu espíritu durante %s min"] = { false, { Stats.Spirit, }, false, reduction = true } -- s5021
L["remate que aumenta la velocidad de ataque cuerpo a cuerpo un %s%. dura más por cada punto de combo:\r\n %s punto: %s s.\r\n %s puntos: %s s.\r\n %s puntos: %s s.\r\n %s puntos: %s s.\r\n %s puntos: %s s"] = { { Stats.MeleeHaste, }, false, false, false, false, false, false, false, false, false, false, } -- s5171
L["inflige %s p. de daño de fuego cada %s s a un enemigo, lo que aumenta un %s% el tiempo entre sus ataques y reduce un %s% su movimiento durante %s s"] = { false, false, { Stats.MeleeHaste, }, false, false, reduction = true } -- s5213
L["aumenta %s p. el daño infligido durante %s s"] = { { Stats.AverageWeaponDamage, }, false, } -- s5217
L["aumenta en %s p. la armadura de un objetivo amistoso y en %s p. todos los atributos durante %s min"] = { { Stats.Armor, }, { Stats.AllStats, }, false, } -- s5232
L["aumenta en %s p. la armadura de un objetivo amistoso, en %s p. todos los atributos y en %s p. todas las resistencias durante %s min"] = { { Stats.Armor, }, { Stats.AllStats, }, false, false, } -- s5234
L["el guerrero grita aumentando en %s p. el poder de ataque cuerpo a cuerpo de los miembros del grupo en un radio de %s m. dura %s min"] = { { Stats.AttackPower, }, false, false, } -- s5242
L["aumenta la probabilidad de parar un %s%"] = { { Stats.Parry, }, } -- s5256
L["aumenta %s p. el aguante"] = { { Stats.Stamina, }, } -- s5257
L["aumenta el tiempo entre los ataques de un enemigo un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, reduction = true } -- s5271
L["la probabilidad de esquivar del pícaro aumentará un %s% durante %s s"] = { { Stats.Dodge, }, false, } -- s5277
L["aumenta el aguante del objetivo %s p. durante %s min"] = { { Stats.Spirit, }, false, } -- s5321
L["aumenta un %s% el tiempo entre los ataques de un enemigo durante %s s"] = { { Stats.MeleeHaste, }, false, reduction = true } -- s5337
L["aumenta el daño con hachas de dos manos en %s p. con una probabilidad del %s% de reducir la armadura del enemigo en %s durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, false, } -- s5341
L["aumenta el daño con mazas de dos manos en %s p. con una probabilidad del %s% de aturdir al objetivo durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, } -- s5349
L["aumenta el daño con espadas de dos manos en %s p. y la probabilidad de parar utilizando una en un %s%"] = { { Stats.AverageWeaponDamage, }, false, } -- s5356
L["aumenta el daño con espadas de dos manos %s p. y la probabilidad de parar utilizando una en un %s%"] = { { Stats.AverageWeaponDamage, }, false, } -- s5361
L["aumenta la armadura %s p"] = { { Stats.Armor, }, } -- s5363
L["reduce %s p. la resistencia a la naturaleza de un enemigo durante %s min"] = { { Stats.NatureResistance, }, false, reduction = true } -- s5413
L["aumenta la probabilidad de esquivar del taumaturgo un %s% durante %s s"] = { { Stats.Dodge, }, false, } -- s5426
L["aumenta el daño con hachas de dos manos %s p. con una probabilidad del %s% de reducir la armadura del enemigo en %s durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, false, } -- s5427
L["aumenta el daño con hachas de una mano en %s p. con una probabilidad del %s% de reducir la armadura del enemigo en %s durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, false, } -- s5429
L["aumenta el daño con espadas de una mano en %s p. y la probabilidad de parar en un %s%"] = { { Stats.AverageWeaponDamage, }, { Stats.Parry, }, } -- s5449
L["aumenta la probabilidad de parar al empuñar espadas de dos manos en un %s%"] = { { Stats.Parry, }, } -- s5454
L["aumenta la probabilidad de conseguir un golpe crítico con hachas de dos manos en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5466
L["reduce la armadura un %s% durante %s s"] = { { Stats.Armor, }, false, reduction = true } -- s5480
L["aumenta la probabilidad de conseguir un golpe crítico con espadas de una mano en %s p"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5481
L["aumenta la probabilidad de conseguir un golpe crítico con espadas de una mano en %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5482
L["aumenta la probabilidad de conseguir un golpe crítico con hachas de una mano un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5509
L["aumenta la probabilidad de conseguir un golpe crítico con hachas de una mano en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5510
L["aumenta %s p. tu daño con hachas de una mano y otorga una probabilidad del %s% de reducir %s p. la armadura del enemigo durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, false, } -- s5513
L["reduce la probabilidad de un enemigo de golpear un %s% durante %s s"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, reduction = true } -- s5514
L["aumenta la probabilidad de conseguir un golpe crítico con mazas de dos manos en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5527
L["aumenta la probabilidad de conseguir un golpe crítico con mazas de una mano en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5545
L["aumenta el daño con mazas de una mano en %s p. con una probabilidad del %s% de aturdir al objetivo durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, } -- s5549
L["el objetivo es rodeado de insectos, lo que reduce su probabilidad de golpear un %s% e inflige %s p. de daño de naturaleza durante %s s"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, false, reduction = true } -- s5570
L["aumenta la probabilidad de conseguir un golpe crítico con dagas en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5585
L["aumenta la probabilidad de conseguir un golpe crítico con dagas un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5586
L["aumenta el daño con armas de fuego en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s5626
L["aumenta el daño con armas de fuego en %s p. con una probabilidad del %s% de aturdir al objetivo durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, } -- s5638
L["aumenta el daño físico %s p. durante %s min"] = { { Stats.AverageWeaponDamage, }, false, } -- s5665
L["restaura %s p. de salud cada %s s"] = { { Stats.HealthRegen, }, false, } -- s5707
L["aumenta la probabilidad de conseguir un golpe crítico con bastones en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5715
L["aumenta el daño con arcos en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s5742
L["aumenta el daño con arcos en %s p. con una probabilidad del %s% de hacer sangrar al objetivo con un daño de %s p. durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, false, } -- s5751
L["reduce %s p. la fuerza de un enemigo y %s p. su agilidad durante %s s"] = { { Stats.Strength, }, { Stats.Agility, }, false, reduction = true } -- s5781
L["aumenta tu armadura en %s p. cuando usas un escudo o rodela"] = { { Stats.Armor, }, } -- s5786
L["aumenta la probabilidad de conseguir un golpe crítico con magia de naturaleza en un %s%"] = { { Stats.SpellCrit, }, } -- s5812
L["aumenta la probabilidad de conseguir un golpe crítico con magia de fuego en %s%"] = { { Stats.SpellCrit, }, } -- s5835
L["aumenta la probabilidad de conseguir un golpe crítico con magia de fuego en un %s%"] = { { Stats.SpellCrit, }, } -- s5840
L["imbuye al objetivo de poder sagrado y aumenta su aguante %s p. durante %s min"] = { { Stats.Stamina, }, false, } -- s5862
L["aumenta la probabilidad de conseguir un golpe crítico con magia de la escarcha en un %s%"] = { { Stats.SpellCrit, }, } -- s5866
L["reduce un %s% la probabilidad de golpe de un enemigo durante %s s"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, reduction = true } -- s5884
L["aumenta la probabilidad de conseguir un golpe crítico con magia de las sombras en un %s%"] = { { Stats.SpellCrit, }, } -- s5896
L["esta cerveza suave aumenta tu espíritu %s p., pero reduce tu aguante %s p. durante %s min"] = { { Stats.Spirit, }, { Stats.Stamina, }, false, } -- s5909
L["reduce la probabilidad de golpear del objetivo un %s% durante %s s"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, reduction = true } -- s5917
L["aumenta un %s% la probabilidad de efecto crítico de tus hechizos sagrados"] = { { Stats.SpellCrit, }, } -- s5923
L["aumenta un %s% extra tu probabilidad de parar por cada punto de combo. dura %s s"] = { { Stats.Parry, }, false, } -- s5934
L["aumenta %s p el daño con armas arrojadizas"] = { { Stats.AverageWeaponDamage, }, } -- s5952
L["aumenta el daño con armas arrojadizas en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s5953
L["aumenta el daño con ballestas en %s p"] = { { Stats.AverageWeaponDamage, }, } -- s5969
L["aumenta la probabilidad de conseguir un golpe crítico con ballestas en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s5970
L["aumenta el daño con ballestas en %s p. con una probabilidad del %s% de hacer sangrar al objetivo con un daño de %s p. durante %s s"] = { { Stats.AverageWeaponDamage, }, false, false, false, } -- s5976
L["aumenta el daño con varitas en %s p"] = { { Stats.SpellDamage, }, } -- s6089
L["aumenta el daño con varitas en un %s%"] = { { Stats.SpellDamage, }, } -- s6091
L["aumenta la probabilidad de conseguir un golpe crítico con varitas en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s6094
L["aumenta tu intelecto %s p., pero reduce tu aguante %s p. durante %s min"] = { { Stats.Intellect, }, { Stats.Stamina, }, false, } -- s6114
L["aumenta %s p. tu resistencia a toda la magia y permite que un %s% de tu regeneración de maná persista mientras lanzas hechizos. un mago solo puede tener un tipo de hechizo de armadura activo a la vez. dura %s min"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, false, } -- s6117
L["reduce el poder de ataque cuerpo a cuerpo de todos los enemigos que estén a %s m. en %s p. durante %s s"] = { false, { Stats.AttackPower, }, false, reduction = true } -- s6190
L["aumenta la velocidad de movimiento del taumaturgo un %s% durante %s s. e inflige %s p. de daño extra en su primer ataque"] = { false, false, { Stats.AverageWeaponDamage, }, } -- s6268
L["mejora la salud de la mascota %s p"] = { { Stats.Health, }, } -- s6280
L["aumenta %s p. el aguante de los miembros del grupo"] = { { Stats.Stamina, }, } -- s6307
L["aumenta el daño infligido por la mascota %s p"] = { { Stats.AverageWeaponDamage, }, } -- s6311
L["aumenta el espíritu de la mascota %s p. se debe adiestrar mediante la doma de bestias"] = { { Stats.Spirit, }, } -- s6328
L["golpea a los enemigos cercanos, aumenta el tiempo entre sus ataques un %s% durante %s s y les inflige %s p. de daño. puede afectar a un máximo de %s objetivos"] = { { Stats.MeleeHaste, }, false, false, false, reduction = true } -- s6343
L["aumenta %s p. la resistencia de las mascotas a todos los tipos de magia"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- s6443
L["encanta de forma permanente una capa que aumenta %s p. de resistencia a las sombras"] = { { Stats.ShadowResistance, }, } -- s6476
L["aumenta %s p. el daño físico que infligen los miembros de grupo cercanos durante %s min"] = { { Stats.AverageWeaponDamage, }, false, } -- s6507
L["un filo cortante aumenta %s p. el daño infligido con esta arma"] = { { Stats.AverageWeaponDamage, }, } -- s6514
L["lanza tierra a los ojos del enemigo reduciendo su probabilidad de golpear un %s% durante %s s"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, reduction = true } -- s6530
L["el guerrero grita y aumenta %s p. el poder de ataque cuerpo a cuerpo de todos los miembros del grupo en un radio de %s m. dura %s min"] = { { Stats.AttackPower, }, false, false, } -- s6673
L["dispara contra un enemigo, le inflige daño normal y reduce %s p. su armadura durante %s s"] = { { Stats.Armor, }, false, reduction = true } -- s6685
L["remate que aumenta la velocidad de ataque cuerpo a cuerpo en un %s%. se prolonga con cada punto de combo:\r\n %s punto: %s s\r\n %s puntos: %s s\r\n %s puntos: %s s\r\n %s puntos: %s s\r\n %s puntos: %s s"] = { { Stats.MeleeHaste, }, false, false, false, false, false, false, false, false, false, false, } -- s6774
L["aumenta el daño causado en %s p. durante %s s"] = { { Stats.AverageWeaponDamage, }, false, } -- s6793
L["inflige %s p. de daño de naturaleza a un enemigo cada %s s, aumentando el tiempo entre sus ataques un %s% y ralentizando su movimiento un %s% durante %s s"] = { false, false, { Stats.MeleeHaste, }, false, false, reduction = true } -- s6814
L["reduce %s p. el intelecto de los enemigos cercanos durante %s s"] = { { Stats.Intellect, }, false, reduction = true } -- s6818
L["aumenta %s p. el daño físico causado por el taumaturgo pero reduce su velocidad de movimiento un %s%. dura %s s"] = { { Stats.AverageWeaponDamage, }, false, false, } -- s6864
L["aumenta el tiempo entre los ataques de un enemigo un %s% y su velocidad de movimiento un %s% durante %s min"] = { { Stats.MeleeHaste, }, false, false, reduction = true } -- s6907
L["aumenta %s p. el daño físico infligido por el taumaturgo y acelera su velocidad de movimiento un %s% durante %s s. además, echa una maldición al enemigo la primera vez que lo alcanza lo que reduce en %s p. el daño físico que cause, en %s p. el daño mágico que inflija y aumenta en %s p. el daño físico que sufra durante %s min"] = { { Stats.AverageWeaponDamage, }, false, false, { Stats.AverageWeaponDamage, }, { Stats.SpellDamage, }, false, false, } -- s6921
L["reduce %s p. la armadura de un enemigo durante %s min. mientras esté afectado, el objetivo no puede utilizar sigilo ni invisibilidad"] = { { Stats.Armor, }, false, reduction = true } -- s6950
L["reduce %s p. la fuerza de un enemigo durante %s min"] = { { Stats.Strength, }, false, reduction = true } -- s6951
L["aumenta la fuerza de un aliado %s p. durante %s min"] = { { Stats.Strength, }, false, } -- s6957
L["aumenta %s p. la armadura del taumaturgo durante %s s"] = { { Stats.Armor, }, false, } -- s7020
L["aumenta %s p. el daño físico causado por los miembros del grupo cercanos. los jugadores solo pueden tener un aura activa por paladín a la vez. el aura dura hasta que se cancela"] = { { Stats.AverageWeaponDamage, }, } -- s7069
L["aumenta el tiempo entre los ataques de los enemigos cercanos un %s% y su velocidad de lanzamiento un %s%. además proporciona un %s% de probabilidad de contagiar a un aliado al ser golpeado. dura %s min"] = { { Stats.MeleeHaste, }, { Stats.SpellHaste, }, false, false, reduction = true } -- s7102
L["aumenta el tiempo entre los ataques de un enemigo un %s%, reduce su velocidad de lanzamiento de hechizos un %s% y reduce su movimiento un %s%. dura %s min"] = { { Stats.MeleeHaste, }, { Stats.SpellHaste, }, false, false, reduction = true } -- s7127
L["carga contra un enemigo, aumenta la velocidad e inflige %s p. de daño extra en el primer ataque"] = { { Stats.AverageWeaponDamage, }, } -- s7137
L["conecta un contrapeso a una espada, maza, hacha o arma de asta de dos manos y la vuelve un %s% más rápida"] = { { Stats.MeleeHaste, }, } -- s7218
L["reduce la probabilidad de golpear de los enemigos cercanos un %s% durante %s s"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, reduction = true } -- s7272
L["aumenta el tiempo entre los ataques de los enemigos cercanos un %s% y reduce su velocidad de movimiento un %s% durante %s min"] = { { Stats.MeleeHaste, }, false, false, reduction = true } -- s7279
L["reduce %s p. la fuerza de un enemigo y %s p. su aguante durante %s min"] = { { Stats.Strength, }, { Stats.Stamina, }, false, reduction = true } -- s7289
L["aumenta %s p. la armadura y %s p. la resistencia a la escarcha. si un enemigo golpea al taumaturgo, su movimiento puede verse ralentizado un %s% y el tiempo entre sus ataques puede aumentar un %s% durante %s s. un mago solo puede tener un tipo de hechizo de armadura activo a la vez. dura %s min"] = { { Stats.Armor, }, { Stats.FrostResistance, }, false, { Stats.MeleeHaste, }, false, false, } -- s7302
L["aumenta tu espíritu %s p"] = { { Stats.Spirit, }, } -- s7353
L["inflige %s p. de daño de naturaleza a un enemigo cada %s s, aumenta el tiempo entre sus ataques un %s% y ralentiza su movimiento un %s% durante %s s"] = { false, false, { Stats.MeleeHaste, }, false, false, reduction = true } -- s7357
L["aumenta el espíritu de los miembros del grupo cercanos %s p"] = { { Stats.Spirit, }, } -- s7363
L["carga contra un enemigo, inmovilizándolo durante %s s, y añade %s p. de poder de ataque cuerpo a cuerpo al siguiente ataque del jabalí"] = { false, { Stats.AttackPower, }, } -- s7371
L["aumenta un %s% la probabilidad de golpe crítico.\r\naumenta un %s% todo el daño recibido.\r\nreduce un %s% la amenaza generada"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, false, false, } -- s7381
L["desgarra la armadura del objetivo, reduce %s p. por cada hender armadura e inflige una gran cantidad de amenaza. se puede acumular hasta %s veces. dura %s s"] = { { Stats.Armor, }, false, false, reduction = true } -- s7386
L["hiende el armadura del objetivo reduciendo %s p. por hender armadura y causando una gran cantidad de amenaza. se puede acumular hasta %s veces. dura %s s"] = { { Stats.Armor, }, false, false, reduction = true } -- s7405
L["encanta de forma permanente unos brazales para que aumenten la salud del portador en %s p"] = { { Stats.Health, }, } -- s7418
L["encanta de forma permanente una pieza de armadura de torso para que aumente la salud del portador en %s p"] = { { Stats.Health, }, } -- s7420
L["encanta de forma permanente unos brazales para que aumente %s p. la habilidad de defensa del portador"] = { { Stats.Defense, }, } -- s7428
L["encanta una pieza de armadura de torso de forma permanente para aumentar %s p. el maná del portador"] = { { Stats.Mana, }, } -- s7443
L["imbuye una pieza de armadura de torso con propiedades regeneradoras que aumentan %s p. el espíritu de quien la lleva durante %s minutos"] = { { Stats.Spirit, }, false, } -- s7451
L["encanta de forma permanente una capa para que aumente la resistencia a todos los tipos de magia en %s p"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- s7454
L["encanta de forma permanente unos brazales para que aumenten en %s p. el aguante del portador"] = { { Stats.Stamina, }, } -- s7457
L["%s p. de fuerza"] = { { Stats.Strength, }, } -- s7464
L["%s p. de intelecto"] = { { Stats.Intellect, }, } -- s7468
L["%s p. de agilidad"] = { { Stats.Agility, }, } -- s7471
L["%s p. de espíritu"] = { { Stats.Spirit, }, } -- s7474
L["%s p. de aguante"] = { { Stats.Stamina, }, } -- s7477
L["aumenta %s p. el índice de defensa"] = { { Stats.Defense, }, } -- s7511
L["aumenta el índice de defensa %s p"] = { { Stats.Defense, }, } -- s7516
L["espadas aumentadas %s p"] = { { Stats.WeaponSkill, }, } -- s7520
L["espadas de dos manos aumentadas %s p"] = { { Stats.WeaponSkill, }, } -- s7529
L["mazas aumentadas %s p"] = { { Stats.WeaponSkill, }, } -- s7532
L["mazas de dos manos aumentadas %s p"] = { { Stats.WeaponSkill, }, } -- s7533
L["dagas aumentadas %s p"] = { { Stats.WeaponSkill, }, } -- s7534
L["arcos aumentados %s p"] = { { Stats.WeaponSkill, }, } -- s7535
L["armas de fuego aumentadas %s p"] = { { Stats.WeaponSkill, }, } -- s7536
L["hachas aumentadas %s p"] = { { Stats.WeaponSkill, }, } -- s7538
L["hachas de dos manos aumentadas %s p"] = { { Stats.WeaponSkill, }, } -- s7549
L["mejora un %s% tu probabilidad de conseguir un golpe crítico"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s7597
L["aumenta hasta %s p. la sanación de los hechizos y efectos"] = { { Stats.HealingPower, }, } -- s7675
L["aumenta hasta %s p. el daño que infligen los hechizos y efectos de fuego"] = { { Stats.FireDamage, }, } -- s7683
L["aumenta hasta %s p. el daño que infligen los hechizos y efectos de naturaleza"] = { { Stats.NatureDamage, }, } -- s7690
L["aumenta hasta %s p. el daño que infligen los hechizos y efectos de escarcha"] = { { Stats.FrostDamage, }, } -- s7697
L["aumenta hasta %s p. el daño que infligen los hechizos y efectos de las sombras"] = { { Stats.ShadowDamage, }, } -- s7704
L["encanta de forma permanente un arma de combate cuerpo a cuerpo de dos manos para que inflija %s p. de daño extra"] = { { Stats.AverageWeaponDamage, }, } -- s7745
L["aumenta el intelecto %s p"] = { { Stats.Intellect, }, } -- s7764
L["encanta unos brazales de forma permanente para aumentar %s p. el espíritu del portador"] = { { Stats.Spirit, }, } -- s7766
L["imbuye brazales con sabiduría que aumenta %s p. el intelecto del portador durante %s minutos"] = { { Stats.Intellect, }, false, } -- s7769
L["encanta una capa para obtener %s p. extra de armadura"] = { { Stats.Armor, }, } -- s7771
L["encanta de forma permanente unos brazales para que aumenten en %s p. la agilidad del portador"] = { { Stats.Agility, }, } -- s7779
L["encanta unos brazales de forma permanente para aumentar %s p. la fuerza del portador"] = { { Stats.Strength, }, } -- s7782
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para que inflija %s p. de daño extra"] = { { Stats.AverageWeaponDamage, }, } -- s7788
L["encanta un arma de combate cuerpo a cuerpo de dos manos de forma permanente para añadir %s p. de intelecto"] = { { Stats.Intellect, }, } -- s7793
L["aumenta el aguante de los miembros del grupo en %s p"] = { { Stats.Stamina, }, } -- s7804
L["aumenta el daño de los hechizos de fuego en %s p. durante %s s"] = { { Stats.FireDamage, }, false, } -- s7843
L["aumenta %s p. el daño de fuego con hechizos durante %s min"] = { { Stats.FireDamage, }, false, } -- s7844
L["imbuye una pieza de armadura de torso con propiedades regeneradoras que aumentan el espíritu de quien la lleva en %s p. durante %s minutos"] = { { Stats.Spirit, }, false, } -- s7853
L["encanta un brazal de forma permanente para que aumente %s p. el espíritu del portador"] = { { Stats.Spirit, }, } -- s7859
L["encanta de forma permanente una capa para que aumente la resistencia al fuego en %s p"] = { { Stats.FireResistance, }, } -- s7861
L["encanta de forma permanente un par de botas para que aumenten %s p. el aguante del portador"] = { { Stats.Stamina, }, } -- s7863
L["imbuye una capa para otorgarle %s p. adicionales de armadura durante %s minutos"] = { { Stats.Armor, }, false, } -- s7865
L["encanta de forma permanente un par de botas para que aumenten %s p. la agilidad del portador"] = { { Stats.Agility, }, } -- s7867
L["reduce %s p. la agilidad de un enemigo durante %s min"] = { { Stats.Agility, }, false, reduction = true } -- s7901
L["cambio a forma de serpiente durante %s min, aumentando el daño de ataque %s p. y el tiempo entre los ataques un %s%"] = { false, { Stats.AverageWeaponDamage, }, { Stats.MeleeHaste, }, } -- s7965
L["aumenta un %s% el tiempo entre los ataques de un enemigo y ralentiza un %s% su velocidad de movimiento durante %s s"] = { { Stats.MeleeHaste, }, false, false, reduction = true } -- s7992
L["reduce la fuerza de un enemigo %s p., la agilidad %s p. y el aguante en %s durante %s min"] = { { Stats.Strength, }, { Stats.Agility, }, { Stats.Stamina, }, false, reduction = true } -- s7997
L["imbuye el arma del chamán, aumenta %s p. su poder de ataque cuerpo a cuerpo y permite que los ataques cuerpo a cuerpo inflijan una amenaza extra cuando se usa esa arma. dura %s minutos"] = { { Stats.AttackPower, }, false, } -- s8017
L["cambio a forma de serpiente durante %s s. aumentando el daño físico %s p., pero sacrificando la facultad de lanzar hechizos"] = { false, { Stats.AverageWeaponDamage, }, } -- s8041
L["aumenta el espíritu %s p. durante %s min"] = { { Stats.Spirit, }, false, } -- s8068
L["inflige %s p. de daño de naturaleza a los enemigos cercanos, aumenta el tiempo entre sus ataques un %s% y reduce su velocidad de movimiento un %s% durante %s s"] = { false, { Stats.MeleeHaste, }, false, false, reduction = true } -- s8078
L["aumenta %s p. la armadura del objetivo durante %s min"] = { { Stats.Armor, }, false, } -- s8091
L["aumenta %s p. el aguante del objetivo durante %s min"] = { { Stats.Stamina, }, false, } -- s8099
L["aumenta %s p. el espíritu del objetivo durante %s min"] = { { Stats.Spirit, }, false, } -- s8112
L["aumenta %s p. la agilidad del objetivo aliado durante %s min"] = { { Stats.Agility, }, false, } -- s8115
L["aumenta %s p. la fuerza del objetivo durante %s min"] = { { Stats.Strength, }, false, } -- s8118
L["reduce %s p. el aguante de un enemigo durante %s min"] = { { Stats.Stamina, }, false, reduction = true } -- s8138
L["reduce el intelecto de un enemigo %s p. y su espíritu %s p. durante %s min"] = { { Stats.Intellect, }, { Stats.Spirit, }, false, reduction = true } -- s8139
L["una ráfaga de energía colma al taumaturgo, lo que aumenta su daño %s p. y su armadura %s p. durante %s s"] = { { Stats.AverageWeaponDamage, }, { Stats.Armor, }, false, } -- s8191
L["golpea a los enemigos cercanos con truenos, aumenta el tiempo entre sus ataques un %s% durante %s s y les inflige %s p. de daño. puede afectar a un máximo de %s objetivos"] = { { Stats.MeleeHaste, }, false, false, false, reduction = true } -- s8198
L["tu tamaño aumenta y tu fuerza sube %s p. para corresponderse con tu tamaño nuevo. dura %s min"] = { { Stats.Strength, }, false, } -- s8212
L["reduce la armadura de los enemigos cercanos %s p. durante %s min"] = { { Stats.Armor, }, false, reduction = true } -- s8245
L["inflige daño normal más %s p. a un enemigo y a sus aliados cercanos, afectando hasta a %s objetivos y aumentando el tiempo entre sus ataques un %s% durante %s s"] = { false, false, { Stats.MeleeHaste, }, false, reduction = true } -- s8255
L["otorga %s p. de armadura extra a los miembros del grupo cercanos durante %s min. los jugadores solo pueden tener un aura a la vez por cada paladín"] = { { Stats.Armor, }, false, } -- s8258
L["aumenta un %s% la velocidad de ataque del taumaturgo y %s p. el daño físico que inflige durante %s min"] = { { Stats.MeleeHaste, }, { Stats.AverageWeaponDamage, }, false, } -- s8269
L["el daño físico causado por el objetivo se reduce %s p. durante %s min"] = { { Stats.AverageWeaponDamage, }, false, reduction = true } -- s8277
L["aumenta %s p. el daño físico causado por un aliado durante %s min"] = { { Stats.AverageWeaponDamage, }, false, } -- s8365
L["daño infligido por el objetivo reducido %s p. durante %s min"] = { { Stats.AverageWeaponDamage, }, false, reduction = true } -- s8552
L["aumenta la velocidad de ataque de un aliado un %s% durante %s s, pero también le inflige %s p. de daño de naturaleza cada %s s"] = { { Stats.MeleeHaste, }, false, false, false, } -- s8699
L["debilita al objetivo reduciendo el daño que causa en %s p. y reduce la eficacia de cualquier sanación en un %s%. dura %s min"] = { { Stats.AverageWeaponDamage, }, false, false, reduction = true } -- s9035
L["aumenta %s p. el poder de ataque cuerpo a cuerpo de los miembros del grupo cercanos durante %s min"] = { { Stats.AttackPower, }, false, } -- s9128
L["mejora un %s% tu probabilidad de conseguir un golpe crítico con armas misiles"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s9132
L["aumenta %s p. el poder de ataque"] = { { Stats.GenericAttackPower, }, } -- s9136
L["reduce la armadura del objetivo %s p. durante %s min"] = { { Stats.Armor, }, false, reduction = true } -- s9176
L["aumenta hasta %s p. el daño y la sanación de los hechizos y efectos mágicos"] = { { Stats.SpellPower, }, } -- s9342
L["inmoviliza al taumaturgo y aumenta el daño físico que inflige %s p. durante %s s"] = { { Stats.AverageWeaponDamage, }, false, } -- s9576
L["%s p. de armadura"] = { { Stats.Armor, }, } -- s9760
L["aumenta %s p. la armadura y %s p. la resistencia a la escarcha. si un enemigo golpea al taumaturgo, su movimiento puede verse ralentizado un %s%, y el tiempo entre sus ataques puede aumentar un %s% durante %s s. un mago solo puede tener un tipo de hechizo de armadura activo a la vez. dura %s min"] = { { Stats.Armor, }, { Stats.FrostResistance, }, false, { Stats.MeleeHaste, }, false, false, } -- s10219
L["sacrifica la vida del taumaturgo para reducir el aguante de los enemigos cercanos %s p. durante %s min"] = { { Stats.Stamina, }, false, reduction = true } -- s10251
L["%s p. de defensa durante %s s"] = { { Stats.Defense, }, false, } -- s10351
L["aumenta %s p. el daño infligido al objetivo mediante ataques físicos durante %s min. se acumula hasta %s veces"] = { { Stats.AverageWeaponDamage, }, false, false, } -- s10370
L["aumenta el tiempo entre los ataques de la víctima %s p. durante %s s"] = { { Stats.MeleeHaste, }, false, reduction = true } -- s10371
L["maldice a un objetivo y reduce el daño que inflige %s p. durante %s min"] = { { Stats.AverageWeaponDamage, }, false, reduction = true } -- s10651
L["aumenta %s p. la fuerza cuando se consume. el efecto dura %s minutos"] = { { Stats.Strength, }, false, } -- s10667
L["aumenta %s p. el aguante cuando se consume. el efecto dura %s minutos"] = { { Stats.Stamina, }, false, } -- s10668
L["aumenta %s p. la agilidad cuando se consume. el efecto dura %s minutos"] = { { Stats.Agility, }, false, } -- s10669
L["aumenta %s p. la fuerza cuando se consume"] = { { Stats.Strength, }, } -- s10670
L["aumenta %s p. el aguante cuando se consume"] = { { Stats.Stamina, }, } -- s10671
L["aumenta %s p. la agilidad cuando se consume"] = { { Stats.Agility, }, } -- s10672
L["aumenta %s p. el intelecto cuando se consume"] = { { Stats.Intellect, }, } -- s10690
L["aumenta %s p. el espíritu cuando se consume"] = { { Stats.Spirit, }, } -- s10691
L["aumenta %s p. el intelecto cuando se consume. el efecto dura %s minutos"] = { { Stats.Intellect, }, false, } -- s10692
L["aumenta %s p. el espíritu cuando se consume. el efecto dura %s minutos"] = { { Stats.Spirit, }, false, } -- s10693
L["acelera el ataque de los aliados mecánicos cercanos un %s% y aumenta el daño físico que infligen %s p. durante %s s"] = { { Stats.MeleeHaste, }, { Stats.AverageWeaponDamage, }, false, } -- s10732
L["aumenta el espíritu %s p"] = { { Stats.Spirit, }, } -- s10767
L["aumenta el tiempo entre los ataques de un enemigo un %s% y su velocidad de movimiento un %s% durante %s s. el hechizo retraso también aturde al enemigo durante %s s"] = { { Stats.MeleeHaste, }, false, false, false, reduction = true } -- s10855
L["controla una mente humanoide de un nivel máximo de %s, pero aumenta el tiempo entre ataques en un %s%. dura un máximo de %s min"] = { false, { Stats.MeleeHaste, }, false, reduction = true } -- s10911
L["reduce %s p. el poder de ataque cuerpo a cuerpo de los enemigos cercanos durante %s s"] = { { Stats.AttackPower, }, false, reduction = true } -- s10968
L["reduce %s p. la resistencia de tu objetivo a todos tus hechizos y reduce un %s% la amenaza que generan tus hechizos arcanos"] = { { Stats.SpellPenetration, }, false, } -- s11210
L["aumenta %s p. la resistencia a las sombras. si un enemigo ataca a quien la bebe, el atacante tiene una probabilidad del %s% de quedar afectado por una enfermedad que aumenta %s p. el daño que recibe durante %s s. dura %s min"] = { { Stats.ShadowResistance, }, false, false, false, false, } -- s11371
L["aumenta %s p. el daño con hechizos durante %s min"] = { { Stats.SpellDamage, }, false, } -- s11390
L["inflige daño normal más %s p. a un enemigo, lo que reduce %s p. su fuerza y %s p. su agilidad durante %s min"] = { false, { Stats.Strength, }, { Stats.Agility, }, false, reduction = true } -- s11397
L["aumenta %s p. tu fuerza durante %s h"] = { { Stats.Strength, }, false, } -- s11405
L["aumenta un %s% el tiempo entre los ataques de un enemigo y un %s% su velocidad de movimiento. además, reduce su fuerza un %s%. dura %s s"] = { { Stats.MeleeHaste, }, false, false, false, reduction = true } -- s11443
L["aumenta %s p. el daño de las sombras con hechizos durante %s min"] = { { Stats.ShadowDamage, }, false, } -- s11474
L["golpea al enemigo con la ira de sul'thraze. reduce la fuerza %s p. e inflige %s p. de daño de las sombras con %s p. de daño extra durante %s s"] = { { Stats.Strength, }, false, false, false, reduction = true } -- s11658
L["maldice al objetivo durante %s min, disminuyendo su resistencia al fuego y a la escarcha en %s p. y aumentando el daño de fuego y de escarcha que recibe en un %s%. un brujo solo puede tener una maldición activa a la vez sobre un mismo objetivo"] = { false, { Stats.FireResistance, Stats.FrostResistance, }, false, reduction = true } -- s11721
L["reduce la fuerza de un enemigo %s p. y su aguante %s p. durante %s min"] = { { Stats.Strength, }, { Stats.Stamina, }, false, reduction = true } -- s11963
L["pincha la armadura del objetivo y la reduce %s p"] = { { Stats.Armor, }, reduction = true } -- s11791
L["reduce la fuerza de los enemigos cercanos %s p. y su aguante %s p. durante %s min"] = { { Stats.Strength, }, { Stats.Stamina, }, false, reduction = true } -- s11892
L["corta la armadura de un enemigo y la reduce %s p. por cada uso de hender armadura. se puede aplicar hasta %s veces. dura %s s"] = { { Stats.Armor, }, false, false, } -- s11971
L["reduce %s p. el daño físico que inflige un enemigo durante %s min. un brujo solo puede tener una maldición activa sobre cada objetivo"] = { { Stats.AverageWeaponDamage, }, false, reduction = true } -- s11980
L["inflige %s p. de daño de naturaleza a los enemigos cercanos y reduce su probabilidad de golpear un %s% durante %s s"] = { false, { Stats.MeleeHit, Stats.RangedHit, }, false, reduction = true } -- s11983
L["provoca que un enemigo fije su atención en el taumaturgo y aumenta la velocidad de ataque del taumaturgo un %s% durante %s s. mientras el objetivo fija su atención en el taumaturgo, se niega a atacar a nadie más"] = { { Stats.MeleeHaste, }, false, } -- s12021
L["otorga %s p. de aguante extra a los miembros del grupo en un radio de %s m"] = { { Stats.Stamina, }, false, } -- s12022
L["aumenta la probabilidad de conseguir un golpe crítico con armas de asta en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s12165
L["inutiliza a un enemigo, inflige daño normal más %s p. y reduce su fuerza %s p. durante %s s"] = { false, { Stats.Strength, }, false, reduction = true } -- s12166
L["aumenta la probabilidad de bloquear del taumaturgo un %s% durante %s s pero solo bloqueará %s ataque"] = { { Stats.BlockChance, }, false, false, } -- s12169
L["aumenta la probabilidad de bloquear del taumaturgo un %s% durante %s s pero solo bloqueará %s ataques"] = { { Stats.BlockChance, }, false, false, } -- s12169
L["reduce la velocidad de lanzamiento de los enemigos cercanos un %s% y aumenta el tiempo entre sus ataques un %s%. dura %s min"] = { { Stats.SpellHaste, }, { Stats.MeleeHaste, }, false, reduction = true } -- s12255
L["aumenta un %s% tu probabilidad de bloquear ataques con un escudo y tienes una probabilidad del %s% de generar %s p. de ira cuando bloqueas"] = { { Stats.BlockChance, }, false, false, } -- s12298
L["aumenta tu velocidad de ataque un %s% en tus %s próximos golpes después de asestar un golpe crítico cuerpo a cuerpo"] = { { Stats.MeleeHaste, }, false, ignoreSum = true } -- s12319
L["aumenta un %s% tu probabilidad de obtener un golpe crítico con armas cuerpo a cuerpo"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s12320
L["reduce todos los atributos de los enemigos cercanos %s p. durante %s min"] = { { Stats.AllStats, }, false, reduction = true } -- s12530
L["reviste al taumaturgo con una capa de escarcha que dura %s min. el hielo incrementa %s p. la armadura, ralentiza el movimiento de los atacantes cuerpo a cuerpo al %s% y aumenta el tiempo entre los ataques un %s%"] = { false, { Stats.Armor, }, { Stats.MeleeHaste, }, false, } -- s12544
L["aumenta %s p. el daño infligido y un %s% la velocidad de ataque durante %s s"] = { { Stats.AttackPower, }, { Stats.MeleeHaste, }, false, } -- s12686
L["aumenta un %s% tu probabilidad de conseguir un golpe crítico con hachas"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s12700
L["aumenta el daño %s p. durante %s s"] = { { Stats.AverageWeaponDamage, }, false, } -- s12731
L["aumenta tu velocidad normal de regeneración de salud y maná en %s p"] = { { Stats.GenericManaRegen, }, } -- s12732
L["aumenta %s p. la armadura, %s p. todas las resistencias y otorga inmunidad al miedo durante %s s"] = { { Stats.Armor, }, { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, } -- s12733
L["reduce %s p. el daño físico que infligen los enemigos cercanos durante %s min. un brujo solo puede tener una maldición activa sobre cada objetivo"] = { { Stats.AverageWeaponDamage, }, false, reduction = true } -- s12741
L["aumenta la probabilidad de conseguir un golpe crítico con armas de asta en %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s12831
L["aumenta la probabilidad de conseguir un golpe crítico con armas cuerpo a cuerpo en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s12853
L["enloquece temporalmente a un objetivo enemigo, aumentando su velocidad de ataque un %s% y su movimiento un %s%, y provocando que ataque a sus propios aliados durante %s s"] = { { Stats.MeleeHaste, }, false, false, } -- s12888
L["obliga a los enemigos próximos a hablar en demoníaco lo que reduce su velocidad para hechizar un %s% durante %s s. cada brujo solo puede tener una maldición activa sobre cada objetivo"] = { { Stats.SpellHaste, }, false, reduction = true } -- s12889
L["reduce %s p. la fuerza y %s p. la agilidad de los enemigos cercanos durante %s s, e impide que lancen hechizos"] = { { Stats.Strength, }, { Stats.Agility, }, false, reduction = true } -- s12946
L["aumenta %s p. la defensa, %s p. la resistencia a las sombras y %s p. tu regeneración de salud normal"] = { { Stats.Defense, Stats.HealthRegen, }, { Stats.ShadowResistance, }, false, } -- s12956
L["el cazador asume los aspectos de un mono aumentando en un %s% la probabilidad de esquivar. solo puede haber %s aspecto activo cada vez"] = { { Stats.Dodge, }, false, } -- s13163
L["el cazador asume los aspectos de un halcón y aumenta %s p. el poder de ataque a distancia. solo puede haber un aspecto activo a la vez"] = { { Stats.RangedAttackPower, }, } -- s13165
L["aumenta un %s% el daño físico causado por el pollo y acelera un %s% sus ataques durante %s s"] = { { Stats.AverageWeaponDamage, }, { Stats.MeleeHaste, }, false, } -- s13168
L["aumenta el intelecto de un aliado %s p. durante %s min"] = { { Stats.Intellect, }, false, } -- s13326
L["obliga a un enemigo a hablar en demoníaco y reduce su velocidad de lanzamiento un %s% durante %s s. cada brujo solo puede tener una maldición activa sobre cada objetivo"] = { { Stats.SpellHaste, }, false, reduction = true } -- s13338
L["encanta de forma permanente un escudo para que aumente %sp. el aguante del portador"] = { { Stats.Stamina, }, } -- s13378
L["encanta un arma de combate cuerpo a cuerpo de dos manos de forma permanente para añadir %s p. de espíritu"] = { { Stats.Spirit, }, } -- s13380
L["encanta forma permanente una capa de para que otorgue %s p. de agilidad"] = { { Stats.Agility, }, } -- s13419
L["encanta de forma permanente una capa para que aumente la armadura en %s p"] = { { Stats.Armor, }, } -- s13421
L["reduces la probabilidad de bloquear de un objetivo un %s% durante %s s"] = { { Stats.BlockChance, }, false, reduction = true } -- s13459
L["encanta de forma permanente un escudo para que aumente %s p. la armadura"] = { { Stats.Armor, }, } -- s13464
L["encanta de forma permanente un escudo para que proporcione %s p. de espíritu"] = { { Stats.Spirit, }, } -- s13485
L["reduce el poder de ataque de un objetivo %s p. durante %s s"] = { { Stats.GenericAttackPower, }, false, reduction = true } -- s13490
L["encanta de forma permanente un brazal para que aumente el aguante del portador en %s p"] = { { Stats.Stamina, }, } -- s13501
L["encanta de forma permanente una capa para que aumente %s p. la resistencia a las sombras"] = { { Stats.ShadowResistance, }, } -- s13522
L["ácido corrosivo que inflige %s p. de daño de naturaleza cada %s s y reduce la armadura del objetivo %s p. durante %s s"] = { false, false, { Stats.Armor, }, false, reduction = true } -- s13526
L["reduce la fuerza del objetivo %s p. durante %s s"] = { { Stats.Strength, }, false, reduction = true } -- s13528
L["golpea a los enemigos cercanos con truenos, lo que aumenta el tiempo entre sus ataques un %s% durante %s s y les inflige %s p. de daño de naturaleza. puede afectar a un máximo de %s objetivos"] = { { Stats.MeleeHaste, }, false, false, false, reduction = true } -- s13532
L["encanta un brazal de forma permanente para que aumente %s p. la fuerza del portador"] = { { Stats.Strength, }, } -- s13536
L["aumenta %s p. la velocidad de ataque de todos los miembros del grupo cercanos. los jugadores solo pueden tener un aura activa por paladín a la vez. el aura dura hasta que se cancela"] = { { Stats.MeleeHaste, }, } -- s13589
L["aumenta hasta %s p. el daño que infligen los hechizos y efectos arcanos"] = { { Stats.ArcaneDamage, }, } -- s13590
L["encanta de forma permanente una pieza de armadura de torso para que aumente el maná del portador en %s p"] = { { Stats.Mana, }, } -- s13607
L["encanta de forma permanente un brazal para que aumente el intelecto del portador en %s p"] = { { Stats.Intellect, }, } -- s13622
L["encanta de forma permanente una pieza de armadura de torso para que proporcione %s de todas las estadísticas"] = { { Stats.AllStats, }, } -- s13626
L["encanta de forma permanente un escudo para que proporcione %s p. de aguante"] = { { Stats.Stamina, }, } -- s13631
L["encanta de forma permanente una capa para que proporcione %s p. de armadura extra"] = { { Stats.Armor, }, } -- s13635
L["encanta de forma permanente unas botas para que proporcionen %s p. de agilidad"] = { { Stats.Agility, }, } -- s13637
L["encanta de forma permanente una pieza de armadura de torso para que proporcione %s de salud"] = { { Stats.Health, }, } -- s13640
L["encanta de forma permanente unos brazales para que proporcionen %s p. de espíritu"] = { { Stats.Spirit, }, } -- s13642
L["encanta de forma permanente unas botas para que proporcionen %s p. de aguante"] = { { Stats.Stamina, }, } -- s13644
L["encanta de forma permanente unos brazales para que otorguen %s p. de defensa"] = { { Stats.Defense, }, } -- s13646
L["encanta de forma permanente unos brazales para que proporcionen %s p. de aguante"] = { { Stats.Stamina, }, } -- s13648
L["encanta de forma permanente una capa para que proporcione %s p. de resistencia al fuego"] = { { Stats.FireResistance, }, } -- s13657
L["encanta de forma permanente unos brazales para que proporcionen %s p. de fuerza"] = { { Stats.Strength, }, } -- s13661
L["encanta de forma permanente una pieza de armadura de torso para que proporcione %s de maná"] = { { Stats.Mana, }, } -- s13663
L["aumenta un %s% tu probabilidad de parar un ataque"] = { { Stats.Parry, }, } -- s13665
L["aumenta un %s% tu probabilidad de esquivar un ataque"] = { { Stats.Dodge, }, } -- s13669
L["aumenta tu probabilidad de esquivar un ataque un %s%"] = { { Stats.Dodge, }, } -- s13671
L["aumenta un %s% tu probabilidad de bloquear ataques con un escudo"] = { { Stats.BlockChance, }, } -- s13674
L["aumenta la probabilidad de bloquear ataques con un escudo un %s%"] = { { Stats.BlockChance, }, } -- s13676
L["encanta de forma permanente unas botas para que proporcionen %s p. de espíritu"] = { { Stats.Spirit, }, } -- s13687
L["encanta de forma permanente un escudo para que proporcione una probabilidad de bloqueo de %s%"] = { { Stats.BlockChance, }, } -- s13689
L["aumenta un %s% tu probabilidad de golpear con armas cuerpo a cuerpo"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- s13705
L["aumenta la probabilidad de conseguir un golpe crítico con armas de puño en un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s13707
L["aumenta un %s% tu probabilidad de esquivar"] = { { Stats.Dodge, }, } -- s13712
L["aumenta tu probabilidad de parar un %s%"] = { { Stats.Parry, }, } -- s13713
L["aumenta tu probabilidad de parar en un %s%"] = { { Stats.Parry, }, } -- s13855
L["aumenta la resistencia al fuego %s p. y reduce todo el daño de fuego sufrido hasta %s p. durante %s s"] = { { Stats.FireResistance, }, false, false, } -- s13744
L["aumenta un %s% el tiempo entre los ataques de los enemigos cercanos y ralentiza un %s% su velocidad de movimiento durante %s s"] = { { Stats.MeleeHaste, }, false, false, reduction = true } -- s13747
L["protege al taumaturgo, lo que aumenta %s p. la armadura, %s p. la resistencia a las sombras y la regeneración de salud durante %s min"] = { { Stats.Armor, }, { Stats.ShadowResistance, }, false, } -- s13787
L["encanta de forma permanente una capa para que proporcione %s p. de resistencia a todo"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- s13794
L["encanta de forma permanente unos guantes para que proporcionen %s p. de agilidad"] = { { Stats.Agility, }, } -- s13815
L["encanta de forma permanente unos brazales para que proporcionen %s p. de intelecto"] = { { Stats.Intellect, }, } -- s13822
L["encanta de forma permanente unos brazales para que otorguen %s de espíritu"] = { { Stats.Spirit, }, } -- s13846
L["aumenta el aguante de un aliado %s p. durante %s min"] = { { Stats.Stamina, }, false, } -- s13864
L["protege al taumaturgo de todos los ataques y hechizos durante %s s, pero también aumenta el tiempo entre los ataques un %s% durante ese tiempo"] = { false, { Stats.MeleeHaste, }, reduction = true } -- s13874
L["aumenta un %s% tu velocidad de ataque. además, los ataques golpean a un enemigo cercano adicional. dura %s s"] = { { Stats.MeleeHaste, }, false, } -- s13877
L["encanta una capa de forma permanente que otorga %s p. de agilidad"] = { { Stats.Agility, }, } -- s13882
L["encanta de forma permanente unos guantes para que proporcionen %s p. de fuerza"] = { { Stats.Strength, }, } -- s13887
L["encanta de forma permanente un escudo para que proporcione %s p. de resistencia a la escarcha"] = { { Stats.FrostResistance, }, } -- s13933
L["encanta de forma permanente un arma de combate cuerpo a cuerpo de dos manos para que inflija %s p. de daño"] = { { Stats.AverageWeaponDamage, }, } -- s13937
L["encanta de forma permanente unos brazales para que otorguen %s p. de aguante"] = { { Stats.Stamina, }, } -- s13945
L["encanta unos guantes de forma permanente para otorgar un bonus de %s% de velocidad de ataque"] = { { Stats.MeleeHaste, Stats.RangedHaste, }, } -- s13948
L["corroe la armadura de un enemigo, reduciéndola %s p. en cada aplicación. se puede aplicar hasta un máximo de %s veces. dura %s s"] = { { Stats.Armor, }, false, false, } -- s14120
L["aumenta la probabilidad de conseguir un golpe crítico un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s14138
L["hace que tus ataques ignoren %s p. de la armadura de tu objetivo y aumenta un %s% el daño que inflige tu facultad ruptura. el monto de armadura que se reduce aumenta con el nivel"] = { { Stats.ArmorPenetration, }, false, } -- s14171
L["un golpe que causa un %s% de daño de arma y aumenta en un %s% la probabilidad de esquivar durante %s s. otorga %s p. de combo"] = { false, { Stats.Dodge, }, false, false, } -- s14278
L["reduce la velocidad de lanzamiento de los enemigos cercanos un %s% durante %s min"] = { { Stats.SpellHaste, }, false, reduction = true } -- s14538
L["%s p. de resistencia al fuego"] = { { Stats.FireResistance, }, } -- s14540
L["%s p. de resistencia a la escarcha"] = { { Stats.FrostResistance, }, } -- s14541
L["%s p. de resistencia a la naturaleza"] = { { Stats.NatureResistance, }, } -- s14622
L["%s p. de resistencia a las sombras"] = { { Stats.ShadowResistance, }, } -- s14663
L["%s p. resistencia a lo arcano"] = { { Stats.ArcaneResistance, }, } -- s14664
L["aumenta %s p. el daño que inflige el taumaturgo durante %s min"] = { { Stats.AverageWeaponDamage, Stats.FireDamage, }, false, } -- s14744
L["imbuye al objetivo de poder sagrado y aumenta su espíritu en %s p. durante %s min"] = { { Stats.Spirit, }, false, } -- s14752
L["aumenta un %s% la velocidad de ataque del taumaturgo y aumenta el daño físico que inflige %s p. durante %s s"] = { { Stats.MeleeHaste, }, { Stats.AverageWeaponDamage, }, false, } -- s14872
L["aumenta el tiempo entre los ataques de un enemigo un %s% y ralentiza su velocidad de movimiento un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, false, reduction = true } -- s14897
L["aumenta la probabilidad de esquivar del pícaro un %s% durante %s s"] = { { Stats.Dodge, }, false, } -- s15087
L["aumenta tu velocidad de ataque en un %s% en tus %s próximos golpes después de asestar un golpe crítico"] = { { Stats.MeleeHaste, }, false, ignoreSum = true } -- s15088
L["aumenta la resistencia al fuego de un aliado %s p. durante %s h"] = { { Stats.FireResistance, }, false, } -- s15123
L["provoca que las unidades amigas que se encuentren en un radio de %s metro atacan un %s% más rápido"] = { false, { Stats.MeleeHaste, }, } -- s15167
L["reduce la armadura de un enemigo %s p. durante %s min"] = { { Stats.Armor, }, false, reduction = true } -- s15235
L["reduce la armadura de un objetivo %s p. durante %s s"] = { { Stats.Armor, }, false, reduction = true } -- s15280
L["aumenta %s p. el daño mágico infligido por un aliado durante %s min"] = { { Stats.SpellDamage, }, false, } -- s15288
L["añade de forma permanente %s p. de maná a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Mana, }, } -- s15340
L["aumenta un %s% la probabilidad de golpe crítico con un ataque cuerpo a cuerpo, a distancia o con hechizos, y aumenta %s p. todos los atributos durante %s h"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, { Stats.AllStats, }, false, } -- s15366
L["añade de forma permanente %s puntos de salud a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Health, }, } -- s15389
L["añade de forma permanente %s p. de armadura a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Armor, }, } -- s15391
L["añade de forma permanente %s p. de resistencia al fuego a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.FireResistance, }, } -- s15394
L["añade de forma permanente %s p. de fuerza a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Strength, }, } -- s15397
L["añade de forma permanente %s p. de aguante a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, } -- s15400
L["añade de forma permanente %s p. de agilidad a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Agility, }, } -- s15402
L["añade de forma permanente %s p. de intelecto a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Intellect, }, } -- s15404
L["añade de forma permanente %s p. de espíritu a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Spirit, }, } -- s15406
L["mejora tu probabilidad de golpear un %s%"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- s15464
L["obliga a los enemigos próximos a hablar en demoníaco y reduce su velocidad de lanzamiento un %s% durante %s s. cada brujo solo puede tener una maldición activa sobre cada objetivo"] = { { Stats.SpellHaste, }, false, reduction = true } -- s15470
L["inmoviliza un enemigo, aumenta el tiempo entre sus ataques un %s% y evita que pueda lanzar hechizos durante %s s"] = { { Stats.MeleeHaste, }, false, reduction = true } -- s15471
L["corta a los enemigos cercanos y la reduce %s p. por cada uso de hender armadura. se puede acumular hasta %s veces. dura %s s"] = { { Stats.Armor, }, false, false, } -- s15502
L["aumenta el poder de ataque %s p. durante %s s"] = { { Stats.GenericAttackPower, }, false, } -- s15602
L["reduce el poder de ataque cuerpo a cuerpo de los enemigos cercanos %s p. durante %s s"] = { { Stats.AttackPower, }, false, reduction = true } -- s15727
L["reviste a un aliado con una capa de escarcha que dura %s min. el hielo aumenta la armadura %s p., ralentiza el movimiento de los atacantes cuerpo a cuerpo al %s% y aumenta el tiempo entre los ataques un %s%"] = { false, { Stats.Armor, }, { Stats.MeleeHaste, }, false, } -- s15784
L["reduce todos los atributos de un enemigo %s p. durante %s min"] = { { Stats.AllStats, }, false, reduction = true } -- s15848
L["aumenta la resistencia a todos los tipos de magia %s p"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- s15967
L["reduce el aguante de un enemigo %s p. durante %s s"] = { { Stats.Stamina, }, false, reduction = true } -- s16172
L["aumenta un %s% tu probabilidad de golpear con hechizos y ataques cuerpo a cuerpo"] = { { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, } -- s16180
L["aumenta %s p. el poder de ataque de un enemigo pero reduce su armadura en %s durante %s min. los enemigos malditos no conocerán los efectos de miedo y no saldrán huyendo despavoridos en combate. cada brujo solo puede tener una maldición activa sobre cada objetivo"] = { { Stats.AttackPower, }, { Stats.Armor, }, false, } -- s16231
L["aumenta la probabilidad de bloquear ataques con un escudo en un %s% y aumenta la cantidad bloqueada en un %s%"] = { { Stats.BlockChance, }, false, } -- s16253
L["aumenta tu probabilidad de esquivar un %s% extra"] = { { Stats.Dodge, }, } -- s16254
L["mejora un %s% tu probabilidad de obtener un golpe crítico con tus ataques de arma"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s16255
L["aumenta tu velocidad de ataque un %s% en tus %s próximos golpes después de asestar un golpe crítico"] = { { Stats.MeleeHaste, }, false, ignoreSum = true } -- s16256
L["aumenta tu probabilidad de esquivar en un %s% extra"] = { { Stats.Dodge, }, } -- s16273
L["imbuye el arma del chamán, aumenta %s p. su poder de ataque cuerpo a cuerpo y permite que los ataques cuerpo a cuerpo generen una amenaza extra cuando se usa esa arma. dura %s minutos"] = { { Stats.AttackPower, }, false, } -- s16315
L["aumenta un %s% tu probabilidad de esquivar durante %s s"] = { { Stats.Dodge, }, false, } -- s16321
L["aumenta %s p. la resistencia a la escarcha durante %s min"] = { { Stats.FrostResistance, }, false, } -- s16325
L["aumenta %s p. la resistencia al fuego durante %s min"] = { { Stats.FireResistance, }, false, } -- s16326
L["aumenta el poder de ataque %s p. durante %s min"] = { { Stats.GenericAttackPower, }, false, } -- s16329
L["reduce la resistencia a las sombras de un enemigo %s p. durante %s min"] = { { Stats.ShadowResistance, }, false, reduction = true } -- s16429
L["reduce la probabilidad de un enemigo un %s% durante %s min"] = { { Stats.Dodge, }, false, reduction = true } -- s16448
L["reduce el poder de ataque cuerpo a cuerpo de un enemigo %s p. y su poder de ataque a distancia %s p. durante %s min"] = { { Stats.GenericAttackPower, }, false, false, reduction = true } -- s16449
L["golpea a un objetivo, le inflige %s p. de daño de fuego y aumenta %s p. el daño de fuego que le infliges al objetivo durante %s s"] = { false, { Stats.FireDamage, }, false, } -- s16454
L["reduce la velocidad de movimiento de un enemigo un %s% y reduce el daño físico y mágico que inflige %s p. dura %s min"] = { false, { Stats.SpellDamage, }, false, reduction = true } -- s16458
L["reduce un %s% la probabilidad de un enemigo de asestar un golpe crítico en un ataque cuerpo a cuerpo y reduce un %s% su probabilidad de asestar un golpe crítico con un hechizo. dura %s min"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, { Stats.SpellCrit, }, false, reduction = true } -- s16460
L["aumenta la armadura %s p. durante %s s pero impide lanzar otros hechizos y ataques mientras dure"] = { { Stats.Armor, }, false, } -- s16470
L["reduce la armadura de un enemigo %s p. durante %s min. mientras esté afectado, el objetivo no puede utilizar sigilo ni invisibilidad"] = { { Stats.Armor, }, false, reduction = true } -- s16498
L["aumenta %s p. el daño de las sombras causado por un aliado durante %s min"] = { { Stats.ShadowDamage, }, false, } -- s16587
L["aumenta %s p. los puntos de salud. %s% de celeridad a los ataques cuerpo a cuerpo. %s p. de regeneración de maná cada %s s"] = { { Stats.Health, }, { Stats.MeleeHaste, }, { Stats.GenericManaRegen, }, false, } -- s16609
L["aumenta la fuerza %s p"] = { { Stats.Strength, }, } -- s16612
L["aumenta el aguante %s p. dura %s min"] = { { Stats.Stamina, }, false, } -- s16617
L["aumenta %s p. el daño físico infligido por el taumaturgo y acelera un %s% su ataque durante %s s"] = { { Stats.AverageWeaponDamage, }, { Stats.MeleeHaste, }, false, } -- s16789
L["reduce la probabilidad de los enemigos cercanos de golpear un %s% durante %s s"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, reduction = true } -- s16867
L["el enemigo sufre la maldición leñogris, que reduce sus resistencias a la magia %s p. se puede aplicar un máximo de %s veces"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, reduction = true } -- s16871
L["imbuye al objetivo de poder sagrado y aumenta su aguante en %s p. durante %s min"] = { { Stats.Stamina, }, false, } -- s16873
L["aumenta el intelecto del objetivo %s p. durante %s min"] = { { Stats.Intellect, }, false, } -- s16876
L["aumenta %s p. la armadura de un objetivo amistoso, %s p. todos los atributos y %s p. todas las resistencias durante %s min"] = { { Stats.Armor, }, { Stats.AllStats, }, false, false, } -- s16878
L["aumenta tu fuerza %s p., el aguante %s p. y tu tamaño durante %s min"] = { { Stats.Strength, }, { Stats.Stamina, }, false, } -- s16883
L["aumenta la salud máxima del jugador %s p. durante %s h"] = { { Stats.Health, }, false, } -- s16884
L["aumenta la armadura en %s p. durante %s h"] = { { Stats.Armor, }, false, } -- s16887
L["aumenta el intelecto %s p. durante %s h"] = { { Stats.Intellect, }, false, } -- s16888
L["aumenta el daño con hechizos %s p. durante %s min"] = { { Stats.SpellDamage, }, false, } -- s16889
L["crea una tormenta violenta en el área objetivo que inflige %s p. de daño de naturaleza a los enemigos cada %s s y aumenta el tiempo entre los ataques de los enemigos un %s%. dura %s s. el druida debe canalizar su magia para mantener el hechizo"] = { false, false, { Stats.MeleeHaste, }, false, reduction = true } -- s16914
L["te sana %s p. y aumenta tu fuerza %s p. durante %s s"] = { false, { Stats.Strength, }, false, } -- s16916
L["el movimiento del objetivo se ralentiza un %s% y aumenta el tiempo entre los ataques un %s% durante %s s"] = { false, { Stats.MeleeHaste, }, false, reduction = true } -- s16927
L["reduce la armadura de un enemigo %s p. se puede acumular hasta %s veces"] = { { Stats.Armor, }, false, reduction = true } -- s16928
L["aumenta un %s% la probabilidad de golpe crítico de los miembros del grupo. dura %s s"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, false, } -- s16939
L["aumenta la probabilidad de conseguir un golpe crítico mientras estés en forma de oso, oso temible o felina un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s16942
L["aumenta la agilidad %s p"] = { { Stats.Agility, }, } -- s17013
L["aumenta %s p. tu poder de ataque cuerpo a cuerpo y tu tamaño durante %s min"] = { { Stats.AttackPower, }, false, } -- s17038
L["inflige %s p. de daño y reduce su intelecto %s p. durante %s s"] = { false, { Stats.Intellect, }, false, reduction = true } -- s17148
L["aumenta %s p. la fuerza durante %s s"] = { { Stats.Strength, }, false, } -- s17152
L["inflige %s p. de daño de naturaleza al atacante al golpear y aumenta la resistencia a la naturaleza %s p. durante %s s"] = { false, { Stats.NatureResistance, }, false, } -- s17154
L["aumenta la resistencia a lo arcano de un aliado %s p. durante %s h"] = { { Stats.ArcaneResistance, }, false, } -- s17175
L["defensa %s p. ¡cómo si hubieras vuelto a nacer!"] = { { Stats.Defense, }, } -- s17178
L["reduce todas las estadísticas %s p. e inflige %s p. de daño de naturaleza cada %s s a todos los enemigos que estén en un radio de %s m del taumaturgo durante %s s"] = { { Stats.AllStats, }, false, false, false, false, reduction = true } -- s17196
L["aumenta el poder de ataque cuerpo a cuerpo del taumaturgo %s p. durante %s min"] = { { Stats.AttackPower, }, false, } -- s17205
L["controla a un enemigo humanoide hasta el nivel %s durante %s min y aumenta su salud máxima un %s%"] = { false, false, { Stats.Health, }, } -- s17244
L["aumenta la resistencia al fuego en %s p. y causa %s p. de daño de fuego a cualquiera que te ataque en un cuerpo a cuerpo durante %s min"] = { { Stats.FireResistance, }, false, false, } -- s17275
L["pincha la armadura del objetivo y la reduce %s p. se puede aplicar hasta %s veces"] = { { Stats.Armor, }, false, reduction = true } -- s17315
L["disminuye la velocidad de lanzamiento de los enemigos y aumenta el tiempo entre sus ataques cuerpo a cuerpo y a distancia un %s% durante %s s"] = { { Stats.SpellHaste, }, false, reduction = true } -- s17331
L["cuando te golpean en combate, tiene un %s% de probabilidad de aumentar la probabilidad de bloqueo un %s% durante %s s"] = { false, { Stats.BlockChance, }, false, ignoreSum = true } -- s17350
L["la probabilidad de bloqueo aumenta un %s%"] = { { Stats.BlockChance, }, } -- s17351
L["crea una tormenta violenta en el área objetivo que inflige %s p. de daño de naturaleza a los enemigos cada %s s y aumenta el tiempo entre los ataques un %s%. dura %s s. el druida debe canalizar su magia para mantener el hechizo"] = { false, false, { Stats.MeleeHaste, }, false, reduction = true } -- s17401
L["reduce el poder de ataque del objetivo %s p. durante %s s y te lo transfiere a ti en forma de espíritu"] = { { Stats.GenericAttackPower, }, false, reduction = true } -- s17494
L["reduce todos los atributos del objetivo %s p. durante %s min"] = { { Stats.AllStats, }, false, reduction = true } -- s17505
L["aumenta %s p. la ira y %s p. la fuerza durante %s s"] = { false, { Stats.Strength, }, false, } -- s17528
L["aumenta el intelecto y el espíritu %s p. durante %s h"] = { { Stats.Intellect, Stats.Spirit, }, false, } -- s17535
L["aumenta %s p. la fuerza y el aguante durante %s h"] = { { Stats.Strength, Stats.Stamina, }, false, } -- s17537
L["aumenta %s p. la agilidad y un %s% la probabilidad de asestar un golpe crítico durante %s h"] = { { Stats.Agility, }, { Stats.MeleeCrit, Stats.RangedCrit, }, false, } -- s17538
L["aumenta %s p. la salud máxima del jugador durante %s h. solo puedes disfrutar del efecto de un frasco a la vez. el efecto persiste aun después de la muerte"] = { { Stats.Health, }, false, } -- s17626
L["aumenta el maná máximo del jugador %s p. durante %s h. solo puedes disfrutar del efecto de un frasco a la vez. el efecto persiste aun después de la muerte"] = { { Stats.Mana, }, false, } -- s17627
L["aumenta el daño que infligen los hechizos y efectos mágicos hasta %s p. durante %s h. solo puedes disfrutar del efecto de un frasco a la vez. el efecto persiste aun después de la muerte"] = { { Stats.SpellDamage, }, false, } -- s17628
L["aumenta %s p. tu resistencia a todas las escuelas de magia durante %s h. solo puedes disfrutar del efecto de un frasco a la vez. el efecto persiste aun después de la muerte y se acumula con todos los demás objetos y hechizos de resistencia"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, } -- s17629
L["aumenta la velocidad de lanzamiento de un aliado un %s% durante %s s"] = { { Stats.SpellHaste, }, false, } -- s17633
L["rodea al taumaturgo con un aura que reduce periódicamente %s p. la resistencia a las sombras de los enemigos cercanos. el aura dura hasta que se cancela"] = { { Stats.ShadowResistance, }, ignoreSum = true, reduction = true } -- s17695
L["mejora tu probabilidad de asestar un golpe crítico un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s17713
L["maldice al objetivo durante %s min, disminuyendo su resistencia a las sombras y arcana en %s p. y aumentando el daño de las sombras y arcano que recibe en un %s%. un brujo solo puede tener una maldición activa a la vez sobre un mismo objetivo"] = { false, { Stats.ShadowResistance, Stats.ArcaneResistance, }, false, reduction = true } -- s17862
L["corta la armadura de un enemigo y la de sus aliados cercanos. alcanza hasta a %s objetivos. la armadura de un objetivo se reduce %s p. por cada uso de hender armadura. se puede aplicar hasta %s veces. dura %s s"] = { false, { Stats.Armor, }, false, false, reduction = true } -- s17963
L["corroe la armadura de un enemigo y la reduce %s p. en cada aplicación. puede aplicarse hasta un máximo de %s veces. dura %s s"] = { { Stats.Armor, }, false, false, } -- s18070
L["cuando recibes un golpe en combate, tiene un %s% de probabilidad de alzar un escudo con espinas que inflige %s p. de daño de naturaleza a los atacantes y aumenta %s p. tu resistencia a la naturaleza durante %s s"] = { false, false, { Stats.NatureResistance, }, false, ignoreSum = true } -- s18097
L["aumenta %s p. la fuerza durante %s min"] = { { Stats.Strength, }, false, } -- s18125
L["restaura %s p. de maná durante %s s. debes permanecer sentado mientras bebes. también aumenta tu espíritu %s p. durante %s min"] = { { Stats.GenericManaRegen, }, false, { Stats.Spirit, }, false, } -- s18140
L["reduce la resistencia a la naturaleza de un enemigo %s p. durante %s min"] = { { Stats.NatureResistance, }, false, reduction = true } -- s18151
L["aumenta el aguante %s p. durante %s min"] = { { Stats.Stamina, }, false, } -- s18191
L["restaura %s p. de salud durante %s s. debes permanecer sentado mientras comes. si comes durante %s s también aumentará %s p. tu agilidad durante %s min"] = { false, false, false, { Stats.Agility, }, false, ignoreSum = true } -- s18230
L["da %s p. de intelecto extra a los miembros del grupo en un radio de %s m"] = { { Stats.Intellect, }, false, } -- s18264
L["restaura %s p. de maná cada %s s"] = { { Stats.GenericManaRegen, }, false, } -- s18378
L["reduce el poder de ataque de un enemigo %s p. durante %s s"] = { { Stats.GenericAttackPower, }, false, reduction = true } -- s18381
L["mejora tu probabilidad de asestar un golpe crítico con hechizos un %s%"] = { { Stats.SpellCrit, }, } -- s18382
L["aumenta %s p. el daño físico que inflige el taumaturgo y acelera sus ataques un %s% durante %s s"] = { { Stats.AverageWeaponDamage, }, { Stats.MeleeHaste, }, false, } -- s18501
L["pica al enemigo y reduce su fuerza y agilidad %s p. durante %s s"] = { { Stats.Strength, Stats.Agility, }, false, reduction = true } -- s18545
L["inmoviliza al taumaturgo pero aumenta la velocidad de sus ataques un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, } -- s18546
L["%s p. todas las resistencias"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- s18672
L["aumenta %s p. el poder de ataque durante %s s"] = { { Stats.GenericAttackPower, }, false, } -- s18787
L["al recibir un golpe en combate hay un %s% de probabilidad de rodear al taumaturgo con hueso aumentando su armadura en %s durante %s s"] = { false, { Stats.Armor, }, false, } -- s18828
L["aumenta la armadura %s p. durante %s s"] = { { Stats.Armor, }, false, } -- s18946
L["quema a un enemigo infligiendo %s p. de daño y reduce su resistencia al fuego %s p. durante %s s"] = { false, { Stats.FireResistance, }, false, reduction = true } -- s18958
L["aumenta el tiempo entre los ataques de los enemigos cercanos un %s% y ralentiza su velocidad de movimiento un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, false, reduction = true } -- s19137
L["cuando se activa, aumenta la probabilidad de esquivar y de parar un %s% durante %s s"] = { { Stats.Parry, Stats.Dodge, }, false, } -- s19263
L["reduce el daño a distancia en %s p. y aumenta la probabilidad de esquivar en un %s% durante %s s"] = { false, { Stats.Dodge, }, false, } -- s19289
L["aumenta la probabilidad de golpear un %s% y la probabilidad de resistir los efectos de reducción de movimiento un %s% más"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, } -- s19290
L["reduce el tiempo de lanzamiento un %s% y aumenta el tiempo entre los ataques en un%s% durante %s min"] = { { Stats.SpellHaste, }, { Stats.MeleeHaste, Stats.RangedHaste, }, false, reduction = true } -- s19365
L["reduce la resistencia al fuego de los enemigos cercanos %s p. durante %s min"] = { { Stats.FireResistance, }, false, reduction = true } -- s19366
L["aumenta un %s% tu probabilidad de conseguir un golpe crítico con todos los ataques"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s19370
L["aumenta tu probabilidad de golpe crítico un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s19426
L["aumenta tu probabilidad de golpe crítico con armas a distancia un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s19426
L["corrompe la sangre del manáfago durante %s s. si el manáfago recibe un golpe cuerpo a cuerpo, el atacante pierde %s p. de poder de ataque cuerpo a cuerpo durante %s s. se acumula hasta %s veces en un objetivo"] = { false, { Stats.AttackPower, }, false, false, ignoreSum = true, reduction = true } -- s19478
L["aumenta %s p. el poder de ataque a distancia de los miembros del grupo que están en un radio de %s m, y %s p. más el poder de ataque a distancia del cazador. dura %s min"] = { { Stats.GenericAttackPower, }, false, { Stats.RangedAttackPower, }, false, } -- s19506
L["aumenta %s p. el poder de ataque de los miembros del grupo que están en un radio de %s m, y %s p. más el poder de ataque a distancia del cazador. dura %s min"] = { { Stats.GenericAttackPower, }, false, { Stats.RangedAttackPower, }, false, } -- s19506
L["aumenta %s p. el poder de ataque a distancia de los miembros del grupo que están en un radio de %s m. dura %s min"] = { { Stats.GenericAttackPower, }, false, false, } -- s19506
L["aumenta %s p. el poder de ataque de los miembros del grupo que están en un radio de %s m. dura %s min"] = { { Stats.GenericAttackPower, }, false, false, } -- s19506
L["da a tu mascota la probabilidad de aumentar un %s% su velocidad de ataque durante %s s después de asestar un golpe crítico"] = { { Stats.MeleeHaste, }, false, } -- s19615
L["aumenta todas las resistencias %s p. durante %s s"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, } -- s19638
L["coloca una bendición sobre el objetivo aliado que aumenta %s p. el poder de ataque cuerpo a cuerpo durante %s min. los jugadores solo pueden tener una bendición activa por paladín a la vez"] = { { Stats.AttackPower, }, false, } -- s19740
L["reduce el poder de ataque cuerpo a cuerpo y a distancia de los enemigos cercanos %s p. durante %s s"] = { { Stats.GenericAttackPower, }, false, reduction = true } -- s19778
L["aumenta un %s% el daño físico causado por un aliado y acelera un %s% sus ataques durante %s s"] = { false, { Stats.MeleeHaste, }, false, } -- s19779
L["encanta de forma permanente una capa que aumenta %s p. de resistencia al fuego"] = { { Stats.FireResistance, }, } -- s19932
L["encanta de forma permanente unos brazales para que ototguen %s p. de intelecto"] = { { Stats.Intellect, }, } -- s20008
L["encanta de forma permanente unos brazales para que otorguen %s p. de espíritu"] = { { Stats.Spirit, }, } -- s20009
L["encanta de forma permanente unos brazales para que otorguen %s p. de fuerza"] = { { Stats.Strength, }, } -- s20010
L["encanta una capa de forma permanente para que otorgue %s p. a todas las resistencias"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- s20014
L["encanta de forma permanente una capa para que proporcione %s p. más de armadura"] = { { Stats.Armor, }, } -- s20015
L["encanta una pieza de armadura de torso de forma permanente para otorgar %s a todas las estadísticas"] = { { Stats.AllStats, }, } -- s20025
L["encanta de forma permanente una pieza de armadura de torso para que proporcione %s p. de salud"] = { { Stats.Health, }, } -- s20026
L["encanta de forma permanente una pieza de armadura de torso para que proporcione %s p. de maná"] = { { Stats.Mana, }, } -- s20028
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para que inflija %s p. más de daño"] = { { Stats.AverageWeaponDamage, }, } -- s20031
L["encanta de forma permanente un arma de combate cuerpo a cuerpo de dos manos para añadir %s p. de espíritu"] = { { Stats.Spirit, }, } -- s20035
L["encanta de forma permanente un arma de combate cuerpo a cuerpo de dos manos para añadir %s p. de intelecto"] = { { Stats.Intellect, }, } -- s20036
L["aumenta la probabilidad de asestar un golpe crítico con armas cuerpo a cuerpo un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s20117
L["aumenta la probabilidad de bloquear ataques con tu escudo un %s% después de ser víctima de un golpe crítico. dura %s s o %s ataques bloqueados"] = { { Stats.BlockChance, }, false, false, ignoreSum = true } -- s20127
L["colma al paladín del espíritu de un cruzado durante %s s, otorgando %s p. de poder de ataque cuerpo a cuerpo. el paladín ataca un %s% más rápido pero causa menos daño en cada ataque. un paladín solo puede tener un sello activo a la vez.\r\n\r\ndesatar la energía de este sello sentenciará a un enemigo durante %s s, aumentando el daño sagrado recibido en un máximo de %s p. tus golpes cuerpo a cuerpo restablecerán la duración del hechizo. un paladín solo puede tener una sentencia activa a la vez"] = { false, { Stats.AttackPower, }, false, false, false, } -- s20162
L["la resistencia a la naturaleza aumenta %s p"] = { { Stats.NatureResistance, }, } -- s20551
L["la resistencia a las sombras aumenta %s p"] = { { Stats.ShadowResistance, }, } -- s20579
L["aumenta la probabilidad de esquivar un %s%"] = { { Stats.Dodge, }, } -- s20582
L["aumenta el aguante %s p. y reduce el daño físico sufrido %s p. durante %s min. sin embargo, reduce la fuerza y la agilidad %s p. y aumenta el daño mágico recibido hasta %s p"] = { { Stats.Stamina, }, false, false, false, false, } -- s20587
L["resistencia a lo arcano aumentada %s p"] = { { Stats.ArcaneResistance, }, } -- s20592
L["la resistencia a la escarcha aumenta %s p"] = { { Stats.FrostResistance, }, } -- s20596
L["bastones aumentados %s"] = { { Stats.WeaponSkill, }, } -- s20607
L["reduce tu probabilidad de parar un ataque un %s%"] = { { Stats.Parry, }, reduction = true } -- s21352
L["sana a un aliado %s p. de daño cada %s s y aumenta su armadura %s p. durante %s min"] = { false, false, { Stats.Armor, }, false, } -- s20655
L["controla a un enemigo humanoide hasta el nivel %s durante %s s, aumenta %s p. el daño de magia y %s p. el daño físico que inflige"] = { false, false, { Stats.SpellDamage, }, { Stats.AverageWeaponDamage, }, } -- s20668
L["aumenta la armadura del taumaturgo %s p y acelera su regeneración de salud durante %s min"] = { { Stats.Armor, }, false, } -- s20798
L["tiempo entre los ataques de un enemigo aumentado un %s% y movimiento reducido un %s%. la fuerza se reduce un %s%. dura %s s"] = { { Stats.MeleeHaste, }, false, false, false, reduction = true } -- s20812
L["al recibir un golpe en combate hay un %s% de probabilidad de aumentar la armadura de todo el grupo en %s durante %s s"] = { false, { Stats.Armor, }, false, ignoreSum = true } -- s20847
L["¡aumenta %s p. el aguante durante %s min y te emborracha hasta la médula!"] = { { Stats.Stamina, }, false, } -- s20875
L["subyuga al demonio objetivo, de nivel %s como máximo, y lo obliga a seguir tus órdenes. mientras está subyugado, el tiempo entre los ataques del demonio aumenta un %s% y su velocidad de lanzamiento de hechizos se reduce un %s%. dura un máximo de %s min"] = { false, { Stats.MeleeHaste, }, { Stats.SpellHaste, }, false, reduction = true } -- s20882
L["aumenta la probabilidad de bloqueo un %s% durante %s s e inflige %s p. de daño sagrado por cada ataque bloqueado mientras está activo. el daño que inflige escudo sagrado genera un %s% más de amenaza.\r\n\r\nactivar escudo sagrado también te otorga %s p. de poder con hechizos por cada punto de tu habilidad de defensa por encima de %s p. dura %s s"] = { { Stats.BlockChance, }, false, false, false, { Stats.SpellDamage, }, false, false, } -- s20925
L["aumenta la probabilidad de bloqueo un %s% durante %s s e inflige %s p. de daño sagrado por cada ataque bloqueado mientras está activo. el daño que inflige escudo sagrado genera un %s% más de amenaza. cada bloqueo consume una carga. %s cargas.\r\n\r\nactivar escudo sagrado también te otorga %s p. de poder con hechizos por cada punto de tu habilidad de defensa por encima de %s p. dura %s s"] = { { Stats.BlockChance, }, false, false, false, false, { Stats.SpellDamage, }, false, false, } -- s20925
L["aumenta la probabilidad de bloqueo un %s% durante %s s e inflige %s p. de daño sagrado por cada ataque bloqueado mientras está activo. el daño que inflige escudo sagrado genera un %s% más de amenaza"] = { { Stats.BlockChance, }, false, false, false, } -- s20925
L["aumenta la probabilidad de bloqueo un %s% durante %s s e inflige %s p. de daño sagrado por cada ataque bloqueado mientras está activo. el daño que inflige escudo sagrado genera un %s% más de amenaza. cada bloqueo consume una carga. %s cargas"] = { { Stats.BlockChance, }, false, false, false, false, } -- s20925
L["aumenta %s p. el poder de ataque de tu mascota"] = { { Stats.GenericAttackPower, }, } -- s20988
L["aumenta %s p. el poder de ataque a distancia"] = { { Stats.RangedAttackPower, }, } -- s21013
L["azota la armadura del enemigo, reduciéndola %s p. con cada machacar armadura. se puede ejecutar un máximo de %s veces. dura %s s"] = { { Stats.Armor, }, false, false, } -- s21055
L["mejora un %s% tu probabilidad de conseguir un golpe crítico con hechizos sagrados"] = { { Stats.SpellCrit, }, } -- s21092
L["golpea a los enemigos cercanos con hielo, lo que les inflige %s p. de daño, aumenta el tiempo entre sus ataques un %s% y reduce su velocidad de movimiento un %s%"] = { false, { Stats.MeleeHaste, }, false, reduction = true } -- s21098
L["lanza una descarga de magia oscura al enemigo, lo que inflige %s p. de daño de las sombras y reduce todas las estadísticas %s p. durante %s s"] = { false, { Stats.AllStats, }, false, reduction = true } -- s21151
L["tus ataques ignoran %s p. de la armadura de tus enemigos durante %s s. este efecto se acumula hasta %s veces"] = { { Stats.ArmorPenetration, }, false, false, } -- s21153
L["restaura %s p. de salud y %s p. de maná por segundo cuando se está cerca de un comandante"] = { { Stats.HealthRegen, }, false, } -- s21172
L["restaura %s p. de salud y %s p. de maná por segundo cuando se está cerca de un teniente"] = { { Stats.HealthRegen, }, false, } -- s21174
L["aumenta hasta %s p. el daño que infligen los hechizos y efectos sagrados"] = { { Stats.HolyDamage, }, } -- s21499
L["aumenta el valor de bloqueo de tu escudo %s p"] = { { Stats.BlockValue, }, } -- s21539
L["infunde poder al grupo del objetivo y aumenta su aguante %s p. durante %s h"] = { { Stats.Stamina, }, false, } -- s21562
L["infunde poder al grupo del objetivo y aumenta su aguante en %s p. durante %s h"] = { { Stats.Stamina, }, false, } -- s21564
L["tu mascota gana %s p. de aguante y %s p. de resistencia a los hechizos contra todos los tipos de magia"] = { { Stats.Stamina, }, false, ignoreSum = true } -- s21741
L["inflige daño de naturaleza a los enemigos de una zona durante %s s, reduciendo su velocidad de movimiento un %s% y aumentando el tiempo entre sus ataques un %s%"] = { false, false, { Stats.MeleeHaste, }, reduction = true } -- s21793
L["cuando infliges daño cuerpo a cuerpo tienes un %s% de probabilidad de reducir %s p. el poder de ataque de tu víctima durante %s s"] = { false, { Stats.AttackPower, }, false, ignoreSum = true, reduction = true } -- s21841
L["entrega la don de lo salvaje al grupo del objetivo y aumenta su armadura en %s p., todos los atributos en %s p. y todas las resistencias en %s durante %s h"] = { { Stats.Armor, }, { Stats.AllStats, }, false, false, } -- s21849
L["mejora tu probabilidad de asestar golpes críticos un %s% con los hechizos de naturaleza"] = { { Stats.SpellCrit, }, } -- s21870
L["aumenta %s p. el daño de escarcha con hechizos durante %s min"] = { { Stats.FrostDamage, }, false, } -- s21920
L["aumenta el aguante de tu mascota %s p. y todas las resistencias a los hechizos %s p"] = { { Stats.Stamina, }, false, ignoreSum = true } -- s21926
L["encanta de forma permanente un arma para que otorgue hasta %s p. de daño de escarcha extra cuando se lanzan hechizos de escarcha"] = { { Stats.FrostDamage, }, } -- s21931
L["aumenta el poder de ataque de tu mascota %s p"] = { { Stats.GenericAttackPower, }, ignoreSum = true } -- s21938
L["inflige %s p. de daño y reduce su armadura %s p"] = { false, { Stats.Armor, }, reduction = true } -- s21961
L["el poder de ataque aumenta %s p"] = { { Stats.GenericAttackPower, }, } -- s21967
L["aumenta todas las estadísticas %s p. durante %s min"] = { { Stats.AllStats, }, false, } -- s21970
L["añade una mira permanente a un arco o arma de fuego que aumenta su probabilidad de golpear un %s%"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- s22779
L["golpea a tu enemigo con relámpagos, inflige %s p. de daño de naturaleza. a continuación pasa a otros enemigos cercanos, reduciendo su resistencia a la naturaleza %s p. puede utilizarse sobre %s objetivos. además, el primero de los objetivos es golpeado por un ciclón que reduce su velocidad de ataque un %s% durante %s s"] = { false, { Stats.NatureResistance, }, false, { Stats.MeleeHaste, }, false, reduction = true } -- s21992
L["encanta de forma permanente una capa que aumenta %s p. de todas las resistencias"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- s22091
L["encanta de forma permanente un arma de combate cuerpo a cuerpo de dos manos para que cause %s p. de daño"] = { { Stats.AverageWeaponDamage, }, } -- s22094
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para que cause %s p. extra de daño"] = { { Stats.AverageWeaponDamage, }, } -- s22095
L["encanta de forma permanente unas botas para proporcionar %s p. de agilidad"] = { { Stats.Agility, }, } -- s22101
L["encanta de forma permanente unas botas para proporcionar %s p. de espíritu"] = { { Stats.Spirit, }, } -- s22102
L["encanta de forma permanente unas botas para proporcionar %s p. de aguante"] = { { Stats.Stamina, }, } -- s22103
L["encanta de forma permanente unos guantes para que concedan %s p. de fuerza"] = { { Stats.Strength, }, } -- s22104
L["encanta de forma permanente unos guantes para que concedan %s p. de agilidad"] = { { Stats.Agility, }, } -- s22105
L["encanta de forma permanente unos guantes para que concedan un bonus de %s p. de velocidad de ataque"] = { { Stats.MeleeHaste, Stats.RangedHaste, }, } -- s22106
L["ballestas aumentadas %s p"] = { { Stats.WeaponSkill, }, } -- s22188
L["disminuye %s p. el daño mágico infligido por el objetivo durante %s min"] = { { Stats.SpellDamage, }, false, reduction = true } -- s22371
L["reduce el aguante de los enemigos cercanos %s p. durante %s min"] = { { Stats.Stamina, }, false, reduction = true } -- s22420
L["azota la armadura del enemigo y la reduce %s p. con cada machacar armadura. se puede aplicar hasta %s veces. dura %s s"] = { { Stats.Armor, }, false, false, } -- s22426
L["añade de forma permanente %s p. de resistencia al fuego a un objeto de la casilla de hombros"] = { { Stats.FireResistance, }, } -- s22593
L["añade de forma permanente %s p. de resistencia a la escarcha a un objeto de la casilla de hombros"] = { { Stats.FrostResistance, }, } -- s22594
L["añade de forma permanente %s p. de resistencia a las sombras a un objeto de la ranura de hombros"] = { { Stats.ShadowResistance, }, } -- s22596
L["añade de forma permanente %s p. de resistencia a la naturaleza a un objeto de la ranura de hombros"] = { { Stats.NatureResistance, }, } -- s22597
L["añade de forma permanente %s p. de resistencia a lo arcano a un objeto de la ranura de hombros"] = { { Stats.ArcaneResistance, }, } -- s22598
L["añade de forma permanente %s p. de resistencia a todas las escuelas de magia a un objeto de la casilla de hombros"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- s22599
L["aumenta la habilidad en defensa %s p. durante %s min"] = { { Stats.Defense, }, false, } -- s22685
L["aumenta %s p. de forma permanente el valor de defensa de un objeto para el torso, las piernas, las manos o los pies. solo puede usarse en objetos de nivel %s o superior"] = { { Stats.Defense, }, false, } -- s22725
L["aumenta el intelecto %s p. durante %s min"] = { { Stats.Intellect, }, false, } -- s22730
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para que añada hasta %s p. de daño a los hechizos"] = { { Stats.SpellPower, }, } -- s22749
L["encanta de forma permanente un arma de combate cuerpo a cuerpo que añade %s p. de sanación a los hechizos de sanación"] = { { Stats.HealingPower, }, } -- s22750
L["un filo cortante aumenta un %s% tu probabilidad de asestar un golpe crítico"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s22755
L["aumenta un %s% la probabilidad de golpe crítico con un arma cuerpo a cuerpo durante %s minutos"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, false, } -- s22756
L["aumenta un %s% tu probabilidad de golpear con un arma de proyectiles"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- s22780
L["aumenta tu resistencia a toda la magia en %s p. y permite que un %s% de tu regeneración de maná continúe mientras lanzas hechizos. un mago solo puede tener un tipo de hechizo de armadura activo a la vez. dura %s min"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, false, } -- s22782
L["aumenta tu resistencia a toda la magia en %s p. y permite que continúe un %s% de tu regeneración de maná mientras lanzas hechizos. un mago solo puede tener un tipo de hechizo de armadura activo a la vez. dura %s min"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, false, } -- s22783
L["¡aumenta %s p. el aguante durante %s min y te emborracha hasta la médula! ¡verde significa que es bueno!"] = { { Stats.Stamina, }, false, } -- s22789
L["aumenta el espíritu %s p., pero reduce el intelecto %s p. durante %s min. suaaave"] = { { Stats.Spirit, }, { Stats.Intellect, }, false, } -- s22790
L["poder de ataque aumentado %s p"] = { { Stats.GenericAttackPower, }, } -- s22817
L["la probabilidad de golpe crítico con un hechizo aumenta un %s%"] = { { Stats.SpellCrit, }, } -- s22820
L["añade de forma permanente un %s% de celeridad a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.MeleeHaste, Stats.RangedHaste, }, } -- s22840
L["añade de forma permanente %s de sanación o daño con hechizos a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.SpellPower, }, } -- s22844
L["añade de forma permanente un %s% de posibilidad de esquivar a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Dodge, }, } -- s22846
L["cuando se activa, otorga al portador %s p. de defensa y %s p. de armadura durante %s s"] = { { Stats.Defense, }, { Stats.Armor, }, false, } -- s22850
L["aumenta un %s% la probabilidad de golpe crítico con hechizos, un %s% para ataques cuerpo a cuerpo y a distancia, y otorga %s p. de poder de ataque. dura %s minutos"] = { { Stats.SpellCrit, }, { Stats.MeleeCrit, Stats.RangedCrit, }, { Stats.GenericAttackPower, }, false, } -- s22888
L["consumido por la furia de illidan: %s p. de bonus de poder de ataque contra demonios, %s% de bonus de probabilidad de golpe, %s% de celeridad cuerpo a cuerpo"] = { false, { Stats.MeleeHit, Stats.RangedHit, }, { Stats.MeleeHaste, }, } -- s22988
L["infunde lucidez al grupo del objetivo, aumentando su intelecto en %s p. durante %s h"] = { { Stats.Intellect, }, false, } -- s23028
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para añadir hasta %s p. de daño a todos los hechizos"] = { { Stats.SpellPower, }, } -- s23143
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para añadir hasta %s p. de sanación a los hechizos de sanación"] = { { Stats.HealingPower, }, } -- s23144
L["reduce la defensa en %s p. durante %s s. esta reducción aumenta la probabilidad de golpear al enemigo consiguiendo un golpe crítico y reduce la probabilidad de que el objetivo bloquee, esquive o pare el ataque"] = { { Stats.Defense, }, false, reduction = true } -- s23162
L["reduce %s p. el poder de ataque cuerpo a cuerpo de un enemigo durante %s s"] = { { Stats.AttackPower, }, false, reduction = true } -- s23262
L["aumenta hasta %s p. el daño y la sanación de los hechizos y efectos mágicos durante %s s"] = { { Stats.SpellPower, }, false, } -- s23271
L["mejora tu probabilidad de conseguir un golpe crítico un %s% con los hechizos"] = { { Stats.SpellCrit, }, } -- s23433
L["mejora tu probabilidad de conseguir un golpe crítico un %s% con los hechizos de las sombras"] = { { Stats.SpellCrit, }, } -- s23440
L["mejora un %s% tu probabilidad de conseguir un golpe crítico con hechizos de las sombras"] = { { Stats.SpellCrit, }, } -- s23443
L["aumenta el daño causado por los hechizos de fuego y los efectos hasta en %s p"] = { { Stats.FireDamage, }, } -- s23480
L["aumenta un %s% la velocidad de ataque del taumaturgo y el daño físico que inflige %s p. durante %s min"] = { { Stats.MeleeHaste, }, { Stats.AverageWeaponDamage, }, false, } -- s26527
L["aumenta %s p. el valor de bloqueo de tu escudo"] = { { Stats.BlockValue, }, } -- s23562
L["acelera la mente y aumenta un %s% la velocidad de lanzamiento del mago durante %s s"] = { { Stats.SpellHaste, }, false, } -- s23723
L["mejora un %s% tu probabilidad de golpear con hechizos"] = { { Stats.SpellHit, }, } -- s23727
L["energiza a un paladín con luz, lo que aumenta un %s% la velocidad de ataque cuerpo a cuerpo y un %s% la de lanzamiento de hechizos durante %s s"] = { { Stats.MeleeHaste, }, { Stats.SpellHaste, }, false, } -- s23733
L["aumenta la armadura %s p. y sana %s p. de daño cada vez que sufres daño a distancia o en ataque cuerpo a cuerpo durante %s s"] = { { Stats.Armor, }, false, false, } -- s23780
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para que proporcione %s de fuerza"] = { { Stats.Strength, }, } -- s23799
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para que proporcione %s p. de agilidad"] = { { Stats.Agility, }, } -- s23800
L["encanta de forma permanente unos brazales para que restauren %s p. de maná cada %s s"] = { { Stats.GenericManaRegen, }, false, } -- s23801
L["encanta de forma permanente unos brazales para que aumenten %s p. los efectos de tus hechizos de sanación"] = { { Stats.HealingPower, }, } -- s23802
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para que proporcione %s p. de espíritu"] = { { Stats.Spirit, }, } -- s23803
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para que proporcione %s p. de intelecto"] = { { Stats.Intellect, }, } -- s23804
L["inflige %s p. de daño de naturaleza a los enemigos cercanos, lo que aumenta el tiempo entre sus ataques un %s% y reduce su velocidad de movimiento un %s% durante %s s"] = { false, { Stats.MeleeHaste, }, false, false, reduction = true } -- s23931
L["infunde poder al objetivo y aumenta su aguante en %s p. durante %s h"] = { { Stats.Stamina, }, false, } -- s23947
L["cuando está activo, la probabilidad de golpe crítico con hechizos y ataques aumenta un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- s23964
L["aumenta el daño infligido %s p. durante %s s"] = { { Stats.AverageWeaponDamage, }, false, } -- s24043
L["reduce la armadura de un enemigo %s p. e inflige %s p. de daño de naturaleza cada %s s durante %s s"] = { { Stats.Armor, }, false, false, false, reduction = true } -- s24111
L["añade de forma permanente %s p. de aguante, %s p. de defensa y %s p. de valor de bloqueo de escudo a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Defense, }, { Stats.BlockValue, }, } -- s24149
L["añade de forma permanente %s p. de aguante y %s p. de defensa, y le aumenta hasta %s p. la sanación a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Defense, }, { Stats.HealingPower, }, } -- s24160
L["añade de forma permanente %s p. de poder de ataque y un %s% de posibilidad de esquivar a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.AttackPower, }, { Stats.Dodge, }, } -- s24161
L["añade de forma permanente %s p. de poder de ataque a distancia, %s p. de aguante y un %s% de probabilidad de golpear a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.RangedAttackPower, }, { Stats.Stamina, }, { Stats.MeleeHit, Stats.RangedHit, }, } -- s24162
L["añade de forma permanente %s p. de intelecto y le aumenta hasta %s p. la sanación y el daño con hechizos a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Intellect, }, { Stats.SpellPower, }, } -- s24163
L["añade de forma permanente %s p. a todos los hechizos de sanación y daño y un %s% de probabilidad de golpear con hechizos a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.SpellPower, }, { Stats.SpellHit, }, } -- s24164
L["añade de forma permanente %s p. de aguante y le aumenta hasta %s p. el daño y la sanación con hechizos a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.SpellPower, }, } -- s24165
L["añade de forma permanente %s p. de aguante y %s p. de maná cada %s s, y le aumenta hasta %s p. la sanación a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.GenericManaRegen, }, false, { Stats.HealingPower, }, } -- s24167
L["añade de forma permanente %s p. de aguante y %s p. de intelecto, y le aumenta hasta %s p. la sanación a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Intellect, }, { Stats.Stamina, }, { Stats.HealingPower, }, } -- s24168
L["%s attack power"] = { { Stats.GenericAttackPower, }, } -- s24218
L["improves your chance to get a critical strike by %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s24219
L["imbuye a quien lo lanza una bendición de los dioses primigenios. aumenta el poder de ataque a distancia y cuerpo a cuerpo %s p. durante %s s"] = { { Stats.GenericAttackPower, }, false, } -- s24255
L["da una pequeña probabilidad de imbuir al que lo ejerce con una bendición de los dioses primigenios cuando se inflige daño en cuerpo a cuerpo o a distancia. el poder de ataque a distancia y cuerpo a cuerpo aumenta en %s durante %s s"] = { { Stats.GenericAttackPower, }, false, ignoreSum = true } -- s24256
L["aumenta %s p. tu poder de ataque y un %s% tu probabilidad de golpear. el efecto dura %s s"] = { { Stats.GenericAttackPower, }, { Stats.MeleeHit, Stats.RangedHit, }, false, } -- s24352
L["aumenta hasta %s p. la sanación de los hechizos y efectos durante %s s"] = { { Stats.HealingPower, }, false, } -- s24354
L["armas de puño aumentadas %s"] = { { Stats.WeaponSkill, }, } -- s24362
L["regenera %s p. de maná cada %s s durante %s h"] = { { Stats.GenericManaRegen, }, false, false, } -- s24363
L["aumenta %s p. el espíritu del jugador y %s p. su aguante durante %s h. solo puedes beneficiarte del efecto de una poción de zanza a la vez"] = { { Stats.Spirit, }, { Stats.Stamina, }, false, } -- s24382
L["restaura %s p. de maná y aumenta hasta %s p. el daño de tu siguiente hechizo de fuego. el efecto dura %s min"] = { false, { Stats.FireDamage, }, false, } -- s24389
L["aumenta el tiempo entre los ataques de los enemigos cercanos un %s%, ralentiza su velocidad de movimiento un %s% y su velocidad de lanzamiento de hechizos un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, { Stats.SpellHaste, }, false, reduction = true } -- s24415
L["añade de forma permanente hasta %s p. de sanación de hechizos y efectos a un objeto de la casilla de hombros"] = { { Stats.HealingPower, }, } -- s24420
L["añade de forma permanente daño y hasta %s p. de daño y sanación de hechizos y efectos mágicos a un objeto de la casilla de hombros"] = { { Stats.SpellPower, }, } -- s24421
L["añade de forma permanente %s p. de poder de ataque a un objeto de la casilla de hombros"] = { { Stats.AttackPower, Stats.RangedAttackPower, }, } -- s24422
L["golpea a un único enemigo con %s p. de daño y disminuye el poder de ataque cuerpo a cuerpo de todos los enemigos cercanos en %s p. el efecto dura %s s"] = { false, { Stats.AttackPower, }, false, reduction = true } -- s24423
L["restaura %s p. de salud cada %s s. y aumenta tu fuerza %s p. dura %s min"] = { { Stats.HealthRegen, }, false, { Stats.Strength, }, false, } -- s24427
L["aumenta la resistencia a lo arcano en %s p"] = { { Stats.ArcaneResistance, }, } -- s24493
L["aumenta un %s% la probabilidad de golpe crítico de tus hechizos sagrados durante %s s"] = { { Stats.SpellCrit, }, false, } -- s24498
L["armadura aumentada en %s p"] = { { Stats.Armor, }, } -- s24545
L["golpea a un único enemigo con %s p. de daño y disminuye el poder de ataque de todos los enemigos cercanos en %s p. el efecto dura %s s"] = { false, { Stats.AttackPower, }, false, reduction = true } -- s24577
L["aumenta un %s% la probabilidad de golpear con hechizos durante %s s"] = { { Stats.SpellHit, }, false, } -- s24610
L["aumenta la velocidad de ataque de un aliado un %s% durante %s s pero también le inflige %s p. de daño de naturaleza cada %s s"] = { { Stats.MeleeHaste, }, false, false, false, } -- s24672
L["%s p. de poder de ataque solo en las formas felina, de oso y de oso temible"] = { { Stats.FeralAttackPower, }, } -- s24694
L["restaura %s p. de salud durante %s s. debes permanecer sentado mientras comes. si comes durante al menos %s s, estarás bien alimentado y ganarás %s p. de fuerza durante %s min"] = { false, false, false, { Stats.Strength, }, false, ignoreSum = true } -- s24800
L["aumenta la probabilidad del druida de esquivar un %s% durante %s s"] = { { Stats.Dodge, }, false, } -- s24864
L["aumenta un %s% tu probabilidad de golpe crítico con hechizos y ataques cuerpo a cuerpo. dura %s s"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, false, } -- s24865
L["el objetivo es rodeado de insectos, lo que reduce su probabilidad de golpear en un %s% y causa %s p. de daño de naturaleza durante %s s"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, false, reduction = true } -- s24974
L["aumenta hasta %s p. la sanación de los hechizos y efectos mágicos durante %s s"] = { { Stats.HealingPower, }, false, } -- s24998
L["encanta unos guantes de forma permanente para que aumenten hasta %s p. el daño de las sombras"] = { { Stats.ShadowDamage, }, } -- s25073
L["encanta unos guantes de forma permanente para que aumenten hasta %s p. el daño de escarcha"] = { { Stats.FrostDamage, }, } -- s25074
L["encanta unos guantes de forma permanente para que aumenten hasta %s p. el daño de fuego"] = { { Stats.FireDamage, }, } -- s25078
L["encanta unos guantes de forma permanente para aumentar los hechizos de sanación del taumaturgo hasta %s p"] = { { Stats.HealingPower, }, } -- s25079
L["encanta unos guantes de forma permanente para que aumenten %s p. la agilidad"] = { { Stats.Agility, }, } -- s25080
L["encanta una capa de forma permanente para que proporcione %s p. de resistencia al fuego"] = { { Stats.FireResistance, }, } -- s25081
L["encanta una capa de forma permanente que otorga %s p. de resistencia a la naturaleza"] = { { Stats.NatureResistance, }, } -- s25082
L["encanta una capa de forma permanente para que proporcione un %s% de probabilidad de esquivar"] = { { Stats.Dodge, }, } -- s25086
L["aumenta hasta %s p. el daño y la sanación realizados con hechizos y efectos mágicos. también aumenta un %s% la probabilidad de recibir un golpe crítico con hechizos"] = { { Stats.SpellPower, }, { Stats.SpellCrit, }, } -- s25113
L["restaura %s p. de maná cada %s s y aumenta %s p. la sanación realizada"] = { { Stats.GenericManaRegen, }, false, { Stats.HealingPower, }, } -- s25116
L["cuando se aplica al arma objetivo, aumenta hasta %s p. el daño con hechizos. dura %s minutos"] = { { Stats.SpellPower, }, false, } -- s25117
L["cuando se aplica al arma objetivo, restaura %s p. de maná al taumaturgo cada %s s. dura %s minutos"] = { { Stats.GenericManaRegen, }, false, false, } -- s25118
L["cuando se aplica al arma objetivo, aumenta hasta %s p. el daño con hechizos y aumenta un %s% la probabilidad de golpe crítico con hechizos. dura %s minutos"] = { { Stats.SpellPower, }, { Stats.SpellCrit, }, false, } -- s25122
L["cuando se aplica al arma objetivo, restaura %s p. de maná al taumaturgo cada %s s y aumenta hasta %s p. el efecto de los hechizos de sanación. dura %s minutos"] = { { Stats.GenericManaRegen, }, false, { Stats.HealingPower, }, false, } -- s25123
L["inflige el daño normal más %s p. a un enemigo y a sus aliados cercanos. afecta hasta a %s objetivos. los golpes también reducen la armadura %s p"] = { false, false, { Stats.Armor, }, reduction = true } -- s25174
L["restaura %s p. de maná durante %s s. debes permanecer sentado mientras comes"] = { { Stats.GenericManaRegen, }, false, } -- s25701
L["la resistencia al fuego de los objetivos de tus hechizos se reduce %s p"] = { { Stats.FirePenetration, }, } -- s25717
L["la resistencia a la escarcha de los objetivos de tus hechizos se reduce %s p"] = { { Stats.FrostPenetration, }, } -- s25718
L["da la posibilidad de que, al golpear, su resistencia a tus hechizos dañinos se reduzca en %s durante %s s"] = { { Stats.SpellPenetration, }, false, ignoreSum = true } -- s25767
L["otorga la bendición de poderío superior a todos los miembros de la banda o grupo que pertenecen a la misma clase que el objetivo, lo que aumenta %s p. el poder de ataque cuerpo a cuerpo durante %s min. los jugadores solo pueden tener una bendición a la vez por cada paladín"] = { { Stats.AttackPower, }, false, } -- s25782
L["aumenta un %s% la velocidad de ataque de todos los miembros del grupo cercanos y un %s% el daño de ataque"] = { { Stats.MeleeHaste, }, false, } -- s25801
L["todas las estadísticas se reducen %s p.\r\ninflige %s p. de daño de las sombras cada %s s"] = { false, { Stats.AllStats, }, false, reduction = true } -- s25805
L["debilita al objetivo reduciendo el daño que inflige %s p. durante %s min"] = { { Stats.AverageWeaponDamage, }, false, reduction = true } -- s25816
L["el objetivo recibe %s% cuerpo a cuerpo crítico"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s25848
L["aumenta tu poder de ataque a distancia y cuerpo a cuerpo %s p. el efecto dura %s s"] = { { Stats.GenericAttackPower, }, false, } -- s25891
L["%s p. de daño con arma"] = { { Stats.AverageWeaponDamage, }, } -- s25901
L["da la posibilidad de que tus hechizos dañinos aumenten el daño del hechizo y sus efectos en %s p. durante %s s"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s25906
L["las resistencias mágicas de los objetivos de tus hechizos se reducen %s p"] = { { Stats.SpellPenetration, }, } -- s25975
L["reduce un %s% todo el daño que recibe tu mascota, pero aumenta un %s% el tiempo entre sus ataques. dura %s s"] = { false, { Stats.MeleeHaste, }, false, reduction = true } -- s26064
L["aumenta un %s% el daño infligido por todos los miembros del grupo cercanos y un %s% su velocidad de ataque durante %s s"] = { false, { Stats.MeleeHaste, }, false, } -- s26099
L["tus hechizos descarga de relámpagos, cadena de relámpagos y choque tienen un %s% de probabilidad de otorgar hasta %s p. de daño de naturaleza a los hechizos durante %s s"] = { false, { Stats.NatureDamage, }, false, ignoreSum = true } -- s26119
L["aumenta la sanación por hechizos y efectos hasta %s p"] = { { Stats.NatureDamage, }, } -- s26120
L["aumenta hasta %s p. el daño que infligen los hechizos y efectos de naturaleza durante %s s"] = { { Stats.NatureDamage, }, false, } -- s26121
L["aumentar la probabilidad de golpear con hechizos en un %s% durante %s s cuando resisten a uno de tus hechizos"] = { { Stats.SpellHit, }, false, ignoreSum = true } -- s26128
L["aumenta la probabilidad de golpe con hechizos un %s% durante %s s. se acumula hasta %s veces"] = { { Stats.SpellHit, }, false, false, } -- s26129
L["encanta de forma permanente una pieza de armadura de torso para que proporcione %s p. de todas las estadísticas"] = { { Stats.AllStats, }, } -- s27107
L["aumenta hasta %s p. el daño infligido con hechizos y efectos mágicos, y reduce %s p. las resistencias mágicas de los objetivos de tus hechizos durante %s s"] = { { Stats.SpellDamage, }, { Stats.SpellPenetration, }, false, } -- s26166
L["los ataques cuerpo a cuerpo o a distancia otorgan la probabilidad de aplicar un efecto de penetración de armadura sobre ti durante %s s, lo que reduce %s p. la armadura física del objetivo contra tus propios ataques. el efecto de penetración de armadura se puede aplicar hasta %s veces"] = { false, { Stats.ArmorPenetration, }, false, ignoreSum = true } -- s26480
L["inflige un %s% de daño con arma y deja al objetivo desequilibrado reduciendo su habilidad de defensa %s p. durante %s s"] = { false, { Stats.Defense, }, false, reduction = true } -- s26613
L["restaura %s p. de maná por segundo"] = { { Stats.GenericManaRegen, }, } -- s26647
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para añadir %s p. de espíritu"] = { { Stats.Spirit, }, } -- s26743
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para añadir %s p. de agilidad"] = { { Stats.Agility, }, } -- s26792
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para añadir %s p. de intelecto"] = { { Stats.Intellect, }, } -- s26793
L["aumenta la probabilidad de que tu mascota aseste un golpe crítico un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, ignoreSum = true } -- s27043
L["encanta de forma permanente un arma de combate cuerpo a cuerpo para añadir %s p. de fuerza"] = { { Stats.Strength, }, } -- s27093
L["encanta de forma permanente unos guantes para aumentar los hechizos de sanación del taumaturgo hasta %s p"] = { { Stats.HealingPower, }, } -- s27111
L["encanta de forma permanente unos guantes para aumentar %s p. la agilidad"] = { { Stats.Agility, }, } -- s27112
L["encanta de forma permanente unos brazales para que aumenten los efectos de tus hechizos de sanación %s p"] = { { Stats.HealingPower, }, } -- s27116
L["encanta de forma permanente un escudo para que proporcione una probabilidad de bloquear de %s%"] = { { Stats.BlockChance, }, } -- s27117
L["encanta de forma permanente una capa que aumenta %s p. de resistencia a la naturaleza"] = { { Stats.NatureResistance, }, } -- s27119
L["encanta de forma permanente una capa para que proporciona un %s% de probabilidad de esquivar"] = { { Stats.Dodge, }, } -- s27122
L["los ataques cuerpo a cuerpo tienen una probabilidad de aumentar hasta %s p. el daño y las sanaciones que realizas con efectos y hechizos mágicos durante %s s"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s27498
L["crea una violenta tormenta en el área objetivo que inflige %s p. de daño de naturaleza a los enemigos cada %s s y aumentando el tiempo entre los ataques de los enemigos un %s%. dura %s s. el druida debe canalizar su magia para mantener el hechizo"] = { false, false, { Stats.MeleeHaste, }, false, reduction = true } -- s27530
L["el guerrero grita aumentando %s p. el poder de ataque de los miembros del grupo en un radio de %s m. dura %s min"] = { { Stats.AttackPower, }, false, false, } -- s27578
L["reduce el poder de ataque de todos los enemigos que estén en un radio de %s m %s p. durante %s s"] = { false, { Stats.GenericAttackPower, }, false, reduction = true } -- s27579
L["aumenta todas las resistencias mágicas %s p. durante %s min"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, } -- s27652
L["aumenta la probabilidad de esquivar un %s% durante %s min"] = { { Stats.Dodge, }, false, } -- s27653
L["aumenta hasta %s p. tu daño con hechizos y hasta %s p. tu sanación durante %s s"] = { { Stats.SpellDamage, }, { Stats.HealingPower, }, false, } -- s27675
L["infunde poder al grupo del objetivo y aumenta su espíritu en %s p. durante %s h"] = { { Stats.Spirit, }, false, } -- s27681
L["aumenta %s p. la defensa durante %s h"] = { { Stats.Defense, }, false, } -- s27720
L["aumenta hasta %s p. el daño de los hechizos durante %s h"] = { { Stats.SpellDamage, }, false, } -- s27721
L["aumenta la sanación aportada por los hechizos mágicos hasta en %s p. durante %s h"] = { { Stats.HealingPower, }, false, } -- s27722
L["mejora un %s% tu probabilidad de golpear durante %s h"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, } -- s27723
L["el lanzamiento de hechizos tiene la probabilidad de aumentar hasta %s p. el daño y las sanaciones que realizas durante %s s"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s27774
L["encanta de forma permanente un arma de combate cuerpo a cuerpo de dos manos para que otorgue %s p. de agilidad"] = { { Stats.Agility, }, } -- s27837
L["aumenta un %s% la probabilidad de golpe crítico con hechizos de todos los miembros del grupo en un radio de %s m"] = { { Stats.SpellCrit, }, false, } -- s28142
L["aumenta hasta %s p. el daño y la sanación realizados con hechizos y efectos mágicos sobre los miembros de grupo en un radio de %s m"] = { { Stats.SpellPower, }, false, } -- s28143
L["aumenta hasta %s p. la sanación realizada con hechizos y efectos mágicos sobre los miembros de grupo en un radio de %s m"] = { { Stats.HealingPower, }, false, } -- s28144
L["restaura %s p. de maná cada %s s a todos los miembros del grupo en un radio de %s m"] = { { Stats.GenericManaRegen, }, false, false, } -- s28145
L["aumenta hasta %s p. tu daño con hechizos y hasta %s p. tu sanación"] = { { Stats.SpellDamage, }, { Stats.HealingPower, }, } -- s28155
L["añade de forma permanente %s p. de resistencia a la naturaleza a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.NatureResistance, }, } -- s28161
L["añade de forma permanente %s p. de resistencia a la escarcha a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.FrostResistance, }, } -- s28163
L["añade de forma permanente %s p. de resistencia a las sombras a un objeto de la casilla para pierna o cabeza. no se suma a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.ShadowResistance, }, } -- s28165
L["tus próximos %s hechizos de daño o sanación lanzados en %s s otorgarán un bonus de hasta %s p. de daño y %s p. de sanación, que se acumula hasta %s veces. expira después de %s hechizos de daño o sanación o %s s, lo que ocurra primero"] = { false, false, { Stats.SpellDamage, }, { Stats.HealingPower, }, false, false, false, ignoreSum = true } -- s28200
L["aumenta el valor de bloqueo de tu escudo en %s p"] = { { Stats.BlockValue, }, } -- s28331
L["aumenta el poder de ataque a distancia y cuerpo a cuerpo en %s p"] = { { Stats.GenericAttackPower, }, } -- s28347
L["se sacrifica para impedir y eliminar efectos de enfurecer. silencia los hechizos de naturaleza durante %s s. reduce un %s% la velocidad de lanzamiento"] = { false, { Stats.SpellHaste, }, reduction = true } -- s28732
L["aumenta la salud de un objetivo amistoso %s p. durante %s s"] = { { Stats.Health, }, false, } -- s28750
L["aumenta la resistencia al fuego %s p. dura %s s"] = { { Stats.FireResistance, }, false, } -- s28765
L["aumenta %s p. la resistencia a la escarcha. dura %s s"] = { { Stats.FrostResistance, }, false, } -- s28766
L["aumenta la resistencia a la naturaleza %s p. dura %s s"] = { { Stats.NatureResistance, }, false, } -- s28768
L["aumenta la resistencia a las sombras %s p. dura %s s"] = { { Stats.ShadowResistance, }, false, } -- s28769
L["aumenta la resistencia a lo arcano %s p. dura %s s"] = { { Stats.ArcaneResistance, }, false, } -- s28770
L["aumenta el valor de bloqueo de tu escudo %s p. durante %s s"] = { { Stats.BlockValue, }, false, } -- s28773
L["aumenta la resistencia a todos los tipos de magia %s p. durante %s s"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, } -- s28778
L["aumenta hasta %s p. la sanación de los siguientes %s hechizos durante %s s"] = { { Stats.HealingPower, }, false, false, } -- s28780
L["aumenta %s p. la armadura de un objetivo amistoso durante %s s"] = { { Stats.Armor, }, false, } -- s28790
L["aumenta %s p. el poder de ataque de un objetivo amistoso durante %s s"] = { { Stats.GenericAttackPower, }, false, } -- s28791
L["aumenta hasta %s p. el daño con hechizos y la sanación del objetivo aliado durante %s s"] = { { Stats.SpellDamage, }, false, } -- s28793
L["aumenta %s p. cada %s s la regeneración de maná de un objetivo amistoso durante %s s"] = { { Stats.GenericManaRegen, }, false, false, } -- s28795
L["cada hechizo que lanzas puede activar una epifanía que aumenta %s p. la regeneración de maná durante %s s"] = { { Stats.GenericManaRegen, }, false, ignoreSum = true } -- s28802
L["aumenta %s p. la regeneración de maná de un objetivo aliado cada %s s durante %s s"] = { { Stats.GenericManaRegen, }, false, false, } -- s28804
L["tu hechizo escudo de relámpagos también te otorga %s p. de maná cada %s s mientras está activo"] = { { Stats.GenericManaRegen, }, false, ignoreSum = true } -- s28821
L["mejora un %s% tu probabilidad de obtener un golpe crítico con hechizos contra los no-muertos"] = { { Stats.SpellCrit, }, } -- s29069
L["aumenta %s p. la resistencia al fuego"] = { { Stats.FireResistance, }, } -- s29175
L["los golpes críticos de tus hechizos ofensivos aumentarán un %s% tu probabilidad de lograr un golpe crítico con ataques cuerpo a cuerpo durante %s s"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, false, ignoreSum = true } -- s29179
L["aumenta el tiempo entre los ataques de un enemigo un %s% y reduce su velocidad de movimiento un %s%. además reduce su fuerza un %s%. dura %s s"] = { { Stats.MeleeHaste, }, false, false, false, reduction = true } -- s29212
L["aumenta el aguante %s p. durante %s h"] = { { Stats.Stamina, }, false, } -- s29235
L["mejora tu probabilidad de golpear en un %s% durante %s h"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, } -- s29331
L["aumenta hasta %s p. la sanación de hechizos mágicos durante %s h"] = { { Stats.HealingPower, }, false, } -- s29334
L["aumenta %s p. todas las resistencias y provoca que todos los hechizos que resistas por completo restablezcan el %s% de tu maná total. tiempo de reutilización: %s segundo"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, false, } -- s29441
L["aumenta todas las resistencias en %s p. y provoca que todos los hechizos que resistas por completo restauren el %s% del total de tu maná. %s segundo de tiempo de reutilización"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, false, } -- s29445
L["añade de forma permanente daño y hasta %s p. de daño y sanación de hechizos y efectos mágicos a un objeto de la casilla de hombros, y aumenta un %s% tu probabilidad de asestar un golpe crítico con hechizos"] = { { Stats.SpellPower, }, { Stats.SpellCrit, }, } -- s29467
L["añade de forma permanente hasta %s p. de sanación de hechizos y efectos mágicos a un objeto de la casilla de hombros y aumenta %s p. de maná cada %s s tu regeneración de maná"] = { { Stats.HealingPower, }, { Stats.GenericManaRegen, }, false, } -- s29475
L["añade de forma permanente %s p. de aguante y %s p. de armadura a un objeto de la casilla de hombros"] = { { Stats.Stamina, }, { Stats.Armor, }, } -- s29480
L["añade de forma permanente %s p. de poder de ataque a un objeto de la casilla de hombros y aumenta un %s% tu probabilidad de asestar un golpe crítico"] = { { Stats.GenericAttackPower, Stats.AttackPower, Stats.RangedAttackPower, }, { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- s1219512
L["aumenta el poder de ataque en %s p. y %s p. extra cada %s s. dura %s s"] = { { Stats.GenericAttackPower, }, { Stats.GenericAttackPower, }, false, false, ignoreSum = true } -- s29602
L["aumenta tu probabilidad de golpear con doble empuñadura en un %s% extra"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- s30821
L["enferma a un enemigo durante %s min, aumentando el daño físico que recibe %s p. y reduce su aguante %s p. el objetivo enfermo tiene un %s% de probabilidad de propagar su enfermedad a uno de sus aliados cercanos al ser golpeado en combate"] = { false, false, { Stats.Stamina, }, false, reduction = true } -- s30081
L["aumenta el aguante durante %s hora"] = { { Stats.Stamina, }, } -- s30088
L["aumenta el aguante %s p. cuando se consume"] = { { Stats.Stamina, }, } -- s30164
L["aumenta la agilidad %s p. cuando se consume"] = { { Stats.Agility, }, } -- s30173
L["aumenta el intelecto %s p. cuando se consume"] = { { Stats.Intellect, }, } -- s30175
L["aumenta el espíritu %s p. cuando se consume"] = { { Stats.Spirit, }, } -- s30177
L["aumenta la fuerza %s p. cuando se consume"] = { { Stats.Strength, }, } -- s30178
L["encanta de forma permanente un arma de combate cuerpo a cuerpo de %s manos y añade %s p. de agilidad"] = { false, { Stats.Agility, }, } -- s30229
L["reduce tu probabilidad de obtener un golpe crítico un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, reduction = true } -- s30297
L["aumenta el daño y la sanación de los hechizos mágicos y los efectos hasta en %s p"] = { { Stats.SpellPower, }, } -- s30776
L["regeneras un %s% de agn como salud cada %s s, incluso en combate"] = { { Stats.HealthRegen, }, false, } -- s30799
L["aumenta tu probabilidad de golpear con doble empuñadura un %s% extra"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- s30816
L["aumenta %s p. tu habilidad con espadas, armas de puño y dagas"] = { { Stats.WeaponSkill, }, } -- s30919
L["aumenta los puntos de salud en %s. %s% de celeridad por ataques cuerpo a cuerpo. %s p. de regeneración de maná cada %s s"] = { { Stats.Health, }, { Stats.MeleeHaste, }, { Stats.GenericManaRegen, }, false, } -- s355366
L["aumenta un %s% la velocidad de ataque del taumaturgo y el daño físico que inflige en %s p. durante %s min"] = { { Stats.MeleeHaste, }, false, false, } -- s365122
L["inflige %s p. de daño de las sombras cada %s s.\r\nreduce un %s% tu probabilidad de golpear con hechizos y ataques cuerpo a cuerpo"] = { false, false, { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, reduction = true } -- s367873
L["aumenta la velocidad de ataque del taumaturgo un %s% y todo el daño que inflige aumenta %s p. durante %s s"] = { { Stats.MeleeHaste, }, false, false, } -- s368388
L["inflige %s p. de daño de naturaleza cada %s s y también aumenta un %s% la probabilidad de golpe crítico con un ataque cuerpo a cuerpo, a distancia o con hechizos durante %s s"] = { false, false, { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, false, } -- s370337
L["la salud aumenta un %s% y el daño aumenta un %s%"] = { false, { Stats.SpellDamage, }, } -- s370444
L["te clavas el colmillo, lo que aumenta un %s% la velocidad de movimiento, ataque y lanzamiento durante %s h"] = { { Stats.SpellHaste, Stats.MeleeHaste, Stats.RangedHaste, }, false, } -- s370832
L["remate que aumenta la probabilidad de parar. dura más y aumenta la probabilidad de parar por cada punto de combo:\r\n   %s p.  : %s segundos, %s% de parar\r\n   %s p.: %s segundos, %s% de parar\r\n   %s p.: %s segundos, %s% de parar\r\n   %s p.: %s segundos, %s% de parar\r\n   %s p.: %s segundos, %s% de parar"] = { false, { Stats.Parry, }, false, false, false, false, false, false, false, false, false, false, false, false, false, } -- s398198
L["aumenta un %s% la probabilidad de golpe crítico a un yelmo durante %s minutos"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, false, } -- s399699
L["remate que aumenta un %s% tu probabilidad de parar y otorga %s p. de poder de ataque por cada punto de tu habilidad de defensa por encima de %s p. dura más por cada punto de combo:\r\n\r\n   %s punto: %s s\r\n   %s puntos: %s s\r\n   %s puntos: %s s\r\n   %s puntos: %s s\r\n   %s puntos: %s s"] = { { Stats.Parry, }, false, false, false, false, false, false, false, false, false, false, false, false, } -- s400012
L["maldice al objetivo durante %s min, lo que reduce su resistencia a las sombras y a lo arcano un %s% y aumenta el daño de las sombras y arcano recibido un %s%. un brujo solo puede tener una maldición activa a la vez sobre un mismo objetivo"] = { false, { Stats.ShadowResistance, Stats.ArcaneResistance, }, false, reduction = true } -- s402791
L["maldice al objetivo durante %s min, lo que reduce su resistencia al fuego y a la escarcha %s p. y aumenta el daño recibido de fuego y escarcha un %s%. un brujo solo puede tener una maldición activa a la vez sobre un mismo objetivo"] = { false, { Stats.FireResistance, Stats.FrostResistance, }, false, reduction = true } -- s402792
L["obliga al objetivo a hablar en demoníaco, lo que ralentiza el lanzamiento de todos los hechizos un %s%. solo puede haber una maldición por brujo activa en un objetivo. dura %s min"] = { { Stats.SpellHaste, }, false, reduction = true } -- s402794
L["aumenta el tiempo entre los ataques de un enemigo un %s%. dura %s s"] = { { Stats.MeleeHaste, }, false, reduction = true } -- s402808
L["reduce el poder de ataque del objetivo en %s p. dura %s s"] = { { Stats.GenericAttackPower, }, false, reduction = true } -- s402811
L["reduce la armadura de un enemigo en %s p. dura %s s"] = { { Stats.Armor, }, false, reduction = true } -- s402818
L["el guerrero grita y aumenta el aguante de todos los miembros del grupo en un radio de %s m en %s p. dura %s min"] = { false, { Stats.Stamina, }, false, } -- s403215
L["rodea al taumaturgo con energía vil que aumenta %s p. el daño con hechizos y la sanación, más otro monto de daño con hechizos y sanación equivalente al %s% de tu espíritu. además, recuperas un %s% de tu salud máxima cada %s s. un brujo solo puede tener un tipo de hechizo de armadura activo a la vez. dura %s min"] = { { Stats.SpellPower, }, false, false, false, false, } -- s403619
L["aumenta el daño físico infligido por el taumaturgo en %s p. y acelera sus ataques un %s% durante %s s"] = { false, { Stats.MeleeHaste, }, false, } -- s407791
L["cada vez que bloqueas, regeneras una cantidad de maná equivalente al %s% de tu maná máximo y obtienes una cantidad de armadura equivalente al %s% del valor de armadura de tu escudo, que se acumula hasta %s veces. también aumenta un %s% la probabilidad de bloqueo y un %s% el valor de bloqueo.\r\n\r\nademás, cada vez que usas una habilidad de choque, obtienes %s p. de daño con hechizos por cada punto de tu habilidad de defensa por encima de %s. dura %s s"] = { false, { Stats.Armor, }, false, { Stats.BlockChance, }, false, false, false, false, } -- s408524
L["aumenta la armadura un %s% del valor de armadura de tu escudo durante %s s, se acumula hasta %s veces"] = { { Stats.Armor, }, false, false, } -- s408525
L["tus golpes con emboscada y puñalada desequilibran a tu objetivo, lo que aumenta el tiempo entre sus ataques cuerpo a cuerpo un %s% y reduce su velocidad de movimiento un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, false, reduction = true } -- s408699
L["aumenta un %s% tu probabilidad de golpe crítico y reduce un %s% el costo de todas tus habilidades de disparo"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, false, } -- s409428
L["tus golpes críticos cuerpo a cuerpo y a distancia aumentan tu poder de ataque un monto equivalente al %s% de tu agilidad actual durante %s s"] = { { Stats.GenericAttackPower, }, false, ignoreSum = true } -- s409504
L["tus golpes críticos a distancia aumentan tu poder de ataque un monto equivalente al %s% de tu agilidad durante %s s"] = { { Stats.GenericAttackPower, }, false, } -- s409507
L["aumenta tu probabilidad de golpe crítico con todos los hechizos un %s%, pero tus golpes críticos con hechizos que no son periódicos ahora tienen un costo de maná adicional de un %s% de tu maná base"] = { { Stats.SpellCrit, }, false, } -- s412286
L["aumenta un %s% tu probabilidad de esquivar y un %s% la de tu mascota. además, reduce un %s% el tiempo de reutilización de tu habilidad golpe lateral"] = { { Stats.Dodge, }, false, false, } -- s415428
L["aumenta un %s% tu probabilidad de esquivar y regeneras %s p. de ira cada vez que esquivas mientras estás en forma de oso o forma de oso temible, %s p. de energía mientras estás en forma felina o un %s% de tu maná máximo mientras estás en otra forma"] = { { Stats.Dodge, }, false, false, false, } -- s417051
L["lanza al objetivo veneno corrosivo ancestral que inflige %s p. de daño de naturaleza cada %s s y reduce la armadura del objetivo %s p. durante %s s"] = { false, false, { Stats.Armor, }, false, reduction = true } -- s422996
L["atacas instantáneamente con tu arma de la mano izquierda para infligir daño con arma normal, y tu probabilidad de parar un ataque aumenta un %s% durante %s s o hasta que paras un ataque de manera exitosa. durante %s s, después de usar daga de guardamano, golpe siniestro cuesta %s p. menos de energía, genera un %s% más de amenaza, otorga acumulaciones de bravura ante los golpes y tanto golpe siniestro como eviscerar infligen un %s% más de daño. otorga %s p. de combo.\r\n\r\ndaga de guardamano se beneficia de la mayoría de los talentos y efectos que se activan con golpe siniestro o lo modifican. el efecto de daga de guardamano no debilitará ni potenciará daga de guardamano"] = { { Stats.Parry, }, false, false, false, false, false, false, } -- s424919
L["acelera tu lanzamiento de hechizos, lo que aumenta la velocidad con la que lanzas hechizos un %s% y reduce un %s% el retraso sufrido por los ataques dañinos mientras lanzas hechizos. dura %s s"] = { { Stats.SpellHaste, }, false, false, } -- s425121
L["te sobrecargas con energía vil, lo que aumenta un %s% tu probabilidad y la de tu mascota de esquivar y un %s% tu probabilidad de asestar golpes críticos con todos tus ataques. dura %s s"] = { { Stats.Dodge, }, false, false, } -- s425463
L["aumenta %s m el alcance de tu sentencia y un %s% la probabilidad de golpe con hechizos"] = { false, { Stats.SpellHit, }, } -- s426173
L["cuando se activa, esta habilidad aumenta temporalmente un %s% tu salud máxima durante %s s. después de que el efecto expira, se pierde la salud. además, mientras estás en metamorfosis, venganza hace que tus alas reduzcan tu velocidad de caída"] = { { Stats.Health, }, false, } -- s426195
L["emite un berrido de convocación, que otorga un %s% más de salud máxima a los miembros del grupo y banda en un radio de %s m durante %s s"] = { { Stats.Health, }, false, false, } -- s426490
L["tu habilidad conflagrar también otorga un %s% de celeridad de lanzamiento de hechizos durante %s s y ya no consume inmolar"] = { { Stats.SpellHaste, }, false, ignoreSum = true } -- s427713
L["consume todas las aplicaciones de frenesí de tu mascota, lo que aumenta un %s% tu velocidad de ataque a distancia y otorga %s p. de enfoque a tu mascota por cada aplicación de frenesí consumida. dura %s s.\r\n\r\ntu mascota obtiene frenesí cada vez que usa un ataque básico, lo que aumenta un %s% su velocidad de ataque cuerpo a cuerpo durante %s s. se acumula un máximo de %s veces"] = { { Stats.RangedHaste, }, false, false, { Stats.MeleeHaste, }, false, false, } -- s428726
L["aumenta un %s% la velocidad de ataque cuerpo a cuerpo de tu mascota durante %s s"] = { { Stats.MeleeHaste, }, false, } -- s428728
L["inflige %s p. de daño de fuego al recibir un golpe, aumenta un %s% tu probabilidad de golpe crítico con hechizos y reduce un %s% la probabilidad de recibir un golpe crítico. solo puede haber un hechizo de armadura activo sobre el mago a la vez. dura %s min"] = { false, { Stats.SpellCrit, }, false, false, } -- s428741
L["aumenta un %s% tu probabilidad de golpe crítico con hechizos sagrados. además, tus hechizos de sanación crítica sanan al objetivo un %s% de la cantidad sanada durante %s s"] = { { Stats.SpellCrit, }, false, false, } -- s429142
L["controla a un enemigo humanoide de nivel %s o inferior durante %s s, pero aumenta el tiempo entre sus ataques un %s%"] = { false, false, { Stats.MeleeHaste, }, reduction = true } -- s429688
L["aumenta tu probabilidad de golpear un %s%"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- s429863
L["aumenta tu fuerza %s p"] = { { Stats.Strength, }, } -- s429864
L["desciende hacia la locura, aumenta un %s% la velocidad de ataque y la amenaza que generas durante %s s"] = { { Stats.MeleeHaste, Stats.RangedHaste, }, false, } -- s429867
L["una hoja encantada aumenta tu probabilidad de golpear un %s%. no tiene efecto fuera de las cavernas de brazanegra"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- s430391
L["cuando se aplica al arma objetivo, aumenta la probabilidad de golpe un %s% durante %s minutos. no tiene efecto fuera de las cavernas de brazanegra"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, } -- s430392
L["restaura %s p. de maná cada %s s y aumenta un %s% la probabilidad de golpear con hechizos. este efecto solo se aplica dentro de las cavernas de brazanegra"] = { { Stats.SpellHit, }, false, false, } -- s430406
L["cuando se aplica al arma objetivo, restaura %s p. de maná cada %s segundos y aumenta el golpe con hechizos un %s% durante %s min. no tiene efecto fuera de las cavernas de brazanegra"] = { { Stats.GenericManaRegen, }, false, { Stats.SpellHit, }, false, } -- s430585
L["aumenta un %s% la probabilidad de golpe de hechizos, aumenta un %s% la probabilidad de golpe crítico de ataques cuerpo a cuerpo y a distancia, otorga %s p. de poder de ataque y %s p. de daño con hechizos, y aumenta un %s% la velocidad de movimiento. dura %s minutos"] = { { Stats.SpellHit, }, { Stats.MeleeCrit, Stats.RangedCrit, }, { Stats.GenericAttackPower, }, { Stats.SpellDamage, }, false, false, } -- s430947
L["restaura %s p.de maná cada %s s"] = { { Stats.GenericManaRegen, }, false, } -- s430948
L["aumenta un %s% tu probabilidad de golpear con hechizos"] = { { Stats.SpellHit, }, } -- s430949
L["aumenta %s p. tu resistencia al fuego durante %s min"] = { { Stats.FireResistance, }, false, } -- s430950
L["aumenta %s p. tu resistencia a la escarcha durante %s min"] = { { Stats.FrostResistance, }, false, } -- s430951
L["aumenta un %s% tu probabilidad de asestar golpes críticos con hechizos"] = { { Stats.SpellCrit, }, } -- s430952
L["cuando usas armas de %s manos, tu velocidad de ataque aumenta un %s%"] = { false, { Stats.MeleeHaste, }, } -- s431046
L["encanta un objeto de casilla de hombros permanentemente para aumentar un %s% tu velocidad de ataque"] = { { Stats.MeleeHaste, }, } -- s432137
L["mejora un %s% tu probabilidad de golpear con todos tus hechizos y ataques"] = { { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, } -- s432639
L["perfora la armadura del objetivo y la reduce %s p"] = { { Stats.Armor, }, reduction = true } -- s434837
L["descarga una explosión de energía arcana que inflige %s p. de daño arcano, reduce las resistencias %s p. y aumenta todo el daño con hechizos recibido un %s% durante %s s. solo afecta a los enemigos de niveles inferiores a %s"] = { false, { Stats.FireResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, Stats.NatureResistance, }, false, false, false, reduction = true } -- s434841
L["aumenta en %s p. el daño físico que inflige el taumaturgo y acelera un %s% sus ataques durante %s s"] = { false, { Stats.MeleeHaste, }, false, } -- s434907
L["aumenta %s p. la armadura, pero la velocidad de movimiento se reduce un %s%. el efecto no se puede eliminar y dura %s s"] = { { Stats.Armor, }, false, false, } -- s435359
L["restaura %s p. de maná y %s p. de salud. también aumenta %s p. el poder de ataque y %s p. el daño que infligen todos los hechizos durante %s s"] = { false, false, { Stats.GenericAttackPower, }, { Stats.SpellDamage, }, false, } -- s435973
L["el lanzamiento de hechizos tiene la probabilidad de aumentar hasta %s p. el daño y la sanación que realizas durante %s s"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s435977
L["mejora un %s% tu probabilidad de obtener un golpe crítico con todos tus hechizos y ataques"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- s436239
L["aumenta las armas de puño en %s p"] = { { Stats.WeaponSkill, }, } -- s436285
L["tu mascota obtiene %s p. de aguante y %s p. de intelecto"] = { { Stats.Stamina, }, { Stats.Intellect, }, ignoreSum = true } -- s436375
L["probabilidad de aumentar la fuerza %s p. durante %s s al impactar"] = { { Stats.Strength, }, false, ignoreSum = true } -- s436469
L["aumenta un $%s% la probabilidad de que tu mascota aseste un golpe crítico"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s436689
L["aumenta un %s% la probabilidad de que tu mascota aseste un golpe crítico"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, ignoreSum = true } -- s436690
L["controlas la energía arcana latente del aire a tu alrededor para obtener inspiración cargada, que reduce un %s% el costo de maná de todos los hechizos, y aumenta hasta %s p. el daño y la sanación realizados durante %s s"] = { false, { Stats.SpellPower, }, false, } -- s437327
L["causa un choque neurológico que te inflige %s p. de daño al instante, pero también aumenta tu velocidad de ataque cuerpo a cuerpo y a distancia un %s% durante %s s"] = { false, { Stats.MeleeHaste, Stats.RangedHaste, }, false, } -- s437349
L["obtienes meditación neuromántica, que te permite conservar un %s% de tu regeneración de maná en el lanzamiento de hechizos, y aumenta el daño y la sanación realizados con hechizos hasta %s p. durante %s s"] = { false, { Stats.SpellPower, }, false, } -- s437357
L["causa un choque hiperconductivo que te inflige %s p. de daño al instante, pero también aumenta tu velocidad de lanzamiento de hechizos un %s% durante %s s"] = { false, { Stats.SpellHaste, }, false, } -- s437362
L["obtienes concentración intensa, que reduce la velocidad de movimiento un %s%, pero también aumenta %s p. tu resistencia contra hechizos de todas las escuelas y aumenta la velocidad de ataque cuerpo a cuerpo un %s% durante %s s"] = { { Stats.MeleeHaste, }, { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, { Stats.MeleeHaste, }, false, } -- s437377
L["el taumaturgo escupe un veneno aletargante a los jugadores que inflige %s p. de daño de naturaleza cada %s s durante %s s. este veneno reduce la probabilidad de golpe un %s% y aumenta los costos de habilidades un %s%"] = { false, false, false, { Stats.MeleeHit, Stats.RangedHit, }, false, reduction = true } -- s437390
L["entrar en contacto con la luz hace que el terror estridente entre en un frenesí, lo que aumenta las velocidades de ataque y movimiento un %s$%"] = { { Stats.MeleeHaste, }, } -- s437813
L["aumenta un %s% la probabilidad de golpe crítico con hechizos, otorga %s p. de daño y sanación con hechizos y aumenta un %s% la velocidad de ataque cuerpo a cuerpo y a distancia. dura %s minutos"] = { { Stats.SpellCrit, }, { Stats.SpellPower, }, { Stats.MeleeHaste, Stats.RangedHaste, }, false, } -- s438536
L["otorga un sigilo encantado de innovación que te potencia para aumentar hasta %s p. el daño y la sanación realizados con hechizos, y aumenta el poder de ataque %s p. durante %s min. esto solo aplica fuera de combate"] = { { Stats.SpellPower, }, { Stats.GenericAttackPower, }, false, } -- s439155
L["aumenta el daño con hechizos un máximo de %s p. durante %s min"] = { { Stats.SpellDamage, }, false, } -- s439959
L["daño con arma %s"] = { { Stats.AverageWeaponDamage, }, } -- s440981
L["aumenta la probabilidad de golpear con ataques cuerpo a cuerpo un %s% y reduce la probabilidad de que los enemigos esquiven tus ataques cuerpo a cuerpo un %s% mientras estás en forma de oso, forma felina o forma de oso temible. no se acumula con otras runas de anillo"] = { { Stats.MeleeHit, Stats.RangedHit, }, false, } -- s442901
L["obliga a los enemigos cercanos a hablar en demoníaco, lo que reduce su velocidad de lanzamiento un %s% durante %s s"] = { { Stats.SpellHaste, }, false, reduction = true } -- s444046
L["aumenta %s p. el poder de ataque, el daño con hechizos y la sanación"] = { { Stats.SpellPower, Stats.AttackPower, }, } -- s446219
L["choque de llamas otorga %s p. de poder de ataque, daño con hechizos y sanación durante %s s"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s446220
L["aumenta el poder de ataque a distancia y cuerpo a cuerpo %s p. los beneficios de este frasco solo se aplican en áreas que están bajo la influencia de la pesadilla"] = { { Stats.GenericAttackPower, }, } -- s446228
L["infunde al taumaturgo con una bendición de los dioses primigenios. el poder de ataque cuerpo a cuerpo y a distancia aumenta %s p. durante %s s"] = { { Stats.GenericAttackPower, }, false, } -- s446231
L["otorga una pequeña probabilidad de infundir al portador con una bendición de la serpiente cuando inflige daño cuerpo a cuerpo o a distancia. el poder de ataque cuerpo a cuerpo y a distancia aumenta %s p. durante %s s"] = { { Stats.GenericAttackPower, }, false, ignoreSum = true } -- s446233
L["otorga un sigilo encantado de sueños vivos que te potencia para aumentar hasta %s p. el daño y la sanación realizados con hechizos, y aumenta el poder de ataque %s p. durante %s min. esto solo aplica fuera de combate"] = { { Stats.SpellPower, }, { Stats.GenericAttackPower, }, false, } -- s446240
L["aumenta %s p. el poder con hechizos. los golpes con hechizos exitosos tienen una probabilidad de liberar una descarga de las sombras contra el objetivo. solo puede haber un tipo de poción atal'ai activa a la vez. esto no tiene efecto fuera del templo sumergido"] = { { Stats.SpellDamage, }, } -- s446256
L["inspirado por el mojo de guerra, tu tamaño aumenta y tu fuerza sube %s p. para corresponderse con tu tamaño nuevo. dura %s s"] = { { Stats.Strength, }, false, } -- s446335
L["aumenta el poder de ataque cuerpo a cuerpo y a distancia %s p. con cada ataque cuerpo a cuerpo o a distancia, hay una probabilidad de aumentar de tamaño, lo que aumenta la fuerza o la agilidad (el mayor de los dos) %s p. durante %s s. solo puede haber un tipo de poción atal'ai activa a la vez en cualquier momento. esto no tiene efecto fuera del templo sumergido"] = { { Stats.GenericAttackPower, }, { Stats.Strength, }, false, } -- s446336
L["reduce %s p. todos los atributos de los enemigos cercanos durante %s min"] = { { Stats.AllStats, }, false, reduction = true } -- s446364
L["aumenta %s p. toda la sanación realizada y %s p. la regeneración de maná durante %s s. tus sanaciones tienen una probabilidad de restaurar %s p. de energía, un %s% de maná o %s p. de ira. solo puede haber una poción atal'ai activa a la vez. esto no tiene efecto fuera del templo sumergido"] = { { Stats.HealingPower, }, { Stats.GenericManaRegen, }, false, false, false, false, } -- s446396
L["%s p. de poder de ataque"] = { { Stats.GenericAttackPower, }, } -- s446450
L["añade de forma permanente %s p. de daño y sanación de hechizos y efectos mágicos a un objeto de la casilla de hombros"] = { { Stats.SpellPower, }, } -- s446459
L["los ecos del vacío hacen que te sumerjas en la locura, lo que aumenta tu velocidad de lanzamiento de hechizos un %s%. no tiene efecto fuera de las áreas bajo la influencia de la pesadilla"] = { { Stats.SpellHaste, }, } -- s446528
L["los ecos del vacío hacen que te vuelvas demente, lo que aumenta tu sanación hasta un máximo de %s p. no tiene efecto fuera de las áreas bajo la influencia de la pesadilla"] = { { Stats.HealingPower, }, } -- s446541
L["los hechizos que infligen daño tienen una probabilidad de aumentar %s p. tu daño con hechizos y un %s% la probabilidad de esquivar durante %s s. no tiene efecto fuera de las áreas bajo la influencia de la pesadilla"] = { { Stats.SpellDamage, }, { Stats.Dodge, }, false, ignoreSum = true } -- s446570
L["los ecos del vacío corrompen tus pensamientos, lo que aumenta %s p. tu daño con hechizos y un %s% la probabilidad de esquivar. no tiene efecto fuera de las áreas bajo la influencia de la pesadilla"] = { { Stats.SpellDamage, }, { Stats.Dodge, }, } -- s446572
L["tus ataques cuerpo a cuerpo tienen una probabilidad de desatar ecos del vacío, lo que te llena de pavor. tu velocidad de ataque aumenta un %s% y tu poder de ataque %s p. no tiene efecto fuera de las áreas bajo la influencia de la pesadilla"] = { { Stats.MeleeHaste, }, { Stats.AttackPower, }, } -- s446577
L["tus ataques cuerpo a cuerpo tienen una probabilidad de aumentar tu velocidad de ataque un %s% y tu poder de ataque %s p. durante %s s. no tiene efecto fuera de las áreas bajo la influencia de la pesadilla"] = { { Stats.MeleeHaste, }, { Stats.AttackPower, }, false, ignoreSum = true } -- s446579
L["tus ataques cuerpo a cuerpo y hechizos dañinos tienen una probabilidad de aumentar el daño con hechizos hasta un máximo de %s p. durante %s s. no tiene efecto fuera de las áreas bajo la influencia de la pesadilla"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s446592
L["tus ataques cuerpo a cuerpo y hechizos dañinos tienen una probabilidad de desatar ecos del vacío, lo que te infunde miedo y aumenta %s p. el daño de tus hechizos. no tiene efecto fuera de las áreas bajo la influencia de la pesadilla"] = { { Stats.SpellDamage, }, } -- s446597
L["los ataques cuerpo a cuerpo tienen una probabilidad de aumentar %s p. tu poder de ataque y un %s% la probabilidad de esquivar. no tiene efecto fuera de las áreas bajo la influencia de la pesadilla"] = { { Stats.AttackPower, }, { Stats.Dodge, }, ignoreSum = true } -- s446628
L["aumenta %s p. el daño o sanación de tu próximo ataque, hechizo o habilidad"] = { { Stats.SpellPower, }, } -- s446687
L["cuando matas a un objetivo que otorga experiencia u honor, aumenta %s p. el daño o sanación de tu próximo ataque, hechizo o habilidad. este efecto no puede ocurrir más de una vez cada %s s"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s446689
L["aumenta la probabilidad de golpe crítico un %s%, el daño de hechizos en %s y las estadísticas un %s% por %s min"] = { false, { Stats.SpellDamage, }, false, false, } -- s446695
L["tus hechizos dañinos tienen una probabilidad de aumentar %s p. tu daño con hechizos durante %s s"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s446705
L["%s p. de poder de ataque solo en las formas de oso, oso temible y felina"] = { { Stats.AttackPower, }, } -- s446708
L["aumenta %s p. tu poder de ataque cuerpo a cuerpo y a distancia. el efecto dura %s s"] = { { Stats.GenericAttackPower, }, false, } -- s446709
L["aumenta %s p. el poder de sanación durante %s s"] = { { Stats.HealingPower, }, false, } -- s446712
L["controla a un enemigo humanoide de nivel %s o inferior durante %s s, aumenta %s p. el daño mágico que inflige y %s p. el daño físico"] = { false, false, { Stats.SpellDamage, }, { Stats.AverageWeaponDamage, }, } -- s446733
L["envenena al enemigo e inflige %s p. de daño de naturaleza, lo que reduce la resistencia a la naturaleza en %s p. y aumenta el daño de naturaleza y el daño sagrado recibido un %s% durante %s s. solo afecta a los enemigos de niveles inferiores a %s"] = { false, { Stats.NatureResistance, }, false, false, false, reduction = true } -- s447894
L["aumenta hasta un máximo de %s p. el daño infligido con hechizos, hasta un máximo de %s p. la sanación realizada y %s p. la regeneración de maná cada %s s durante %s h. los beneficios de este frasco solo se aplican en áreas bajo la influencia de la pesadilla"] = { { Stats.SpellDamage, }, { Stats.HealingPower, }, { Stats.GenericManaRegen, }, false, false, } -- s448084
L["tus hechizos de daño sagrado te otorgan %s de aumento del daño y %s s de aumento del poder de sanación"] = { { Stats.HealingPower, }, false, ignoreSum = true } -- s449921
L["mejora un %s% tu probabilidad de golpear con todos tus hechizos y ataques mientras estés en forma de oso u oso temible"] = { { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, } -- s449926
L["infligir daño de fuego te otorga %s de poder de ataque, que puede acumularse hasta %s veces. dura %s s"] = { { Stats.AttackPower, }, false, false, ignoreSum = true } -- s449931
L["aumenta la probabilidad de conseguir un golpe crítico con hechizos sagrados un %s%"] = { { Stats.SpellCrit, }, } -- s449933
L["aumenta hasta %s p. el daño y la sanación de los hechizos de naturaleza y sus efectos"] = { { Stats.NatureDamage, Stats.HealingPower, }, } -- s449934
L["el lanzamiento de hechizos tiene la probabilidad de aumentar hasta %s p. el daño y las sanaciones que realizas con tus hechizos de naturaleza durante %s s"] = { { Stats.NatureDamage, }, false, ignoreSum = true } -- s449935
L["obtienes %s de valor de bloqueo %s s después de bloquear"] = { { Stats.BlockValue, }, false, ignoreSum = true } -- s449974
L["fuego rápido te otorga un %s% de velocidad de ataque cuerpo a cuerpo durante %s s"] = { { Stats.MeleeHaste, }, false, ignoreSum = true } -- s450083
L["aumenta un %s% la velocidad de ataque cuerpo a cuerpo"] = { { Stats.MeleeHaste, }, } -- s450084
L["aumenta la probabilidad de conseguir un golpe crítico con todas las armas a distancia un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s450403
L["%s p. de poder de ataque, hasta %s p. de daño aumentado de hechizos y hasta %s p. de sanación aumentada de hechizos"] = { { Stats.GenericAttackPower, }, { Stats.SpellDamage, }, { Stats.HealingPower, }, } -- s450512
L["%s p. de poder de ataque y hasta %s p. de sanación aumentada de hechizos"] = { { Stats.GenericAttackPower, }, false, } -- s450516
L["aumenta un %s% tu probabilidad de golpear con hechizos y ataques"] = { { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, } -- s455858
L["tu mordedura de mangosta también reduce un %s% la probabilidad de esquivar de su objetivo y aumenta un %s% tu probabilidad de golpear durante %s s"] = { { Stats.Dodge, }, { Stats.MeleeHit, Stats.RangedHit, }, false, ignoreSum = true } -- s456389
L["aumenta un %s% tu probabilidad de conseguir un golpe crítico con hechizos y ataques"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- s456489
L["aumenta tu probabilidad de golpe crítico con hechizos y ataques un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- s457323
L["reduce %s p. el poder de ataque cuerpo a cuerpo y a distancia de los enemigos cercanos durante %s s"] = { { Stats.GenericAttackPower, }, false, reduction = true } -- s459165
L["aumenta un %s% tu probabilidad de golpear con todos los hechizos y ataques"] = { { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, } -- s460230
L["aumenta tu velocidad de ataque un %s% durante %s s. no afecta a personajes de nivel %s o más"] = { { Stats.MeleeHaste, }, false, false, } -- s460455
L["añade de forma permanente %s p. de resistencia al fuego a un objeto de la ranura de hombros"] = { { Stats.FireResistance, }, } -- s460963
L["incrementa un %s% la probabilidad de golpe crítico con hechizos sagrados"] = { { Stats.SpellCrit, }, } -- s461953
L["encanta una capa de forma permanente para que otorgue %s p. de resistencia al fuego"] = { { Stats.FireResistance, }, } -- s461129
L["aumenta %s p. tu fuerza. el efecto dura %s s"] = { { Stats.Strength, }, false, } -- s461224
L["aumenta %s p. tu resistencia a la naturaleza. el efecto dura %s s"] = { { Stats.NatureResistance, }, false, } -- s461226
L["aumenta hasta %s p. tu daño de naturaleza. el efecto dura %s s"] = { { Stats.NatureDamage, }, false, } -- s461227
L["activar aluvión de acero ahora te envuelve en llama de las sombras, lo que hace que tus ataques ignoren %s p. de armadura de tu objetivo"] = { { Stats.ArmorPenetration, }, ignoreSum = true } -- s461253
L["los golpes cuerpo a cuerpo tienen una pequeña probabilidad de invocar el espíritu de magmadar para que te ayude en batalla, lo que aumenta un %s% tu velocidad de ataque durante %s s"] = { { Stats.MeleeHaste, }, false, ignoreSum = true } -- s461268
L["tu hechizo golpe de tormenta hace que obtengas %s p. de poder de ataque durante %s s. (es más efectivo con un arma de dos manos)"] = { { Stats.AttackPower, }, false, ignoreSum = true } -- s461298
L["aumenta la sanación de los hechizos y efectos hasta %s p. durante %s s"] = { { Stats.HealingPower, }, false, } -- s461317
L["inspira a todos los miembros de tu grupo o banda a luchar por el valor de azeroth, lo que aumenta un %s% la probabilidad de golpe crítico de todos los hechizos y ataques y otorga %s p. de poder de ataque. dura %s h.\r\n\r\nno se puede combinar con efectos similares de larga duración"] = { { Stats.SpellCrit, Stats.MeleeCrit, Stats.RangedCrit, }, false, false, } -- s461475
L["restaura %s p. de maná durante %s s.  debes permanecer sentado mientras bebes"] = { { Stats.GenericManaRegen, }, false, } -- s461632
L["imbuye el arma del chamán, aumenta %s p. su poder de ataque cuerpo a cuerpo y permite que los ataques cuerpo a cuerpo generen más amenaza cuando se usa esa arma. dura %s hora"] = { { Stats.AttackPower, }, false, } -- s461635
L["aumenta hasta %s p. el daño infligido con hechizos y efectos mágicos"] = { { Stats.SpellDamage, }, } -- s461697
L["golpea a los enemigos cercanos con truenos, aumenta un %s% el tiempo entre sus ataques durante %s s y les inflige %s p. de daño. puede afectar a un máximo de %s objetivos"] = { { Stats.MeleeHaste, }, false, false, false, reduction = true } -- s461810
L["golpea a los enemigos cercanos, aumenta un %s% el tiempo entre sus ataques durante %s s y les inflige %s p. de daño. puede afectar a un máximo de %s objetivos"] = { { Stats.MeleeHaste, }, false, false, false, reduction = true } -- s461830
L["cuando recibes un golpe en combate, hay una probabilidad de hacer huir aterrorizado al atacante durante %s s"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, ignoreSum = true } -- s462229
L["inflige un %s% de daño con arma y deja al objetivo desequilibrado, lo que reduce su habilidad de defensa %s p. durante %s s"] = { false, { Stats.Defense, }, false, reduction = true } -- s462584
L["aumenta un %s% la velocidad de ataque de un aliado durante %s s, pero también le inflige %s p. de daño de naturaleza cada %s s"] = { { Stats.MeleeHaste, }, false, false, false, } -- s462872
L["otorga un sigilo encantado de aguas fluidas que te potencia para aumentar hasta %s p. el daño y la sanación realizados con hechizos, y aumenta el poder de ataque %s p. durante %s h"] = { { Stats.SpellPower, }, { Stats.GenericAttackPower, }, false, } -- s463864
L["cuando se aplica al escudo objetivo, aumenta hasta %s p. el daño y la sanación con hechizos. dura %s minutos"] = { { Stats.SpellPower, }, false, } -- s463868
L["encanta de forma permanente un escudo que añade hasta %s p. de daño a los hechizos y hasta %s p. de sanación a los hechizos de sanación"] = { { Stats.SpellDamage, }, { Stats.HealingPower, }, } -- s463871
L["aumenta hasta %s p. el daño infligido con hechizos y efectos mágicos y hasta $%s p. la sanación infligida con hechizos y efectos mágicos"] = { { Stats.SpellDamage, }, false, } -- s463873
L["aumenta hasta %s p. el daño y la sanación de los efectos y hechizos mágicos"] = { { Stats.SpellPower, }, } -- s466938
L["aumenta hasta %s p. la sanación realizada y hasta %s p. el daño que infligen todos los efectos y hechizos mágicos"] = { { Stats.HealingPower, }, { Stats.SpellDamage, }, } -- s467142
L["aumenta hasta %s p. el daño que infligen los efectos y hechizos arcanos"] = { { Stats.ArcaneDamage, }, } -- s467410
L["aumenta hasta %s p. el daño que infligen los efectos y hechizos de las sombras"] = { { Stats.ShadowDamage, }, } -- s467411
L["aumenta hasta %s p. el daño que infligen los efectos y hechizos de escarcha"] = { { Stats.FrostDamage, }, } -- s467412
L["aumenta hasta %s p. el daño que infligen los efectos y hechizos de naturaleza"] = { { Stats.NatureDamage, }, } -- s467413
L["aumenta hasta %s p. el daño que infligen los efectos y hechizos de fuego"] = { { Stats.FireDamage, }, } -- s467414
L["te sana un %s% y aumenta tu salud máxima un %s% durante %s s"] = { { Stats.Health, }, false, false, } -- s467498
L["energiza a un paladín con luz, lo que aumenta un %s% la velocidad de ataque cuerpo a cuerpo y un %s% la velocidad de lanzamiento de hechizos durante %s s"] = { { Stats.MeleeHaste, }, { Stats.SpellHaste, }, false, } -- s467522
L["restablece un %s% de la salud máxima de tu objetivo, y aumenta un %s% su salud máxima durante %s s"] = { false, { Stats.Health, }, false, } -- s467525
L["da una pequeña probabilidad de imbuir al que lo ejerce con una bendición de los dioses primigenios cuando se inflige daño cuerpo a cuerpo o a distancia. el poder de ataque a distancia y cuerpo a cuerpo aumenta %s p. durante %s s"] = { { Stats.GenericAttackPower, }, false, ignoreSum = true } -- s467743
L["invoca una serpiente embelesada para que te adore, lo que aumenta tu espíritu %s p. durante %s s"] = { { Stats.Spirit, }, false, } -- s467754
L["tu choque de llamas también aumenta un %s% la probabilidad de bloqueo durante %s s o hasta que bloquees un ataque"] = { { Stats.BlockChance, }, false, ignoreSum = true } -- s467889
L["golpea a tu enemigo con relámpagos, inflige %s p. de daño de naturaleza y salta a otros enemigos cercanos. con cada salto, se reduce %s p. la resistencia a la naturaleza de las víctimas. afecta a %s objetivos. además, un ciclón consume al objetivo principal, lo que reduce un %s% su velocidad de ataque durante %s s"] = { false, { Stats.NatureResistance, }, false, { Stats.MeleeHaste, }, false, reduction = true } -- s468156
L["aumenta un %s% tu probabilidad de golpear con todos los hechizos y ataques durante %s s"] = { { Stats.SpellHit, Stats.MeleeHit, Stats.RangedHit, }, false, } -- s468164
L["añade de forma permanente %s p. de aguante y %s p. de intelecto, y aumenta hasta %s p. la sanación a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Intellect, }, { Stats.HealingPower, }, } -- s468314
L["añade de forma permanente %s p. de aguante, un %s% de probabilidad de golpear y aumenta hasta %s p. el daño y la sanación con hechizos a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.SpellHit, Stats.MeleeHit, Stats.RangedHit, }, { Stats.SpellPower, }, } -- s468318
L["añade de forma permanente %s p. de aguante, %s p. de fuerza y %s p. de agilidad a un objeto de la ranura de las piernas o la cabeza. no se acumula con otros encantamientos de la ranura de equipo seleccionada"] = { { Stats.Stamina, }, { Stats.Strength, }, { Stats.Agility, }, } -- s468321
L["añade de forma permanente %s p. de aguante, %s p. de fuerza y %s p. de defensa a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Strength, }, { Stats.Defense, }, } -- s468323
L["añade de forma permanente a un objeto de la casilla para pierna o cabeza %s p. de aguante, %s p. de agilidad y un %s% de probabilidad de golpear. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Agility, }, { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, } -- s468325
L["añade de forma permanente %s p. de aguante y %s p. de defensa, y aumenta hasta %s p. el daño y la sanación con hechizos a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Defense, }, { Stats.SpellPower, }, } -- s468328
L["añade de forma permanente %s p. de aguante y %s p. de fuerza, y aumenta hasta %s p. el daño y la sanación con hechizos a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Strength, }, { Stats.SpellPower, }, } -- s468330
L["añade de forma permanente %s p. de aguante y %s p. de intelecto, y aumenta hasta %s p. el daño y la sanación con hechizos a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Intellect, }, { Stats.SpellPower, }, } -- s468339
L["añade de forma permanente %s p. de aguante, %s p. de agilidad y %s p. de defensa a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Agility, }, { Stats.Defense, }, } -- s468347
L["añade de forma permanente %s p. de aguante y %s p. de fuerza, y aumenta hasta %s p. la sanación y el daño con hechizos a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Strength, }, { Stats.SpellPower, }, } -- s468351
L["añade de forma permanente %s p. de aguante y un %s% de probabilidad de golpear, y aumenta hasta %s p. la sanación y el daño con hechizos a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, { Stats.SpellPower, }, } -- s468354
L["añade de forma permanente %s p. de aguante, %s p. de defensa y un %s% de probabilidad de bloqueo a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Defense, }, { Stats.BlockChance, }, } -- s468362
L["añade de forma permanente %s p. de aguante, un %s% de probabilidad de golpear y %s p. de defensa a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, { Stats.Defense, }, } -- s468368
L["añade de forma permanente %s p. de aguante, %s p. de fuerza y %s p. de agilidad a un objeto de la casilla para pierna o cabeza. no se acumula con a otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Strength, Stats.Agility, }, { Stats.Agility, Stats.Strength, }, } -- s468373
L["añade de forma permanente %s p. de aguante, %s p. de defensa y %s p. de valor de bloqueo con escudo a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Defense, }, { Stats.BlockValue, }, } -- s468376
L["añade de forma permanente %s p. de aguante y %s p. de intelecto, y aumenta hasta %s p. la sanación y el daño con hechizos a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Intellect, }, { Stats.SpellPower, }, } -- s468380
L["añade de forma permanente %s p. de aguante, %s p. de agilidad y %s p. de fuerza a un objeto de la casilla para pierna o cabeza. no se acumula con otros encantamientos de la casilla de equipamiento seleccionada"] = { { Stats.Stamina, }, { Stats.Agility, }, { Stats.Strength, }, } -- s468383
L["alinea al druida con la naturaleza, lo que aumenta un %s% el daño que infligen los hechizos, mejora un %s% los efectos de sanación y aumenta un %s% la probabilidad de golpe crítico de los hechizos durante %s s"] = { false, false, { Stats.SpellCrit, }, false, } -- s468387
L["aumenta hasta %s p. el daño que infligen los efectos y hechizos sagrados"] = { { Stats.HolyDamage, }, } -- s468401
L["aumenta un %s% la velocidad de ataque durante %s s"] = { { Stats.MeleeHaste, }, false, } -- s468408
L["la defensa aumenta %s p"] = { { Stats.Defense, }, } -- s468411
L["aumenta un %s% tu probabilidad de conseguir un golpe crítico con dagas"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- s468436
L["aumenta un %s% la probabilidad de golpe crítico de los hechizos sagrados durante %s s. si choque y terror está grabado, obtienes un %s% más"] = { { Stats.SpellCrit, }, false, false, } -- s468461
L["aumenta %s p. el valor de bloqueo de tu escudo durante %s s, y aumenta %s p. tu ira instantáneamente"] = { { Stats.BlockValue, }, false, false, } -- s468462
L["aumenta tu probabilidad de golpe crítico un %s% y la velocidad de ataque de tu mascota un %s% durante %s s. este hechizo no afecta a las mascotas temporales ni a los demonios subyugados"] = { { Stats.SpellCrit, Stats.MeleeCrit, Stats.RangedCrit, }, { Stats.MeleeHaste, }, false, } -- s468540
L["aumenta hasta %s p. la sanación de los hechizos y los efectos. este efecto no se acumula con otras runas de anillo"] = { { Stats.HealingPower, }, } -- s468758
L["restaura %s p. de maná cada %s s. este efecto no se acumula con otras runas de anillo"] = { { Stats.GenericManaRegen, }, false, } -- s468762
L["aumenta hasta %s p. el daño con hechizos arcanos y de las sombras durante %s s"] = { { Stats.ShadowDamage, Stats.ArcaneDamage, }, false, } -- s468784
L["aumenta hasta %s p. el daño que infligen los efectos y hechizos de las sombras y arcanos"] = { { Stats.ShadowDamage, Stats.ArcaneDamage, }, } -- s468785
L["aumenta la probabilidad de efecto crítico un %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- s469208
L["aumenta el daño con hechizos hasta %s p"] = { { Stats.SpellDamage, }, } -- s469209
L["la sanación realizada aumenta hasta %s p"] = { { Stats.HealingPower, }, } -- s469210
L["permanently enchant a shield to give %s% chance to block"] = { { Stats.BlockChance, }, } -- s469753
L["restaura %s p. de salud durante %s s. debes permanecer sentado mientras comes. si comes durante %s segundos, también aumentará tu daño con hechizos %s p. durante %s min"] = { false, false, false, { Stats.SpellDamage, }, false, ignoreSum = true } -- s470362
L["aumenta la sanación realizada %s p. durante %s min"] = { { Stats.HealingPower, }, false, } -- s470367
L["restaura %s p. de salud durante %s s. debes permanecer sentado mientras comes. si comes durante %s segundos, también aumentará %s p. tu sanación infligida durante %s min"] = { false, false, false, { Stats.HealingPower, }, false, ignoreSum = true } -- s470369
L["cuando se aplica al escudo objetivo, aumenta hasta %s p. el daño y la sanación, y aumenta un %s% la probabilidad de golpe crítico con hechizos. dura %s minutos"] = { { Stats.SpellPower, }, { Stats.SpellCrit, }, false, } -- s471401
L["aumenta hasta %s p. el daño y la sanación realizados con hechizos y efectos mágicos. también aumenta un %s% la probabilidad de asestar un golpe crítico con hechizos"] = { { Stats.SpellPower, }, { Stats.SpellCrit, }, } -- s471402
L["reúne a los miembros de tu grupo o banda cercanos, aumenta un %s% su probabilidad de golpe crítico con hechizos, un %s% su probabilidad de golpe crítico con ataques cuerpo a cuerpo o a distancia, y %s p. su poder de ataque. dura %s h. solo se puede activar con efectos de mundo similares. no lo afectan los cronobeneficios"] = { { Stats.SpellCrit, }, { Stats.MeleeCrit, Stats.RangedCrit, }, { Stats.GenericAttackPower, }, false, } -- s473387
L["invoca una melodía que aumenta un %s% la probabilidad de golpe crítico cuerpo a cuerpo, a distancia o con hechizos, y %s p. los atributos de todos los miembros de tu grupo o banda durante %s h. solo se puede activar con efectos de mundo similares. no lo afectan los cronobeneficios"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, { Stats.AllStats, }, false, } -- s473399
L["aumenta %s p. el poder de ataque, un %s% el aguante y un %s% la probabilidad de golpe crítico con hechizos de todos los miembros de tu grupo o banda durante %s h. solo se puede activar con efectos de mundo similares. no lo afectan los cronobeneficios"] = { { Stats.GenericAttackPower, }, false, { Stats.SpellCrit, }, false, } -- s473403
L["aumenta un máximo de %s p. el daño que infligen los hechizos y efectos"] = { { Stats.SpellDamage, }, } -- s474126
L["los golpes críticos de tus hechizos de daño de naturaleza no periódicos tienen una probabilidad de otorgarte %s p. más de daño con hechizos durante %s s"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s474127
L["reduce %s p. las resistencias mágicas de los objetivos de tus hechizos"] = { { Stats.SpellPenetration, }, } -- s474146
L["enloquece temporalmente a un objetivo enemigo, por lo que su velocidad de ataque aumenta un %s%, y su movimiento, un %s%. además, hace que ataque a sus propios aliados durante %s s. los objetivos afectados reciben un %s% menos de daño"] = { { Stats.MeleeHaste, }, false, false, false, } -- s474400
L["tus ataques cuerpo a cuerpo tienen un %s% menos de probabilidad de ser esquivados o parados"] = { { Stats.DodgeReduction, Stats.ParryReduction, }, } -- s1213201
L["reduce un %s% la probabilidad de que esquiven o paren tus ataques"] = { { Stats.DodgeReduction, Stats.ParryReduction, }, } -- s1213288
L["aumenta %s p. el poder de ataque, y otros %s p. cada %s s. dura %s s"] = { { Stats.GenericAttackPower, }, { Stats.GenericAttackPower, }, false, false, ignoreSum = true } -- s1213366
L["inflige %s p. de daño de naturaleza a los enemigos cercanos, lo que aumenta un %s% el tiempo entre sus ataques y reduce un %s% su velocidad de movimiento durante %s s"] = { false, { Stats.MeleeHaste, }, false, false, reduction = true } -- s1213464
L["encanta de forma permanente una pieza de armadura de torso para aumentar %s p. todas las estadísticas y %s p. la resistencia a la naturaleza"] = { { Stats.AllStats, }, { Stats.NatureResistance, }, } -- s1213616
L["encanta unos guantes de forma permanente para que aumenten hasta %s p. el daño sagrado"] = { { Stats.HolyDamage, }, } -- s1213622
L["encanta unos guantes de forma permanente para que aumenten hasta %s p. el daño arcano"] = { { Stats.ArcaneDamage, }, } -- s1213626
L["aumenta de forma permanente hasta %s p. el daño y la sanación con hechizos de un objeto para el torso, las piernas, las manos o los pies. solo puede usarse en objetos de nivel %s o superior"] = { { Stats.SpellPower, }, false, } -- s1213829
L["aumenta hasta %s p. la sanación realizada por hechizos y efectos mágicos, hasta %s p. el daño que infligen los hechizos y efectos mágicos y %s p. la regeneración de maná cada %s s durante %s h. solo puedes disfrutar del efecto de un frasco a la vez. el efecto persiste aun después de la muerte"] = { { Stats.HealingPower, }, { Stats.SpellDamage, }, { Stats.GenericManaRegen, }, false, false, } -- s1213886
L["aumenta hasta %s p. el daño que infligen los hechizos y efectos mágicos durante %s h. solo puedes disfrutar del efecto de un frasco a la vez. el efecto persiste aun después de la muerte"] = { { Stats.SpellDamage, }, false, } -- s1213892
L["aumenta %s p. el poder de ataque durante %s h. solo puedes disfrutar del efecto de un frasco a la vez. el efecto persiste aun después de la muerte"] = { { Stats.GenericAttackPower, }, false, } -- s1213897
L["aumenta %s p. el aguante y %s p. el valor de defensa durante %s h. solo puedes disfrutar del efecto de un frasco a la vez. el efecto persiste aun después de la muerte"] = { { Stats.Stamina, }, { Stats.Defense, }, false, } -- s1213901
L["aumenta %s p. la agilidad, un %s% la probabilidad de asestar un golpe crítico y %s p. la resistencia a la naturaleza durante %s h"] = { { Stats.Agility, }, { Stats.MeleeCrit, Stats.RangedCrit, }, { Stats.NatureResistance, }, false, } -- s1213904
L["aumenta el daño con hechizos hasta un máximo de %s p. y la resistencia a la naturaleza %s p. durante %s h"] = { { Stats.SpellDamage, }, { Stats.NatureResistance, }, false, } -- s1213914
L["aumenta %s p. la armadura, %s p. la defensa y %s p. la resistencia a la naturaleza durante %s h"] = { { Stats.Armor, }, { Stats.Defense, }, { Stats.NatureResistance, }, false, } -- s1213917
L["aumenta un %s% tu velocidad de ataque"] = { { Stats.MeleeHaste, Stats.RangedHaste, }, } -- s1213971
L["lee el pergamino para invocar la cólera del enjambre y potenciar a tu banda, lo que aumenta hasta %s p. el daño y la sanación realizados con hechizos, y aumenta el poder de ataque %s p. durante %s h"] = { { Stats.SpellPower, }, { Stats.GenericAttackPower, }, false, } -- s1214001
L["cuando se aplica al arma o al escudo objetivo, aumenta hasta %s p. el daño con hechizos y aumenta un %s% la probabilidad de golpe crítico con hechizos. dura %s minutos"] = { { Stats.SpellPower, }, { Stats.SpellCrit, }, false, } -- s1214004
L["te otorga una probabilidad de que, cuando tus hechizos dañinos impacten, el daño de tus hechizos y efectos aumente hasta %s p. durante %s s"] = { { Stats.SpellDamage, }, false, ignoreSum = true } -- s1214280
L["encanta de forma permanente un arma para que añada hasta %s p. de daño a los hechizos"] = { { Stats.SpellPower, }, } -- s1217189
L["encanta de forma permanente unos brazales para que aumenten tu agilidad en %s p"] = { { Stats.Agility, }, } -- s1217203
L["provoca un frenesí rabioso al objetivo, lo que aumenta %s p. su poder de ataque cuerpo a cuerpo pero reduce %s p. su armadura durante %s min. los enemigos rabiosos no huirán e ignorarán los efectos de miedo y horror"] = { { Stats.AttackPower, }, { Stats.Armor, }, false, } -- s1218358
L["tu habilidad bramido nunca falla, y reduce un %s% la probabilidad de que esquiven o paren tus ataques"] = { { Stats.DodgeReduction, Stats.ParryReduction, }, } -- s1218482
L["tu habilidad mano de expiación nunca falla, y reduce un %s% la probabilidad de que esquiven o paren tus ataques"] = { { Stats.DodgeReduction, Stats.ParryReduction, }, } -- s1219205
L["tu habilidad burla nunca falla y reduce un %s% la probabilidad de que esquiven o paren tus ataques"] = { { Stats.DodgeReduction, Stats.ParryReduction, }, } -- s1219356
L["tu habilidad choque de tierra nunca falla cuando se usa como provocación, y reduce un %s% la probabilidad de que esquiven o paren tus ataques"] = { { Stats.DodgeReduction, Stats.ParryReduction, }, } -- s1219382
L["tu habilidad amenaza nunca falla y reduce un %s% la probabilidad de que esquiven o paren tus ataques y de que fallen tus hechizos"] = { { Stats.DodgeReduction, Stats.ParryReduction, Stats.SpellHit, }, } -- s1219474
L["aumenta un %s% tu velocidad de ataque durante %s s"] = { { Stats.MeleeHaste, Stats.RangedHaste, }, false, } -- s1219500
L["refleja el siguiente hechizo ofensivo lanzado sobre el usuario y aumenta las resistencias a todas las escuelas de magia %s p. durante %s s"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, false, } -- s1219501
L["añade de forma permanente hasta %s p. de daño y sanación de hechizos y efectos mágicos a un objeto de la casilla de hombros, y aumenta un %s% tu probabilidad de asestar un golpe crítico con hechizos"] = { { Stats.SpellPower, }, { Stats.SpellCrit, Stats.MeleeCrit, Stats.RangedCrit, }, } -- s1219510
L["añade de forma permanente %s p. de aguante y %s p. de defensa a un objeto de la casilla de hombros"] = { { Stats.Stamina, }, { Stats.Defense, }, } -- s1219511
L["aumenta %s p. el valor de bloqueo de tu escudo durante %s s"] = { { Stats.BlockValue, }, false, } -- s1219520
L["tu habilidad provocar nunca falla y reduce un %s% la probabilidad de que esquiven o paren tus ataques"] = { { Stats.DodgeReduction, Stats.ParryReduction, }, } -- s1219540
L["aumenta un máximo de %s p. el daño y la sanación de los efectos y hechizos mágicos de los miembros del grupo que están en un radio de %s m. las distintas fuentes de este efecto específico no se acumulan"] = { { Stats.SpellPower, }, false, } -- s1219552
L["aumenta un máximo de %s p. la sanación realizada y un máximo de %s p. el daño infligido de los efectos y hechizos mágicos de todos los miembros del grupo que están en un radio de %s. las distintas fuentes de este efecto específico no se acumulan"] = { { Stats.HealingPower, }, { Stats.SpellDamage, }, false, } -- s1219553
L["aumenta un %s% la velocidad de lanzamiento de todos los miembros del grupo que están en un radio de %s m. las distintas fuentes de este efecto específico no se acumulan"] = { { Stats.SpellHaste, }, false, } -- s1219557
L["aumenta un %s% la probabilidad de golpe crítico de todos los miembros del grupo que están en un radio de %s m. las distintas fuentes de este efecto específico no se acumulan"] = { { Stats.SpellCrit, }, false, } -- s1219558
L["encanta un objeto de mano izquierda de forma permanente para que otorgue %s p. de intelecto"] = { { Stats.Intellect, }, } -- s1219577
L["encanta un objeto de mano izquierda de forma permanente para que otorgue %s p. de espíritu"] = { { Stats.Spirit, }, } -- s1219578
L["encanta un objeto de mano izquierda de forma permanente para que otorgue %s p. de intelecto y %s p. de espíritu"] = { { Stats.Intellect, }, { Stats.Spirit, }, } -- s1219579
L["encanta un arma de dos manos de forma permanente para que otorgue %s p. de poder con hechizos"] = { { Stats.SpellPower, }, } -- s1219580
L["encanta un escudo de forma permanente para que otorgue %s p. de aguante"] = { { Stats.Stamina, }, } -- s1219581
L["encanta unos guantes de forma permanente para que otorguen %s p. de fuerza"] = { { Stats.Strength, }, } -- s1219586
L["encanta una capa de forma permanente para que otorgue %s p. de agilidad"] = { { Stats.Agility, }, } -- s1219587
L["aumenta %s p. el poder de ataque de un objetivo aliado durante %s s"] = { { Stats.GenericAttackPower, }, false, } -- s1220545
L["aumenta %s p. la armadura de un objetivo aliado durante %s s"] = { { Stats.Armor, }, false, } -- s1220546
L["mejora un %s% tu probabilidad de asestar un golpe crítico"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- s1220596
L["encanta de forma permanente un escudo para añadir un %s% de probabilidad de golpe crítico"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- s1220623
L["encanta de forma permanente unos brazales que proporcionan %s p. al poder con hechizos"] = { { Stats.SpellPower, }, } -- s1220624
L["aumenta un %s% la velocidad de lanzamiento"] = { { Stats.SpellHaste, }, } -- s1220654
L["aumenta la fuerza del portador %s p., pero también recibe un %s% más el daño de todas las fuentes durante %s s"] = { { Stats.Strength, }, false, false, } -- s1220668
L["aumenta un %s% la velocidad de ataque de un aliado durante %s s"] = { { Stats.MeleeHaste, }, false, } -- s1222564
L["cuando golpeas, otorga la probabilidad de aumentar tu fuerza %s p. y tu velocidad de movimiento un %s% durante %s s"] = { { Stats.Strength, }, false, false, ignoreSum = true } -- s1223341
L["mejora un %s% tu probabilidad de asestar un golpe crítico con hechizos"] = { { Stats.SpellCrit, }, } -- s1223455
L["si comes durante al menos %s s, estarás bien alimentado y ganarás %s p. de fuerza y %s p. de aguante durante %s min"] = { false, { Stats.Strength, }, { Stats.Stamina, }, false, } -- s1225778
L["si comes durante al menos %s s, estarás bien alimentado y ganarás %s p. de agilidad y %s p. de aguante durante %s min"] = { false, { Stats.Agility, }, { Stats.Stamina, }, false, } -- s1225779
L["si comes durante al menos %s s, estarás bien alimentado y ganarás %s p. de daño con hechizos, %s p. de poder de sanación y %s p. de aguante durante %s min"] = { false, { Stats.SpellDamage, }, { Stats.HealingPower, }, { Stats.Stamina, }, false, } -- s1225780
L["si comes durante al menos %s s, estarás bien alimentado y ganarás %s p. de poder de ataque, %s p. de daño con hechizos, %s p. de poder de sanación y %s p. de aguante durante %s min"] = { false, { Stats.AttackPower, }, { Stats.SpellDamage, }, { Stats.HealingPower, }, { Stats.Stamina, }, false, } -- s1225782
L["lanza sobre el objetivo una maldición de temeridad, lo que aumenta %s p. su poder de ataque cuerpo a cuerpo pero disminuye %s p. su armadura durante %s min. los enemigos malditos no huirán e ignorarán los efectos de miedo y horror. un brujo solo puede tener una maldición activa a la vez sobre un mismo objetivo"] = { { Stats.AttackPower, }, { Stats.Armor, }, false, } -- s1225841
L["aumenta un %s% la velocidad de ataque durante %s s cada vez que asestas un golpe crítico"] = { { Stats.MeleeHaste, }, false, ignoreSum = true } -- s1226126
L["tus golpes críticos cuerpo a cuerpo aumentan tu velocidad de ataque un %s% durante %s s"] = { { Stats.MeleeHaste, }, false, ignoreSum = true } -- s1226359
L["tus golpes críticos periódicos otorgan un %s% de celeridad de lanzamiento de hechizos durante %s s, y tu explosión de humo otorga un %s% más de celeridad de lanzamiento de hechizos"] = { { Stats.SpellHaste, }, false, false, ignoreSum = true } -- s1227196
L["la muerte de un compañero despierta una marea de energía que aumenta el daño infligido un %s% y la velocidad de ataque un %s%"] = { false, { Stats.MeleeHaste, }, } -- s1230319
L["te sanas %s p. de salud y tu fuerza aumenta %s p. durante %s s"] = { false, { Stats.Strength, }, false, } -- s1231124
L["restaura %s p. de maná y aumenta %s p. el poder con hechizos durante %s s"] = { false, { Stats.SpellPower, }, false, } -- s1231138
L["aumenta %s p. la defensa y %s p. la armadura durante %s s"] = { { Stats.Defense, }, { Stats.Armor, }, false, } -- s1231254
L["los ataques cuerpo a cuerpo tienen una probabilidad de afligir a tu enemigo con una podredumbre que inflige %s p. de daño de naturaleza cada %s s a todos los enemigos que están en un radio de %s m del taumaturgo durante %s s. la velocidad de ataque de estos objetivos también se reduce un %s%"] = { false, false, false, false, { Stats.MeleeHaste, }, ignoreSum = true, reduction = true } -- s1231258
L["atacar disparando a distancia normal te prepara para la batalla, lo que aumenta %s p. tu defensa y un %s% la velocidad de ataque cuerpo a cuerpo durante %s s"] = { { Stats.Defense, }, { Stats.MeleeHaste, }, false, ignoreSum = true } -- s1231267
L["invoca la voluntad de la montaña, lo que aumenta un %s% el daño físico infligido, %s p. la armadura y un %s% el tamaño durante %s s"] = { false, { Stats.Armor, }, false, false, } -- s1231289
L["la fuerza aumenta\r\n %s p. durante %s s"] = { { Stats.Strength, }, false, } -- s1231339
L["reduce %s p. el intelecto de los enemigos cercanos durante %s min"] = { { Stats.Intellect, }, false, reduction = true } -- s8267
L["reduce la velocidad de lanzamiento de un enemigo un %s% durante %s min"] = { { Stats.SpellHaste, }, false, reduction = true } -- s8272
L["aumenta %s p. el daño con hechizos durante %s h"] = { { Stats.SpellDamage, }, false, } -- s17539
L["aumenta %s p. la agilidad durante %s min"] = { { Stats.Agility, }, false, } -- s18192
L["reduce %s p. el daño físico que inflige un enemigo durante %s s. un brujo solo puede tener una maldición activa sobre cada objetivo"] = { { Stats.AverageWeaponDamage, }, false, reduction = true } -- s18267
L["daño infligido por el objetivo reducido %s p. durante %s s"] = { { Stats.AverageWeaponDamage, }, false, reduction = true } -- s20006
L["aumenta %s p. el poder de ataque cuerpo a cuerpo de los miembros del grupo cercanos durante %s s"] = { { Stats.AverageWeaponDamage, }, false, } -- s24438
L["reduce el poder de ataque cuerpo a cuerpo y a distancia de los enemigos cercanos %s p. durante %s min"] = { { Stats.GenericAttackPower, }, false, reduction = true } -- s28342
L["aumenta %s p. tu poder de ataque cuerpo a cuerpo y tu tamaño durante %s h"] = { { Stats.AttackPower, }, false, } -- s473469
L["reduce %s p. todos los atributos de los enemigos cercanos durante %s s"] = { { Stats.AllStats, }, false, reduction = true } -- s1223265
L["desbloquea tu potencial mientras estás dentro de naxxramas. aumenta un %s%s% tu daño y un %s%s% tu salud por cada pieza de armadura santificada equipada"] = { { Stats.Health, }, false, false, false, } -- s1230224
L["aumenta %s p. la fuerza del portador durante %s s"] = { { Stats.Strength, }, false, } -- s1231551
L["la agilidad aumenta %s p. durante %s s"] = { { Stats.Agility, }, false, } -- s1231406
L["luego de beber el experimento, los ataques cuerpo a cuerpo o a distancia aumentan un %s% tu velocidad de ataque durante %s s. este efecto se acumula hasta %s veces"] = { { Stats.MeleeHaste, }, false, false, ignoreSum = true } -- s1231874
L["aumenta un %s% la celeridad.\r\n\r\neste efecto se acumula"] = { { Stats.MeleeHaste, }, } -- s1232051
L["da %s p. más de armadura a los miembros del grupo cercanos durante %s s. los jugadores solo pueden tener un aura activa por paladín a la vez"] = { { Stats.Armor, }, false, } -- s1233344
L["añade una mira permanente a un arco o arma de fuego que aumenta %s p. el daño que produce"] = { { Stats.AverageWeaponDamage, }, } -- s1235011
L["aumenta un %s% la probabilidad de golpe crítico de los hechizos y un %s% la de ataques cuerpo a cuerpo y a distancia, y otorga %s p. de poder de ataque"] = { { Stats.SpellCrit, }, { Stats.MeleeCrit, Stats.RangedCrit, }, { Stats.GenericAttackPower, }, } -- s1278762
L["aumenta la velocidad de ataque de los aliados cercanos un %s% durante %s min"] = { { Stats.MeleeHaste, }, false, } -- s3631
L["aumenta la velocidad de ataque del taumaturgo un %s% durante %s min"] = { { Stats.MeleeHaste, }, false, } -- s5915
L["te enseña a encantar de forma permanente unos brazales para que añadan hasta %s p. de daño a los hechizos"] = { { Stats.SpellPower, }, } -- i235526
L["te enseña a encantar de forma permanente unos brazales para que aumenten tu agilidad en %s p"] = { { Stats.Agility, }, } -- i235528
L["aprendes una nueva habilidad después de derrotar enemigos %s veces con descarga de relámpagos"] = { { Stats.BlockChance, }, } -- i206386
L["afilado %s"] = { { Stats.AverageWeaponDamage, }, } -- e13
L["armadura reforzada %s"] = { { Stats.Armor, }, } -- e15
L["pesado %s"] = { { Stats.AverageWeaponDamage, }, } -- e19
L["maná %s"] = { { Stats.Mana, }, } -- e24
L["alcance (%s daño)"] = { { Stats.AverageWeaponDamage, }, } -- e30
L["contrapeso %s% de velocidad de ataque"] = { { Stats.MeleeHaste, }, } -- e34
L["salud %s"] = { { Stats.Health, }, } -- e41
L["espíritu %s"] = { { Stats.Spirit, }, } -- e64
L["%s todas las resistencias"] = { { Stats.FireResistance, Stats.NatureResistance, Stats.FrostResistance, Stats.ShadowResistance, Stats.ArcaneResistance, }, } -- e65
L["aguante %s"] = { { Stats.Stamina, }, } -- e66
L["%s daño"] = { { Stats.AverageWeaponDamage, }, } -- e67
L["%s fuerza"] = { { Stats.Strength, }, } -- e68
L["%s aguante"] = { { Stats.Stamina, }, } -- e71
L["%s agilidad"] = { { Stats.Agility, }, } -- e74
L["%s intelecto"] = { { Stats.Intellect, }, } -- e79
L["%s espíritu"] = { { Stats.Spirit, }, } -- e82
L["%s armadura"] = { { Stats.Armor, }, } -- e85
L["%s de defensa"] = { { Stats.Defense, }, } -- e110
L["habilidad con espadas %s"] = { { Stats.WeaponSkill, }, } -- e125
L["habilidad con espadas de dos manos %s"] = { { Stats.WeaponSkill, }, } -- e132
L["habilidad con mazas %s"] = { { Stats.WeaponSkill, }, } -- e139
L["habilidad con mazas de dos manos %s"] = { { Stats.WeaponSkill, }, } -- e146
L["habilidad con hachas de dos manos %s"] = { { Stats.WeaponSkill, }, } -- e160
L["habilidad con dagas %s"] = { { Stats.WeaponSkill, }, } -- e167
L["habilidad con armas de fuego %s"] = { { Stats.WeaponSkill, }, } -- e174
L["habilidad con arco %s"] = { { Stats.WeaponSkill, }, } -- e181
L["golpe crítico %s%"] = { { Stats.MeleeCrit, Stats.RangedCrit, }, } -- e195
L["aumenta la sanación %s"] = { { Stats.HealingPower, }, } -- e206
L["aumenta los efectos de fuego %s"] = { { Stats.FireDamage, }, } -- e213
L["aumenta el daño de fuego %s"] = { { Stats.FireDamage, }, } -- e218
L["aumenta los efectos de naturaleza %s"] = { { Stats.NatureDamage, }, } -- e220
L["aumenta los efectos de escarcha %s"] = { { Stats.FrostDamage, }, } -- e227
L["aumenta los efectos de las sombras %s"] = { { Stats.ShadowDamage, }, } -- e234
L["intelecto %s"] = { { Stats.Intellect, }, } -- e244
L["armadura %s"] = { { Stats.Armor, }, } -- e245
L["agilidad %s"] = { { Stats.Agility, }, } -- e247
L["fuerza %s"] = { { Stats.Strength, }, } -- e248
L["%s resistencia al fuego"] = { { Stats.FireResistance, }, } -- e256
L["aumenta el daño con hechizos %s"] = { { Stats.SpellPower, }, } -- e423
L["muerdepiedras %s"] = { { Stats.AverageWeaponDamage, }, } -- e504
L["%s agilidad / %s espíritu"] = { { Stats.Agility, }, { Stats.Spirit, }, } -- e583
L["%s agilidad / %s intelecto"] = { { Stats.Agility, }, { Stats.Intellect, }, } -- e584
L["%s agilidad / %s aguante"] = { { Stats.Agility, }, { Stats.Stamina, }, } -- e585
L["%s agilidad / %s fuerza"] = { { Stats.Agility, }, { Stats.Strength, }, } -- e586
L["%s intelecto / %s espíritu"] = { { Stats.Intellect, }, { Stats.Spirit, }, } -- e587
L["%s intelecto / %s aguante"] = { { Stats.Intellect, }, { Stats.Stamina, }, } -- e588
L["%s intelecto / %s fuerza"] = { { Stats.Intellect, }, { Stats.Strength, }, } -- e589
L["%s espíritu / %s aguante"] = { { Stats.Spirit, }, { Stats.Stamina, }, } -- e590
L["%s espíritu / %s fuerza"] = { { Stats.Spirit, }, { Stats.Strength, }, } -- e591
L["%s aguante / %s fuerza"] = { { Stats.Stamina, }, { Stats.Strength, }, } -- e592
L["bloqueo %s%"] = { { Stats.BlockChance, }, } -- e763
L["%s resistencia a las sombras"] = { { Stats.ShadowResistance, }, } -- e804
L["todas las estadísticas %s"] = { { Stats.AllStats, }, } -- e847
L["defensa %s"] = { { Stats.Defense, }, } -- e923
L["%s resistencia a la escarcha"] = { { Stats.FrostResistance, }, } -- e926
L["velocidad de ataque %s%"] = { { Stats.MeleeHaste, Stats.RangedHaste, }, } -- e931
L["%s resistencia a lo arcano"] = { { Stats.ArcaneResistance, }, } -- e1243
L["%s resistencia a la naturaleza"] = { { Stats.NatureResistance, }, } -- e1381
L["ps %s"] = { { Stats.Health, }, } -- e1503
L["maná/rf %s/%s"] = { { Stats.Mana, }, { Stats.FireResistance, }, } -- e1523
L["ps/rf %s/%s"] = { { Stats.Health, }, { Stats.FireResistance, }, } -- e1524
L["pa/rf %s/%s"] = { { Stats.Armor, }, { Stats.FireResistance, }, } -- e1525
L["frz/rf %s/%s"] = { { Stats.Strength, }, { Stats.FireResistance, }, } -- e1526
L["agn/rf %s/%s"] = { { Stats.Stamina, }, { Stats.FireResistance, }, } -- e1527
L["agi/rf %s/%s"] = { { Stats.Agility, }, { Stats.FireResistance, }, } -- e1528
L["int/rf %s/%s"] = { { Stats.Intellect, }, { Stats.FireResistance, }, } -- e1529
L["esp/rf %s/%s"] = { { Stats.Spirit, }, { Stats.FireResistance, }, } -- e1530
L["frz/agn %s/%s"] = { { Stats.Strength, Stats.Stamina, }, false, } -- e1531
L["frz/agn/pa/rf %s/%s/%s/%s"] = { { Stats.Strength, Stats.Stamina, }, false, false, { Stats.FireResistance, }, } -- e1532
L["int/esp/maná/rf %s/%s/%s/%s"] = { { Stats.Intellect, Stats.Spirit, }, false, { Stats.Mana, }, { Stats.FireResistance, }, } -- e1543
L["%s poder de ataque"] = { { Stats.AttackPower, Stats.RangedAttackPower, }, } -- e1563
L["%s de poder de ataque"] = { { Stats.AttackPower, Stats.RangedAttackPower, }, } -- e1601
L["daño %s"] = { { Stats.AverageWeaponDamage, }, } -- e1895
L["%s% de bloqueo"] = { { Stats.BlockChance, }, } -- e1983
L["%s poder de ataque a distancia"] = { { Stats.RangedAttackPower, }, } -- e2040
L["%s% de esquivar"] = { { Stats.Dodge, }, } -- e2078
L["%s daño con hechizos arcano"] = { { Stats.ArcaneDamage, }, } -- e2079
L["%s daño con hechizos de las sombras"] = { { Stats.ShadowDamage, }, } -- e2117
L["%s daño con hechizos de fuego"] = { { Stats.FireDamage, }, } -- e2155
L["%s salud cada %s s"] = { false, { Stats.HealthRegen, }, } -- e2413
L["%s daño con hechizos sagrado"] = { { Stats.HolyDamage, }, } -- e2193
L["%s daño con hechizos de escarcha"] = { { Stats.FrostDamage, }, } -- e2231
L["%s daño con hechizos de naturaleza"] = { { Stats.NatureDamage, }, } -- e2269
L["%s de hechizos de sanación"] = { { Stats.HealingPower, }, } -- e2307
L["%s maná cada %s s"] = { { Stats.GenericManaRegen, }, false, } -- e2363
L["%s de maná cada %s s"] = { { Stats.GenericManaRegen, }, false, } -- e2381
L["daño con hechizos de escarcha %s"] = { { Stats.FrostDamage, }, } -- e2443
L["daño con hechizos %s"] = { { Stats.SpellPower, }, } -- e2504
L["hechizos de sanación %s"] = { { Stats.HealingPower, }, } -- e2505
L["%s% de golpe"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- e2523
L["sanación y daño con hechizos %s"] = { { Stats.SpellPower, }, } -- e2544
L["esquivar %s%"] = { { Stats.Dodge, }, } -- e2545
L["regeneración de maná: %s cada %s s"] = { { Stats.GenericManaRegen, }, false, } -- e2565
L["defensa %s/aguante %s /valor de bloqueo %s"] = { { Stats.Defense, }, { Stats.Stamina, }, { Stats.BlockValue, }, } -- e2583
L["defensa %s/aguante %s /hechizos de sanación %s"] = { { Stats.Defense, }, { Stats.Stamina, }, { Stats.HealingPower, }, } -- e2584
L["poder de ataque %s/esquivar %s%"] = { { Stats.AttackPower, }, { Stats.Dodge, }, } -- e2585
L["poder de ataque a distancia %s/aguante %s/golpe %s%"] = { { Stats.RangedAttackPower, }, { Stats.Stamina, }, { Stats.MeleeHit, Stats.RangedHit, }, } -- e2586
L["sanación y daño con hechizos %s/intelecto %s"] = { { Stats.SpellPower, }, { Stats.Intellect, }, } -- e2587
L["sanación y daño con hechizos %s/golpe con hechizos %s%"] = { { Stats.SpellPower, }, { Stats.SpellHit, }, } -- e2588
L["sanación y daño con hechizos %s/aguante %s"] = { { Stats.SpellPower, }, { Stats.Stamina, }, } -- e2589
L["regeneración de maná %s/aguante %s/hechizos de sanación %s"] = { { Stats.GenericManaRegen, }, { Stats.Stamina, }, { Stats.HealingPower, }, } -- e2590
L["intelecto %s/aguante %s/hechizos de sanación %s"] = { { Stats.Intellect, }, { Stats.Stamina, }, { Stats.HealingPower, }, } -- e2591
L["daño con hechizos y sanación %s"] = { { Stats.SpellPower, }, } -- e2605
L["daño y hechizos de sanación %s"] = { { Stats.SpellPower, }, } -- e2607
L["daño de las sombras %s"] = { { Stats.ShadowDamage, }, } -- e2614
L["daño de escarcha %s"] = { { Stats.FrostDamage, }, } -- e2615
L["daño de fuego %s"] = { { Stats.FireDamage, }, } -- e2616
L["sanación %s y %s de maná cada %s s"] = { { Stats.HealingPower, }, { Stats.GenericManaRegen, }, false, } -- e2715
L["aguante %s y armadura %s"] = { { Stats.Stamina, }, { Stats.Armor, }, } -- e2716
L["poder de ataque %s y %s% de golpe crítico"] = { { Stats.AttackPower, Stats.AttackPower, Stats.RangedAttackPower, Stats.RangedAttackPower, }, { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- e7884
L["daño con hechizos %s y %s% de golpe crítico con hechizos"] = { { Stats.SpellPower, }, { Stats.SpellCrit, }, } -- e2721
L["%s de esquivar"] = { { Stats.DodgeRating, }, } -- e2815
L["%s de golpe crítico"] = { { Stats.CritRating, }, } -- e2823
L["%s de celeridad"] = { { Stats.HasteRating, }, } -- e3726
L["%s a todos los atributos"] = { { Stats.Stamina, }, } -- e7007
L["golpe %s%"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- e7098
L["%s daño con hechizos y sanación"] = { { Stats.SpellPower, }, } -- e7325
L["%s hechizos de sanación"] = { { Stats.HealingPower, }, } -- e7326
L["%s ps"] = { { Stats.Health, }, } -- e7565
L["aguante %s/intelecto %s/hechizos de sanación %s"] = { { Stats.Stamina, }, { Stats.Intellect, }, { Stats.HealingPower, }, } -- e7620
L["aguante %s/golpe %s%/sanación y daño con hechizos %s"] = { { Stats.Stamina, }, { Stats.SpellHit, Stats.MeleeHit, Stats.RangedHit, }, { Stats.SpellPower, }, } -- e7614
L["aguante %s/fuerza %s/agilidad %s"] = { { Stats.Stamina, }, { Stats.Strength, }, { Stats.Agility, }, } -- e7615
L["aguante %s/fuerza %s/defensa %s"] = { { Stats.Stamina, }, { Stats.Strength, }, { Stats.Defense, }, } -- e7616
L["aguante %s/agilidad %s/golpe %s%"] = { { Stats.Stamina, }, { Stats.Agility, }, { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, } -- e7617
L["aguante %s/defensa %s/sanación y daño con hechizos %s"] = { { Stats.Stamina, }, { Stats.Defense, }, { Stats.SpellPower, }, } -- e7618
L["aguante %s/fuerza %s/sanación y daño con hechizos %s"] = { { Stats.Stamina, }, { Stats.Strength, }, { Stats.SpellPower, }, } -- e7619
L["aguante %s/intelecto %s/sanación y daño con hechizos %s"] = { { Stats.Stamina, }, { Stats.Intellect, }, { Stats.SpellPower, }, } -- e7621
L["aguante %s/agilidad %s/defensa %s"] = { { Stats.Stamina, }, { Stats.Agility, }, { Stats.Defense, }, } -- e7624
L["aguante %s/defensa %s/probabilidad de bloqueo %s%"] = { { Stats.Stamina, }, { Stats.Defense, }, { Stats.BlockChance, }, } -- e7629
L["aguante %s/golpe %s%/defensa %s"] = { { Stats.Stamina, }, { Stats.MeleeHit, Stats.RangedHit, Stats.SpellHit, }, { Stats.Defense, }, } -- e7631
L["aguante %s/defensa %s/valor de bloqueo %s"] = { { Stats.Stamina, }, { Stats.Defense, }, { Stats.BlockValue, }, } -- e7633
L["aguante %s/agilidad %s/fuerza %s"] = { { Stats.Stamina, }, { Stats.Agility, }, { Stats.Strength, }, } -- e7635
L["daño sagrado %s"] = { { Stats.HolyDamage, }, } -- e7646
L["daño arcano %s"] = { { Stats.ArcaneDamage, }, } -- e7647
L["%s daño con hechizos"] = { { Stats.SpellPower, }, } -- e7655
L["%s intelecto y %s espíritu"] = { { Stats.Intellect, }, { Stats.Spirit, }, } -- e7661
L["%s poder con hechizos"] = { { Stats.SpellPower, }, } -- e7662
L["%s% probabilidad de golpe crítico"] = { { Stats.MeleeCrit, Stats.RangedCrit, Stats.SpellCrit, }, } -- e7664
L["daño con hechizos %s y %s% de golpe crítico"] = { { Stats.SpellPower, }, { Stats.SpellCrit, Stats.MeleeCrit, Stats.RangedCrit, }, } -- e7883
L["aguante %s y defensa %s"] = { { Stats.Stamina, }, { Stats.Defense, }, } -- e7885
L["mira de precisión para aves de corral (%s daño)"] = { { Stats.AverageWeaponDamage, }, } -- e7944
L["mira de precisión para aves de corral (%s% de golpe)"] = { { Stats.MeleeHit, Stats.RangedHit, }, } -- e8057
