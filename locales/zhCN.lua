...
L["Ignore extra sockets"] = "忽略额外插槽"
L["Ignore sockets from professions or consumable items when calculating the stat summary"] = "计算时忽略专业或消耗品提供的额外插槽"
...
L["Show profile name before stat summary"] = "在属性统计前显示配置文件名称"
...
L["Include block chance in Avoidance"] = "在躲避统计中显示格挡几率"
L["Enable to include block chance in Avoidance, Disable for only dodge, parry, miss"] = "启用该选项后将在躲避统计中加入格挡几率，禁用将仅显示躲闪，招架，未击中"
L["Stat - Basic"] = "属性 - 基本"
...
L["Avoidance <- Dodge, Parry, Miss, Block (Optional)"] = "躲避 ← 躲闪, 招架, 怪物未命中, 格挡(可选)"
L["Gems"] = "宝石"
...
L["Swap Profiles"] = "切换配置文件"
L["Swap Profile Keybinding"] = "配置文件切换快捷键"
L["Use a keybind to swap between Primary and Secondary Profiles.\n\nIf \"Enable spec profiles\" is enabled, will use the Primary and Secondary Talents profiles, and will preview items with that spec's talents, glyphs, and passives.\n\nYou can re-use an existing keybind! It will only be used for RatingBuster when an item tooltip is shown."] = "使用快捷键在主配置和副配置之间进行切换。\n\n如果勾选了“启用专精配置文件”，插件将使用主/副天赋对应的配置方案，并在预览装备时自动应用该天赋下的天赋点、雕文和被动技能。\n\n你可以复用现有的快捷键！只有当鼠标提示框显示时，该快捷键才会触发RatingBuster的切换功能。"
L["Primary Profile"] = "主配置文件"
L["Select the primary profile for use with the swap profile keybind. If spec profiles are enabled, this will instead use the Primary Talents profile."] = "选择用于快捷键切换的主配置文件。如果启用了专精配置文件，则会使用主天赋的配置方案。"
L["Secondary Profile"] = "副配置文件"
L["Select the secondary profile for use with the swap profile keybind. If spec profiles are enabled, this will instead use the Secondary Talents profile."] = "选择用于快捷键切换的副配置文件。如果启用了专精配置文件，则会使用副天赋的配置方案。"
...
L["statPatterns"] = {
	...
	[StatLogic.Stats.DefenseRating] = { "防御等级" },
	[StatLogic.Stats.Defense] = { DEFENSE:lower() },
	[StatLogic.Stats.DodgeRating] = { "躲闪等级", "躲闪" },
	[StatLogic.Stats.BlockRating] = { "格挡等级", "格挡" },
	...
	[StatLogic.Stats.HighestSecondary] = { "精通", "急速等级", "急速", "加速等级", "爆击等级", "爆击", },
...
L[StatLogic.Stats.AllStats] = "所有属性"
